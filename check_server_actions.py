...
def main():
    ...
    for root, dirs, files in os.walk(server_actions_dir):
        # Skip node_modules and .next
        dirs[:] = [d for d in dirs if d not in ['node_modules', '.next']]
        
        for file in files:
            if file.startswith('actions') and file.endswith('.ts') and not file.endswith('.test.ts'):
    ...
    print(f"\nServer Actions - Total remaining direct Prisma write violations: {violations_count}")
    ...
    # Save results
    with open('server_actions_violations_report.json', 'w') as f:
        json.dump({
    ...
    print("\nReport saved to server_actions_violations_report.json")
