...
async function main() {
  ...
  try {
    ...
    console.log("J3: Page 1 (limit=2):", page1R.status, "tasks:", tasks1.length, "total:", pagination1?.total || "N/A");
    ...
    console.log("\nJ3: Page 2 (limit=2):", page2R.status, "tasks:", tasks2.length);

    // Check for off-by-one: page 1 and page 2 should have different items
    const overlap = tasks1.filter(t1 => tasks2.some(t2 => t2.id === t1.id)).length;
    console.log("  Overlap between page 1 and 2:", overlap, overlap === 0 ? "✓ No duplicates" : "⚠ Duplicates found");
    ...
    console.log("\nJ3: Page 3 (beyond data):", page3R.status, "tasks:", tasks3.length);

    // === J3: Sorting on Tasks ===
    console.log("\n--- J3: Task Sorting ---");

    // Sort by priority ascending
    const sortPriR = await api(page, "GET", "/api/kitchen/tasks?sortBy=priority&sortOrder=asc");
    const tasksPri = sortPriR.data?.tasks || [];
    console.log("J3: Sort by priority asc:", sortPriR.status, "tasks:", tasksPri.length);

    // Sort by priority descending
    const sortPriDescR = await api(page, "GET", "/api/kitchen/tasks?sortBy=priority&sortOrder=desc");
    const tasksPriDesc = sortPriDescR.data?.tasks || [];
    console.log("J3: Sort by priority desc:", sortPriDescR.status, "tasks:", tasksPriDesc.length);
    ...
    // Sort by createdAt
    const sortCreatedR = await api(page, "GET", "/api/kitchen/tasks?sortBy=createdAt&sortOrder=desc");
    const tasksCreated = sortCreatedR.data?.tasks || [];
    console.log("J3: Sort by createdAt desc:", sortCreatedR.status, "tasks:", tasksCreated.length);
    ...
    console.log("J3: Events page 1 (limit=5):", eventsPage1R.status, "events:", events1.length, "total:", eventsPagination?.total || "N/A");
    ...
    const eventsOverlap = events1.filter(e1 => events2.some(e2 => e2.id === e1.id)).length;
    console.log("J3: Events page 2:", eventsPage2R.status, "events:", events2.length, "overlap:", eventsOverlap === 0 ? "✓ No dupes" : "⚠ dupes");

    // === J3: Sort Events ===
    console.log("\n--- J3: Event Sorting ---");
    const sortEventsR = await api(page, "GET", "/api/events?sortBy=name&sortOrder=asc");
    const eventsSorted = sortEventsR.data?.events || sortEventsR.data?.data || [];
    console.log("J3: Events sort by name asc:", sortEventsR.status, "events:", eventsSorted.length);
    ...
    console.log("J3 — Pagination (tasks):     " + (paginationWorks ? "✓ PASS" : "✗ " + page1R.status));
    console.log("J3 — No page overlap:        " + (noOverlap ? "✓ PASS" : "⚠ FAIL (" + overlap + " duplicates)"));
    console.log("J3 — Page beyond data:      " + (tasks3.length === 0 ? "✓ PASS (empty)" : "⚠ " + tasks3.length + " items"));
    console.log("J3 — Sort (tasks):           " + (sortWorks ? "✓ PASS" : "✗ " + sortPriR.status));
    console.log("J3 — Events pagination:      " + (eventsPage1R.status === 200 ? "✓ PASS" : "✗ " + eventsPage1R.status));
    console.log("J3 — Events sorting:         " + (sortEventsR.status === 200 ? "✓ PASS" : "✗ " + sortEventsR.status));
    console.log("=".repeat(60));
