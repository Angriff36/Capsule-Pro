...
async function main() {
  ...
  await context.route(/\/api\//, async route => {
    try {
      ...
      if (!resp.ok) {
        ...
        failedRequests.push({ url: route.request().url(), status: resp.status, body: body.substring(0, 200) });
      }
  ...
  if (await tryAgain.isVisible()) {
    ...
    const h1After = await page.locator("h1").textContent().catch(() => "none");
    console.log("After retry heading:", h1After);
  }

  console.log("\nFailed API requests:", JSON.stringify(failedRequests, null, 2));
