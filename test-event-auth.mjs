 ...
 * Uses Clerk test credentials to authenticate, then creates a minimal event.
 */
...
async function createAuthenticatedContext(browser) {
  ...
  const storageStatePath = path.join(__dirname, "e2e/.auth/nexus-test-state.json");
  ...
  const status = await page.evaluate(
    async ({ email, code }) => {
      ...
      const attemptResponse = await signIn.attemptFirstFactor({ strategy: "email_code", code });
...
async function runTest() {
  ...
  try {
    ...
    const res = await page.goto(`${BASE}/events`, { waitUntil: "networkidle", timeout: 15000 });
    ...
    if (await fab.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await modal.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await page.goto(`${BASE}/events/new`, { waitUntil: "networkidle", timeout: 10000 });
      }
    }
    ...
    if (await dateInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      const anyDate = page.locator("input[type='date'], input[id='eventDate']").first();
    ...
    // Verify in search
    console.log("\n🔍 Verifying in search...");
    await page.goto(`${BASE}/search?q=${encodeURIComponent(EVENT_TITLE)}`, { waitUntil: "networkidle", timeout: 10000 });
  ...
  return { eventTitle: EVENT_TITLE, eventId, searchFound: true, listFound: true };
}
