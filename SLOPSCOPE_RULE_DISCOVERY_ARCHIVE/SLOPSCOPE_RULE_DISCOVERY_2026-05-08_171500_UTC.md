...
---
## [2026-05-08 08:52] Rule Discovery — skeleton_crud.raw_sql_with_any_cast_when_prisma_model_exists

### Finding
API routes across the `facilities/`, `procurement/`, and `logistics/` modules use Prisma's `$queryRaw` with `as any[]` type casts to perform CRUD operations against database tables, despite corresponding Prisma models existing in the schema. This bypasses TypeScript type safety, loses Prisma's built-in transaction support, and creates a maintenance hazard where schema changes (column renames, type changes) silently break routes at runtime.

### Evidence
- File: `apps/api/app/api/facilities/work-orders/commands/create/route.ts` (lines 60-92)
- Snippet:
```typescript
...
```
- File: `apps/api/app/api/procurement/purchase-orders/commands/create/route.ts` (lines 29-73)
- Snippet:
```typescript
const countResult = await database.$queryRaw`
...
const poResult = await database.$queryRaw`
  INSERT INTO tenant_inventory.purchase_orders (...) VALUES (...)
  RETURNING ...
`;
const po = (poResult as any[])[0];
```
- File: `packages/database/prisma/schema.prisma` (line 6019, 2123) — `MaintenanceWorkOrder` and `PurchaseOrder` models exist with full column definitions, indexes, and relations
- Total scope: 45 occurrences of `as any[]` casts across facilities (work-orders, schedules, areas, assets), procurement (vendors, budgets, purchase-orders), and logistics (drivers, vehicles) command routes

### Why this matters
The Prisma schema defines `MaintenanceWorkOrder`, `FacilityArea`, `FacilityAsset`, `PurchaseOrder`, `Vehicle`, `Driver` and many more models with full type-safe field definitions, relations, and indexes. Yet the API routes that create, update, and delete these entities use raw SQL with `as any[]` casts instead of the generated Prisma client. Consequences:
1. **No compile-time safety** — column renames or type changes in Prisma schema silently break routes; TypeScript cannot catch mismatches because `any[]` disables all type checking on query results.
2. **Lost transaction safety** — the PO create route (line 60-71) inserts line items in a non-transactional `for` loop; a failure mid-loop leaves orphaned items with no rollback.
3. **Lost relation support** — raw SQL cannot leverage Prisma's `include` for eager-loading related entities; each route must manually join or fetch relations separately.
4. **Maintenance burden** — every column change requires hunting down raw SQL strings across multiple route files instead of getting compiler errors from Prisma client usage.

### Proposed detector rule
```json
{
  "id": "skeleton_crud.raw_sql_with_any_cast_when_prisma_model_exists",
  "title": "Raw SQL CRUD with any[] cast when Prisma model exists for same table",
  "category": "skeleton_crud",
  "severity": "medium",
  "confidence": 0.75,
  "detector_type": "hybrid",
  "language_targets": ["typescript", "javascript"],
  "pattern": "\\$queryRaw",
  "negative_patterns": ["\\$queryRawUnsafe"],
  "evidence_required": {
    "file_must_match": ["app/api/.*/commands/(create|update|delete|receive|complete)/route\\.(t|j)sx?$"],
    "companion_pattern": "\\(.*as any\\[\\]\\)"
  },
  "false_positive_controls": {
    "skip_test_files": true,
    "skip_doc_files": true,
  ...
  "user_impact": "Data integrity risk: non-transactional raw SQL writes can leave the database in inconsistent states. Column renames silently break routes at runtime with no compiler warning, causing 500 errors on user actions that previously worked.",
  "repair_guidance": "Replace $queryRaw INSERT/UPDATE/DELETE with the corresponding Prisma model's create/update/delete methods. Wrap multi-step operations in database.$transaction(). The Prisma models for these tables already exist in schema.prisma — use them. For SELECT queries that genuinely need complex joins or aggregations, keep $queryRaw but add explicit generic type parameters instead of casting to any[].",
  "example_source": {
    "file": "apps/api/app/api/facilities/work-orders/commands/create/route.ts",
    "line_or_snippet": "const count = (countResult as any[])[0]?.count || 0;"
  }
}
```

### Implementation note
The regex-based pass flags files using `$queryRaw` with `as any[]` casts in command (write) routes. A cross-file verification step should parse the Prisma schema to extract all `@@map` table-name mappings, then verify that tables referenced in flagged `$queryRaw` calls have corresponding Prisma models. This would raise confidence from 0.75 to 0.9+ by confirming the ORM model exists but is being bypassed. Additionally, flag non-transactional loops containing `$queryRaw` writes (the `for` loop without `$transaction` pattern in PO create) as a secondary severity bump. Official docs not required: generic implementation-evidence rule.
---
## [2026-05-08 16:15] Rule Discovery — skeleton_crud.integration_history_endpoint_always_empty

### Finding
The QuickBooks integration settings page includes a full "Export History" section with loading spinners, empty-state messaging, and a table renderer for past exports. It calls `GET /api/integrations/quickbooks/history` on mount. The backend endpoint always returns `{ exports: [] }` with the comment "Placeholder — no persistence model yet." Meanwhile, three actual export endpoints (bills, invoices, payroll) perform real work — they query the database, generate QuickBooks-format files, upload to object storage, and return download URLs. But none of them record an export history entry. The history UI will perpetually show "No exports yet" regardless of how many exports the user has performed, creating the illusion that the feature exists while silently discarding the audit trail.

### Evidence
- File: `apps/api/app/api/integrations/quickbooks/history/route.ts`
- Snippet: `// Placeholder — no persistence model yet\n    return NextResponse.json({ exports: [] });`
- File: `apps/app/app/(authenticated)/settings/integrations/page.tsx` (line 1635)
- Snippet: `const res = await apiFetch("/api/integrations/quickbooks/history");`
- File: `apps/app/app/(authenticated)/settings/integrations/page.tsx` (line 2000-2006)
- Snippet: `<CardTitle>Export History</CardTitle>\n<CardDescription>Recent QuickBooks file exports</CardDescription>`
- Real export endpoints that don't record history: `apps/api/app/api/inventory/purchase-orders/export/quickbooks/route.ts`, `apps/api/app/api/events/export/quickbooks/route.ts`, `apps/api/app/api/payroll/export/quickbooks/route.ts`

### Why this matters
Users who export QuickBooks files expect to see a log of past exports — filenames, dates, formats, amounts — for reconciliation and compliance. The UI presents this as a working feature (it loads, shows a spinner, has an empty state message). But it can never show data because the history endpoint is a hardcoded stub and no export route writes to any history table. This is integration theater: the export feature itself works, but the audit/verification layer around it is pure facade.

### Proposed detector rule
```json
{
  "id": "skeleton_crud.integration_history_endpoint_always_empty",
  "title": "Integration history/listing endpoint returns hardcoded empty array while sibling export routes perform real work",
  "category": "skeleton_crud",
  "severity": "medium",
  "confidence": 0.8,
  "detector_type": "hybrid",
  "language_targets": ["typescript", "javascript"],
  "pattern": "return\\s+NextResponse\\.json\\(\\{\\s*(exports|history|logs|records|items)\\s*:\\s*\\[\\s*\\]\\s*\\}\\)",
  "negative_patterns": ["database\\.", "findMany\\(", "\\$queryRaw\\(", "prisma\\.", "await.*\\."],
  "evidence_required": {
    "path_match": ["integrations/.*/(history|log|activity|exports)/route\\.(t|j)sx?$"],
    "min_pattern_count": 1
  },
  "false_positive_controls": {
    "skip_test_files": true,
  ...
  "user_impact": "User sees a polished 'Export History' or 'Activity Log' UI section that will always show zero entries, even after performing real exports. They cannot verify past exports, reconcile accounting data, or audit integration usage. The feature appears complete but provides zero functionality.",
  "repair_guidance": "Create a persistence model for export history (e.g., a quickbooks_export_logs table with tenant_id, export_type, filename, format, record_count, total_amount, file_url, created_at). Add an insert call at the end of each QuickBooks export route. Update the history GET endpoint to query this table instead of returning an empty array.",
  "example_source": {
    "file": "apps/api/app/api/integrations/quickbooks/history/route.ts",
    "line_or_snippet": "// Placeholder — no persistence model yet\n    return NextResponse.json({ exports: [] });"
  }
}
```

### Implementation note
The regex pass flags GET routes under `integrations/*/history/` (or similar audit paths) that return hardcoded empty arrays. A cross-file verification step should check for sibling POST routes in the same integration directory that perform real database/storage operations (contain `findMany`, `$queryRaw`, `uploadFile`, etc.) to confirm the history endpoint is the hollow shell in an otherwise real integration. This distinguishes from genuinely empty history endpoints in new features that haven't been built yet. Official docs not required: generic implementation-evidence rule.
---
## [2026-05-08 16:27] Rule Discovery — dashboard_illusion.logistics_tracking_uses_random_gps_coordinates

### Finding
The logistics tracking endpoint (`GET /api/logistics/tracking`) presents itself as a real-time delivery tracking API that returns driver names, vehicle info, shipment status, estimated arrival, and live GPS position data. The endpoint performs substantial database work — fetching active/completed shipments, delivery routes, driver/vehicle/location/supplier info, and building structured timeline objects. However, the GPS position data is entirely fabricated: latitude, longitude, heading, and speed are all generated from `Math.random()` calls with hardcoded Los Angeles coordinates as a base. The comment on line 243 explicitly admits this: "In production, this would come from GPS tracking hardware/SDK."

No GPS SDK, fleet tracking service, geolocation provider, or mobile telemetry integration exists anywhere in the codebase. The browser CSP headers actively block geolocation access (`geolocation=()` in both `apps/api/next.config.ts` and `apps/app/next.config.ts`). The mobile app has no location-tracking or GPS-reporting code. The `position` object returned to the frontend is indistinguishable in structure from a real GPS feed — it has `lat`, `lng`, `heading`, `speed`, and `updatedAt` fields — but every value is random noise around a fixed point in LA, regardless of where the actual delivery destination is.

### Evidence
- File: `apps/api/app/api/logistics/tracking/route.ts`
- Snippet (lines 242-270):
  ```typescript
  ...
  ```
- CSP header blocking geolocation: `apps/api/next.config.ts` line 89: `value: "camera=(), microphone=(), geolocation=()"`
- Cross-reference: grep for GPS/gps/geolocation/telemetry SDK/fleet tracking across the codebase returns zero results for any actual location tracking integration
- Cross-reference: the `position` object is consumed by the frontend tracking UI as if it were real GPS data

### Why this matters
A catering company's logistics dashboard shows drivers moving on a map in real-time. Operations staff use this to estimate delivery times, coordinate kitchen timing, and communicate with clients about when food will arrive. The position data is random noise — the "live tracking" dots jump randomly around a fixed point in Los Angeles on every API call. Heading changes direction randomly. Speed fluctuates between 20-40 mph randomly. None of this reflects actual vehicle movement. An operations manager looking at the tracking dashboard believes they are seeing real-time delivery progress when they are seeing animated dice rolls. This is dashboard illusion at its most literal: a convincing-looking data visualization layer built on fabricated input data.

### Proposed detector rule
```json
{
  "id": "dashboard_illusion.logistics_tracking_uses_random_gps_coordinates",
  "title": "Tracking/location endpoint returns Math.random()-generated GPS coordinates instead of real position data",
  "category": "dashboard_illusion",
  "severity": "high",
  "confidence": 0.92,
  "detector_type": "regex",
  ...
  "negative_patterns": [
    "GPS|gps|geolocation|navigator\\.geolocation|LocationServices",
    "@mapbox|@google.*maps|@turfjs",
    "telemetry.*location|fleet.*track|vehicle.*position"
  ],
  "evidence_required": {
    "file_must_match": ["(tracking|location|fleet|delivery|shipment).*(route|handler|api)"],
    "companion_pattern": "(lat|latitude|lng|longitude|heading|speed|position)\\s*[:=]"
  },
  "false_positive_controls": {
    "skip_test_files": true,
    "skip_doc_files": true,
  ...
  "user_impact": "Operations staff see a live-tracking dashboard with vehicles moving on a map, but all position/heading/speed data is random noise generated by Math.random(). They cannot make reliable decisions about delivery timing, kitchen coordination, or client communication based on this data. The dashboard creates false confidence in logistics visibility.",
  "repair_guidance": "Integrate a real GPS tracking solution: (1) add GPS hardware/SDK to delivery vehicles or use driver mobile app location reporting, (2) create a position ingestion endpoint that stores real GPS coordinates, (3) replace the Math.random() position generation with queries to the stored position data, (4) derive heading and speed from consecutive position readings rather than random values. At minimum, remove the fabricated position data and show 'Position unavailable' rather than fake coordinates.",
  "example_source": {
    "file": "apps/api/app/api/logistics/tracking/route.ts",
    "line_or_snippet": "const jitter = (Math.random() - 0.5) * 0.01;\n\nconst position = {\n  lat: shipment.status === \"in_transit\" ? baseLat + jitter : baseLat - 0.02,\n  lng: shipment.status === \"in_transit\" ? baseLng + jitter : baseLng - 0.01,\n  heading: shipment.status === \"in_transit\" ? Math.floor(Math.random() * 360) : 0,\n  speed: shipment.status === \"in_transit\" ? 20 + Math.floor(Math.random() * 20) : 0,\n};"
  }
}
```

### Implementation note
Detector flags tracking/logistics API routes that use `Math.random()` to generate position-related fields (lat, lng, heading, speed). The `companion_pattern` ensures the file is actually producing coordinate/position data, not just using random numbers for unrelated purposes (like cache-busting or ID generation). The `negative_patterns` check for the presence of any real location/GPS SDK integration in the same file — if a real geolocation provider is also imported, the `Math.random()` may be used for simulation/testing alongside real data. Official docs not required: generic implementation-evidence rule.
---
## [2026-05-08 16:45] Rule Discovery — security_theater.queryrawunsafe_with_string_interpolation

### Finding
The staffing coverage endpoint at `/api/staffing/coverage` uses Prisma's `$queryRawUnsafe` with user-supplied `locationId` directly interpolated into the SQL query string via template literal `${locationFilter}`. While `$1`/`$2`/`$3` parameterized placeholders are used for tenantId/start/end, the `locationId` is manually escaped with single-quote replacement only (`replace(/'/g, "''")`) — a technique that is insufficient to prevent all SQL injection vectors (backslash escapes, multi-byte character attacks, encoding tricks in certain collations). The endpoint presents itself as a production-grade analytics API with auth, tenant isolation, and Sentry error reporting, but contains a textbook SQL injection vulnerability.

### Evidence
- File: `apps/api/app/api/staffing/coverage/route.ts`
- Line 67: `? \`AND ss.location_id = '${locationId.replace(/'/g, "''")}'\``
- Line 71, 90, 114, 130, 149: Five `$queryRawUnsafe` calls all use the same `${locationFilter}` interpolation
- The `locationId` comes from `request.nextUrl.searchParams.get("locationId")` — fully user-controlled

### Why this matters
An attacker can craft a `locationId` parameter to inject arbitrary SQL, potentially reading data from other tenants' schedule_shifts (the query already joins to `tenant.locations`). In a multi-tenant system, this could expose competitor staffing data. The route has proper auth and tenant checks for the parameterized values but the interpolated filter bypasses those protections entirely. The single-quote escaping gives a false sense of security — the route looks safe but is not.

### Proposed detector rule
```json
{
  "id": "security_theater.queryrawunsafe_with_string_interpolation",
  "title": "$queryRawUnsafe called with template literal string interpolation instead of parameterized placeholders",
  ...
  "negative_patterns": ["\\$queryRaw\\s*\\(", "Prisma\\.sql`"],
  "evidence_required": {
    "file_must_not_match": ["\\.test\\.|\\.spec\\.|tests/", "node_modules/", "__mocks__/"],
    "min_pattern_count": 1,
    "companion_pattern": "(searchParams|request\\.(body|query|nextUrl)|req\\.(body|query|params))"
  },
  "false_positive_controls": {
    "skip_test_files": true,
    "skip_doc_files": true,
    "strip_comments": true,
    "path_exclude": ["node_modules/", "\\.next/", "dist/", "coverage/"]
  },
  "user_impact": "SQL injection vulnerability in production API routes allows attackers to read/modify data across tenant boundaries. The manual single-quote escaping gives a false sense of security while leaving the endpoint exploitable.",
  "repair_guidance": "Convert all `$queryRawUnsafe` calls to `$queryRaw` with tagged template literals, passing `locationId` as a parameterized `$N` placeholder instead of string interpolation. Alternatively, use Prisma ORM query builders that handle parameterization automatically.",
  "example_source": {
    "file": "apps/api/app/api/staffing/coverage/route.ts",
    "line_or_snippet": "const locationFilter = locationId ? `AND ss.location_id = '${locationId.replace(/'/g, \"''\")}'` : \"\";"
  }
}
```

### Implementation note
Detector flags `$queryRawUnsafe` calls where the query string contains `${` template interpolation (indicating variables are being injected into SQL rather than passed as parameterized `$N` placeholders). The `companion_pattern` ensures the file handles HTTP requests (confirming user-controlled input is reachable), reducing false positives from utility files that might construct queries from internal constants. The `negative_patterns` check for `$queryRaw` (the safe tagged-template alternative) and `Prisma.sql` — if the file also uses the safe API elsewhere, it may have been partially migrated. Prisma docs on `$queryRawUnsafe`: https://www.prisma.io/docs/orm/prisma-client/queries/raw-database-access#raw-queries-are-not-escaped. The `strip_comments` control ensures that commented-out `$queryRawUnsafe` calls (from before migration to safe APIs) don't trigger false positives — this pattern was observed in procurement/budget routes that were converted from `$queryRawUnsafe` to Prisma ORM.
---
---
## [2026-05-08 17:15] Rule Discovery — security_theater.writeoff_approvedby_client_supplied

### Finding
The collections case PATCH endpoint accepts an `approvedBy` UUID directly from the client request body when performing a `writeOff` action. The route only validates tenant membership via `requireTenantId()` — it never retrieves the calling user's ID, never checks their role or permissions, and never verifies that the `approvedBy` UUID corresponds to a real user with write-off authority. Any authenticated tenant user can write off any collection case amount and claim anyone approved it.

### Evidence
- File: `apps/api/app/api/accounting/collections/cases/[id]/route.ts`
- Line 39: `approvedBy: z.string().uuid()` — accepted from request body without verification
- Line 112: `const tenantId = await requireTenantId()` — no user ID or role extracted
- Line 287-296: `approvedBy` value written directly into case notes as the approver identity
- Line 245-267: `escalateToLegal` action also has no authorization check

### Why this matters
A write-off is a high-privilege financial action that writes off outstanding debt. By accepting the approver identity from the client, the system creates a false audit trail — the `approvedBy` UUID in the notes appears to be an authorization record but is actually whatever the caller provided. Combined with no role/permission check, any regular user in the tenant can perform write-offs and forge approval records. This is both a security vulnerability and an audit integrity issue.

### Proposed detector rule
```json
{
  "id": "security_theater.writeoff_approvedby_client_supplied",
  "title": "Financial write-off accepts client-supplied approver without server-side authorization",
  "category": "security_theater",
  "severity": "critical",
  "confidence": 0.85,
  "detector_type": "hybrid",
  "language_targets": ["typescript", "javascript"],
  "pattern": "approvedBy\\s*[:=]\\s*.*\\b(?:z\\.string|body|request|req)\\b",
  "negative_patterns": ["auth\\(\\)", "requireAuth", "requireRole", "requirePermission", "hasPermission", "isAdmin", "can\\(.*writeOff"],
  "evidence_required": {
    "file_must_match": ["api/.*collections/|api/.*write.?off/|api/.*accounting/"],
    "companion_pattern": "z\\.(?:string|uuid|object)"
  },
  "false_positive_controls": {
    "skip_test_files": true,
    "skip_doc_files": true,
    "strip_comments": true,
    "path_exclude": ["node_modules/", ".next/", "dist/", "__tests__/"]
  },
  "user_impact": "Any authenticated user can write off debt and forge approval records, destroying financial audit trail integrity. Users see write-off approvals in notes and trust them as legitimate authorization events.",
  "repair_guidance": "Replace client-supplied approvedBy with server-side extraction of the calling user's identity from the auth session. Verify the user has a role with write-off authority (e.g., controller, CFO, admin) before proceeding. The approvedBy field should be populated from auth().userId, never from request.body.",
  "example_source": {
    "file": "apps/api/app/api/accounting/collections/cases/[id]/route.ts",
    "line_or_snippet": "const writeOffSchema = z.object({\n  amount: z.number().positive(),\n  reason: z.string().min(1),\n  approvedBy: z.string().uuid(),\n});"
  }
}
```

### Implementation note
Detector flags route handlers in financial/accounting/collections paths where an `approvedBy` field is accepted from request input (zod schema parsing body/request) without evidence of server-side authorization (auth(), requireRole, permission checks). The `negative_patterns` check for common auth/permission primitives — if present, the file likely performs real authorization. The `companion_pattern` for zod schema confirms this is a validated input field, not just a variable name coincidence. This pattern generalizes beyond write-offs to any financial approval flow that trusts client-supplied approver identity.
---
