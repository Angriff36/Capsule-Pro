...
---
---
## [2026-05-07 10:12] Rule Discovery — dashboard_illusion.simulated_gps_tracking_via_math_random

### Finding
The logistics tracking endpoint (`GET /api/logistics/tracking`) presents itself as "Real-time tracking data for active deliveries" but fabricates GPS positions using `Math.random()`, hardcoded LA coordinates, and random speed/heading values. The front-end renders these on a custom SVG map and auto-refreshes every 30 seconds, making vehicles appear to teleport randomly — a convincing-looking but entirely fake real-time tracking experience.
...
const position = {
  lat: shipment.status === "delivered" ? baseLat + 0.05
    : shipment.status === "in_transit" ? baseLat + jitter : baseLat - 0.02,
  lng: shipment.status === "delivered" ? baseLng + 0.03
...
```
- Front-end consumer: `apps/app/app/(authenticated)/logistics/tracking/page.tsx` — renders `position.lat`, `position.lng` on a map, displays `position.speed.toFixed(0) mph` and `position.heading` degrees, auto-refreshes every 30 seconds via `setInterval(loadTrackingData, 30_000)`.
- UI label: "Real-time tracking for active deliveries" (line 337 of page.tsx)

### Why this matters
Users see a map with moving vehicle dots, speed readouts, and heading indicators that change every 30 seconds. This creates a false impression of GPS tracking capability. The data is meaningless — the same vehicle can appear in downtown LA, then a mile away, then back — because it is pure random jitter around hardcoded coordinates. A catering operations team relying on this to coordinate delivery timing or communicate ETAs to clients would be making decisions on fabricated data.

### Proposed detector rule
```json
{
  "id": "dashboard_illusion.simulated_gps_tracking_via_math_random",
  "title": "Simulated GPS Tracking Using Math.random() Instead of Real Telemetry",
  "category": "dashboard_illusion",
  "severity": "high",
  "confidence": 0.95,
  "detector_type": "hybrid",
  ...
  "patterns": [
    ...
    "Hardcoded lat/lng base coordinates near production data",
    "Comment pattern: 'Simulated position' or 'In production, this would come from GPS'",
    "Position object with lat, lng, heading, speed fields sourced from random values"
  ],
  "negative_patterns": [
    "Math.random() in test files",
    "Random values used for non-GPS/non-telemetry purposes (e.g., ID generation, jitter in animations)",
    "Explicitly labeled demo/prototype/mock components"
  ],
  "evidence_required": [
    "API route returning position data containing Math.random()-derived lat/lng",
    "Front-end component consuming and rendering the position data as if real",
    "No actual GPS/telemetry SDK integration (no imports from tracking SDKs, no WebSocket/real-time data source)"
  ],
  "false_positive_controls": [
    "Exclude test files and mock directories",
    "Exclude legitimate demo/prototype prefixes or directories",
    "Require both API and consumer evidence for high-confidence flag"
  ],
  "user_impact": "Users see convincing but entirely fabricated real-time delivery tracking. Vehicle positions, speed, and heading change randomly on each refresh, making the feature unreliable for operational decisions, client ETAs, or logistics coordination.",
  "repair_guidance": "Integrate a real GPS tracking SDK or telematics provider (e.g., Samsara, Geotab, FourKites). Replace the Math.random()-based position generation with actual device-reported coordinates. If GPS integration is not yet available, clearly label the feature as 'demo' or 'simulated' in the UI and remove the 'Real-time tracking' claim until real data flows.",
  "example_source": {
    "file": "apps/api/app/api/logistics/tracking/route.ts",
    "line_or_snippet": "const jitter = (Math.random() - 0.5) * 0.01;\nconst position = {\n  lat: shipment.status === \"in_transit\" ? baseLat + jitter : baseLat - 0.02,\n  heading: shipment.status === \"in_transit\" ? Math.floor(Math.random() * 360) : 0,\n  speed: shipment.status === \"in_transit\" ? 20 + Math.floor(Math.random() * 20) : 0,\n};"
  }
}
```

### Implementation note
Detector should use a hybrid approach: (1) regex scan for `Math.random()` in proximity to lat/lng/heading/speed/position variable assignments, (2) cross-file check that the API endpoint is consumed by a UI component that presents the data as real-time tracking, (3) confirm absence of GPS SDK imports. The comment "In production, this would come from" is a strong signal for this anti-pattern.
---
## [2026-05-07 14:30] Rule Discovery — feature_claim_mismatch.payroll_tax_write_read_asymmetry

### Finding
The payroll engine correctly calculates taxes (federal, state, FICA) during payroll generation via `calculateTaxes()` in `calculator.ts`, and the in-memory `PayrollRecord` model includes full tax fields. However, the Prisma persistence layer (`savePayrollRecords`) silently drops all tax data because the `payroll_line_items` database table lacks columns for taxes. On read-back (`getPayrollRecords`), the data source returns hardcoded zeros: `taxesWithheld: []`, `totalTaxes: 0`, `tips: 0`. This means every subsequent operation — payroll reports, UI display, and QuickBooks exports — receives zeroed tax data despite taxes being correctly computed at generation time.

### Evidence
- File: `packages/payroll-engine/src/dataSource/PrismaPayrollDataSource.ts`
- **Write path (lines 243-283)**: `savePayrollRecords()` persists `hoursRegular`, `hoursOvertime`, `rateRegular`, `rateOvertime`, `grossPay`, `deductions` (JSON), `netPay` — but never writes `taxesWithheld`, `totalTaxes`, `tips`, `taxableIncome`, `department`, or `roleName`. These fields are silently discarded.
- **Read path (lines 375-401)**: `getPayrollRecords()` returns hardcoded stubs:
  ```typescript
  ...
  department: undefined, // BLOCKER: Department model not yet linked to employees.
  roleName: "Default", // BLOCKER: Role join not yet implemented.
  ```
- File: `packages/database/prisma/schema.prisma` (lines 3598-3617): `payroll_line_items` model has no columns for taxes, tips, department, or role.
- File: `packages/payroll-engine/src/core/taxEngine.ts`: A fully implemented 290-line tax calculation engine exists with federal brackets, FICA, Medicare, and state tax support.
- File: `packages/payroll-engine/src/core/calculator.ts` (line 300): `calculatePayroll()` correctly calls `calculateTaxes()` and produces records with populated tax fields.
- File: `packages/payroll-engine/src/services/payrollService.ts` (line 215): `getReport()` calls `getPayrollRecords()` for exports, meaning QuickBooks exports always receive zero tax data.
- File: `apps/app/app/(authenticated)/payroll/runs/[runId]/components/payroll-line-items-table.tsx` (lines 83-105): Frontend has elaborate tax display logic (`getTaxAmount()`, `getTotalTaxes()`) that always renders $0.00.

### Why this matters
This is a **financial data integrity issue** with cascading impact:
1. **Payroll reports show $0.00 in taxes** — managers reviewing payroll see employees with zero tax withholding, which is legally suspicious and operationally misleading.
2. **QuickBooks exports contain zero tax liabilities** — exported journal entries debit payroll expense accounts but credit zero to tax liability accounts, producing incorrect financial statements.
3. **The BLOCKER comments are misleading** — they say "Tax calculation engine not yet implemented" but the engine IS implemented (`taxEngine.ts`, 290 lines). The real problem is that the schema lacks storage columns and the read path doesn't call the engine.
4. **Net pay may appear correct** (it's stored directly), but the tax breakdown is fabrication — the relationship between gross, taxes, deductions, and net is unverifiable from stored data.

### Proposed detector rule
```json
{
  "id": "feature_claim_mismatch.payroll_tax_write_read_asymmetry",
  "title": "Payroll Tax Data Silently Dropped on Persistence (Write-Read Asymmetry)",
  "category": "feature_claim_mismatch",
  "severity": "critical",
  "confidence": 0.95,
  "detector_type": "cross_file",
  "language_targets": ["typescript", "sql", "prisma"],
  "patterns": [
    "save* method writing a subset of model fields to database",
    "get* method returning hardcoded zero/empty values for fields that exist on the model type",
    "BLOCKER comments in read path claiming feature 'not yet implemented' while a working implementation exists elsewhere in the package",
    "Prisma model lacking columns that the application-level TypeScript interface declares",
    "record.field: 0 or record.field: [] or record.field: undefined in a mapper function that should read from DB"
  ],
  "negative_patterns": [
    "Fields intentionally excluded with documented rationale (e.g., computed fields that are always recalculated)",
    "Test fixtures or seed data with hardcoded values",
    "Optional fields that are legitimately null/undefined when not applicable"
  ],
  "evidence_required": [
    "TypeScript interface/type declaring fields (e.g., taxesWithheld, totalTaxes, tips)",
    "Write/save method that omits those fields when persisting to DB",
    "Read/get method returning hardcoded zero/empty for those fields",
    "Database schema (Prisma/SQL) confirming columns are missing",
    "A working implementation of the missing logic elsewhere in the codebase"
  ],
  "false_positive_controls": [
    "Only flag when the same package contains both the write and read paths (avoid cross-package false positives)",
    "Require that the TypeScript model interface includes the zeroed fields (not just arbitrary object properties)",
    "Check for BLOCKER/TODO comments in the read path as a strong confirming signal",
    "Verify a calculation/engine function exists in the same package that SHOULD populate those fields"
  ],
  "user_impact": "Payroll reports display $0.00 in tax withholdings for all employees. QuickBooks exports contain zero tax liability entries, producing incorrect financial statements. Managers cannot verify payroll accuracy. The system appears to handle taxes but actually discards all tax computation results on every save.",
  "repair_guidance": "Add tax-related columns to the payroll_line_items Prisma schema (taxes_withheld JSONB, total_taxes Decimal, tips Decimal, taxable_income Decimal, department String, role_name String). Update savePayrollRecords() to persist these fields. Update getPayrollRecords() to read them from the database instead of returning hardcoded zeros. Alternatively, recalculate taxes on read-back by calling calculateTaxes() with the stored gross pay and employee tax info, though persistence is preferred for auditability.",
  "example_source": {
    "file": "packages/payroll-engine/src/dataSource/PrismaPayrollDataSource.ts",
    "line_or_snippet": "tips: 0, // BLOCKER: TipPool model does not exist in schema.\ntaxesWithheld: [], // BLOCKER: Tax calculation engine not yet implemented.\ntotalTaxes: 0, // BLOCKER: Tax calculation engine not yet implemented."
  }
}
```

### Implementation note
Detector should use cross-file analysis: (1) identify TypeScript interfaces with financial/tax fields, (2) find the corresponding Prisma/DB write operations and check if those fields are persisted, (3) find the corresponding read operations and check if they return hardcoded zeros, (4) confirm a working calculation implementation exists in the same package. The key signal is the BLOCKER comment claiming something "not yet implemented" when it IS implemented — the disconnect is between the engine and the persistence layer, not the engine itself. This pattern is generalizable beyond payroll to any domain where computation exists but results aren't persisted.
---

## [2026-05-07 15:45] Rule Discovery — test_theater.tautological_assertions_in_test_file

### Finding
The settings workflow test file (`apps/app/__tests__/settings/settings-workflow.test.ts`) contains 39 instances of `expect(true).toBe(true)` across 50 total `expect()` calls (78% tautological). The file imports only `describe, expect, it` from vitest — zero application code is imported, rendered, or executed. The tests claim to verify that "Settings pages load without crashes" (SET1), "Low-risk preference changes" (SET2), "Persistence verification" (SET3), "Invalid input validation" (SET4), and "Admin-only settings protection" (SET5), but they perform no actual assertions against the application. Each test body is a code comment describing what the source code does, followed by `expect(true).toBe(true)`.

### Evidence
- File: `apps/app/__tests__/settings/settings-workflow.test.ts`
- Snippet (representative, lines 56-67):
```typescript
...
```
- Only import in file: `import { describe, expect, it } from "vitest";` (line 5)
- 39 of 50 `expect()` calls are `expect(true).toBe(true)`
- Additional scope: `apps/api/__tests__/kitchen/manifest-code-generation.test.ts` (2 instances, informational/documentation tests) and `apps/app/__tests__/api/command-board/agent-loop-timeout.test.ts` (2 instances, tests constants it re-defines locally)

### Why this matters
These tests create a false sense of coverage. A CI pipeline running these tests reports green, but zero application behavior is actually verified. If a regression is introduced in the settings pages (auth bypass, crash on load, broken data flow), these tests will still pass. The test descriptions claim to cover security, integrations, audit logs, persistence, and admin protection — all high-risk areas — but none are tested. This is the most concentrated instance of test theater in the repo: one file with 39 tautological assertions across 5 claimed test categories.

### Proposed detector rule
```json
{
  "id": "test_theater.tautological_assertions_in_test_file",
  "title": "Test file with high ratio of tautological assertions (expect(true).toBe(true))",
  ...
  "negative_patterns": [
    "tests that import and call actual application functions",
    "tests with dynamic boolean expressions like expect(someFunction()).toBe(true)",
    "test frameworks' own fixture setup code"
  ],
  "evidence_required": [
    "Test file contains more than 3 instances of expect(true).toBe(true)",
    "Test file imports only from test framework (vitest, jest, mocha) — no application imports",
    "Test descriptions claim to verify specific application behavior (pages load, auth works, etc.)"
  ],
  "false_positive_controls": [
    "Skip if the test file imports application modules or functions under test",
    "Skip if expect(true).toBe(true) is inside a .skip or .todo block",
    "Skip if the file has fewer than 3 tautological assertions (could be legitimate placeholder for future work)",
    "Allow informational/documentation tests if clearly labeled as such in describe block"
  ],
  "user_impact": "Tests that always pass give false confidence in CI. High-risk areas like security, auth, and admin protection appear tested but have zero actual coverage, meaning regressions can ship undetected.",
  "repair_guidance": "Replace tautological assertions with real tests that import the application code under test. For server components, use render testing or API route integration tests. For the settings module specifically: test that auth-gated pages return 401/404 without auth, test that API routes validate input, test that preferences persist to DB. At minimum, remove the false tests so the coverage gap is visible.",
  "example_source": {
    "file": "apps/app/__tests__/settings/settings-workflow.test.ts",
    "line_or_snippet": "describe(\"Security page\", () => {\n    it(\"renders static ModuleSection placeholder\", () => {\n      // No auth, no DB, no dynamic data\n      expect(true).toBe(true);\n    });\n  });"
  }
}
```

### Implementation note
Detector should use regex to count `expect(true).toBe(true)` occurrences per test file, then use AST or import analysis to check whether the file imports any application code (anything outside the test framework). Flag files with >3 tautological assertions and zero application imports as high-confidence test theater. The ratio metric (tautological / total expect calls) helps distinguish between occasional placeholders (acceptable) and systematic test theater (this rule's target). Could extend to also catch tests that define their own constants and then assert against them (as in agent-loop-timeout.test.ts), though that's a softer signal.

---
## [2026-05-07 22:14] Rule Discovery — feature_claim_mismatch.ai_staffing_is_just_arithmetic

### Finding
The "AI Staffing Recommendations" feature is branded as AI-powered across three layers — page metadata, layout navigation, and the client UI (Sparkles icon, "AI-powered" description) — but the backend API endpoint at `/api/staffing/recommendations` contains zero AI/LLM calls. The entire "recommendation engine" is a deterministic arithmetic formula: `Math.ceil(guestCount / 18) * serviceMultiplier`. There is no import of any AI SDK, no call to OpenAI/Anthropic, no prompt, no model invocation — just hardcoded ratios (captain 10%, server 45%, bartender 15%, culinary 30%) and fixed hourly rates ($32/$24/$26/$22). The test only verifies the response shape matches the UI contract, never questioning whether AI is involved.
...
- Snippet:
  ...
  ```
  Lines 107-108: The entire "AI" logic is `guestCount / 18` rounded up.
  Lines 40-69: `buildRoles()` — hardcoded role percentages, fixed hourly rates, static note strings.
  Lines 72-82: `buildNotes()` — template literal notes, no AI generation.
  No imports of any AI/LLM library anywhere in the file.
- File: `apps/app/app/(authenticated)/staffing/recommendations/page.tsx`
  Lines 8-9: `title: "AI Staffing Recommendations"` / `description: "AI-powered staffing recommendations for events"`
- File: `apps/app/app/(authenticated)/staffing/recommendations/staffing-recommendations-client.tsx`
  Lines 79-82: Page heading "AI Staffing Recommendations" with Sparkles icon and "Get AI-powered staffing recommendations" description.
- File: `apps/app/app/(authenticated)/staffing/layout.tsx`
  Line 21: Nav item label "AI Recommendations"
  Line 35: "Manage shifts, availability, and AI-powered coverage recommendations."
- File: `apps/api/app/api/staffing/recommendations/route.test.ts`
  Lines 6-36: Test only checks response shape and status — no assertion about AI involvement, no verification of intelligence/adaptivity.

### Why this matters
This is a textbook feature claim mismatch with direct user-facing impact. Catering companies are told they're getting "AI-powered" staffing recommendations, which implies the system learns from their historical event data, considers venue constraints, seasonal patterns, staff availability, and produces tailored guidance. In reality, every event gets the same formula: 1 staff per 18 guests, adjusted by a static multiplier for service style. A 200-person wedding and a 200-person corporate gala with identical service styles get identical recommendations regardless of venue, menu complexity, past overstaffing costs, or any other context. The "AI" branding is deceptive — it's a simple ratio calculator dressed up with Sparkles icons. For a B2B product where staffing costs are a major P&L line item, this misrepresentation erodes trust when customers realize the "AI" is just division.

### Proposed detector rule
```json
{
  "id": "feature_claim_mismatch.ai_staffing_is_just_arithmetic",
  "title": "Feature branded as AI-powered has zero AI/LLM implementation",
  "category": "feature_claim_mismatch",
  "severity": "high",
  "confidence": 0.95,
  "detector_type": "cross_file",
  "language_targets": ["typescript", "tsx", "javascript", "jsx", "any"],
  "patterns": [
    "AI [Ss]taffing|AI-powered|ai-powered staffing",
    "Math\\.ceil\\(|Math\\.floor\\(|Math\\.round\\(",
    "hardcoded hourly rates|fixed hourly rate|hourlyRate:\\s*\\d+",
    "serviceMultiplier|SERVICE_STYLE_MULTIPLIERS",
    "guestCount / \\d+"
  ],
  "negative_patterns": [
    "import.*openai|import.*anthropic|import.*@ai-sdk|from.*ai",
    "generateText|streamText|chat\\.completions|invoke.*model",
    "prompt|systemMessage|userMessage",
    "*.test.ts", "*.spec.ts"
  ],
  "evidence_required": [
    "UI/marketing copy contains 'AI-powered' or 'AI' branding for the feature",
    "Backend API route for the feature exists and is reachable from the UI",
    "API route contains no AI/LLM SDK imports or model invocations",
    "API logic is deterministic arithmetic (Math.ceil/floor/round) or static lookup tables",
    "Feature name or page title includes 'AI' but implementation has no AI dependency"
  ],
  "false_positive_controls": [
    "Exclude files that import any AI SDK (openai, anthropic, @ai-sdk, etc.)",
    "Exclude files that call generateText, streamText, chat.completions, or similar LLM APIs",
    "Only flag when the 'AI' claim appears in UI/marketing layer AND the backend handler is identifiable",
    "Allow hybrid cases where AI augments arithmetic (e.g., AI generates notes but staffing is formulaic) — only flag when there is literally zero AI"
  ],
  "user_impact": "Users pay for or trust 'AI-powered' features that are actually simple calculators. This is deceptive marketing that erodes product credibility, especially in B2B contexts where staffing accuracy directly impacts P&L. Customers making business decisions based on 'AI recommendations' are actually using a fixed ratio formula with no learning or personalization.",
  "repair_guidance": "Either (a) integrate a real LLM call that considers historical event data, venue constraints, menu complexity, seasonal patterns, and past staffing outcomes to generate genuinely adaptive recommendations, or (b) honestly rebrand the feature as 'Staffing Calculator' or 'Staffing Estimator' without the AI/Sparkles branding. If AI is planned but not yet implemented, the UI should say 'Coming soon' rather than pretending it's already AI-powered.",
  "example_source": {
    "file": "apps/api/app/api/staffing/recommendations/route.ts",
...
### Implementation note
Detector should cross-reference UI/marketing text containing "AI" branding against the corresponding backend API handler. For each UI element claiming AI power, locate the API endpoint it calls (via apiFetch paths), then scan that endpoint's source for any LLM/AI SDK import or invocation. If the backend handler contains only deterministic logic (Math functions, hardcoded constants, template literals) and zero AI library usage, flag as feature_claim_mismatch. The detector should be parametric — the specific pattern here is "AI staffing" but the general pattern is "feature labeled AI has no AI in its backend." Could extend to scan for Sparkles icon usage paired with non-AI backends as a heuristic signal.

---
## [2026-05-07 15:25] Rule Discovery — automation_theater.cron_route_never_scheduled

### Finding
Two fully implemented cron routes (`/api/cron/email-reminders` and `/api/cron/idempotency-cleanup`) contain real production logic — database queries, auth guards, Sentry error reporting — but are never scheduled in `apps/api/vercel.json`. They will never execute on Vercel. The email-reminders route in particular processes task and shift reminder emails for catering staff; without being scheduled, no one ever receives automated reminders.

### Evidence
- File: `apps/api/vercel.json` — contains only 4 cron entries (webhook-retry, inventory-audit, sentry-fixer, contract-expiration-alerts). Missing: `email-reminders` and `idempotency-cleanup`.
- File: `apps/api/app/api/cron/email-reminders/route.ts` — 388-line fully implemented cron with `processTaskReminders()` and `processShiftReminders()`, querying `KitchenTask`, `KitchenTaskClaim`, `ScheduleShift`, `EmailWorkflow` tables, calling `triggerEmailWorkflows()`.
- File: `apps/api/app/api/cron/idempotency-cleanup/route.ts` — 82-line cleanup route deleting expired `manifestIdempotency` rows.
- Snippet from `vercel.json` (the crons array):
```json
"crons": [
    ...
    { "path": "/api/cron/contract-expiration-alerts", "schedule": "0 7 * * *" }
]
```
Note: no entry for `/api/cron/email-reminders` or `/api/cron/idempotency-cleanup`.

### Why this matters
The email-reminders route is user-facing: staff relying on task and shift reminders will never receive them. The code comments say "This should be called periodically (e.g., every 15 minutes)" but Vercel's cron scheduler has no knowledge of this endpoint. The idempotency-cleanup is infra-facing but still matters — without it, the `manifestIdempotency` table grows unbounded over time. Both routes create a false impression that the system is actively maintaining itself.

### Proposed detector rule
```json
{
  "id": "automation_theater.cron_route_never_scheduled",
  "title": "Cron route exists in codebase but is never scheduled in deployment config",
  "category": "automation_theater",
  "severity": "high",
  "confidence": 0.95,
  "detector_type": "cross_file",
  ...
  "patterns": [
    "route.ts files under cron/ or api/cron/ directories",
    "vercel.json crons array entries",
    "netlify.toml [[scheduled_functions]] entries",
    "GitHub Actions .github/workflows/*.yml schedule triggers"
  ],
  "negative_patterns": [
    "Cron routes that are intentionally on-demand (documented as manual-trigger only)",
    "Routes under test fixtures or __tests__ directories",
    "Routes that are called by other cron routes (transitive scheduling)"
  ],
  "evidence_required": [
    "At least one route.ts file exists under a cron path",
    "The deployment config file (vercel.json, netlify.toml, etc.) does not list that route's path in its schedule configuration",
    "The route file contains actual business logic (not just a stub)"
  ],
  "false_positive_controls": [
    "Verify the route is not imported/called by another scheduled route",
    "Check if external scheduler (GitHub Actions, AWS EventBridge) references the endpoint URL",
    "Exclude routes annotated with @manual or similar decorator indicating intentional on-demand use"
  ],
  "user_impact": "Automated processes that users depend on (email reminders, data cleanup, alerts) silently never execute. Staff miss shift/task reminders. Data tables grow unbounded. The system appears to be maintaining itself but isn't.",
  "repair_guidance": "Add the missing cron entries to vercel.json (or equivalent deployment config) with appropriate schedules. For email-reminders, a schedule like '*/15 * * * *' (every 15 min) matches the comment. For idempotency-cleanup, '0 3 * * *' (daily at 3am) is reasonable. Alternatively, if these routes are not intended for production scheduling yet, add a prominent TODO comment and remove the production-grade auth/logging code that implies they are live.",
  "example_source": {
    "file": "apps/api/app/api/cron/email-reminders/route.ts",
    "line_or_snippet": "POST /api/cron/email-reminders — 388 lines of production logic querying KitchenTask, ScheduleShift, EmailWorkflow tables and calling triggerEmailWorkflows(), but no matching entry in vercel.json crons array."
  }
}
```

### Implementation note
Detector should: (1) discover all route handler files under cron/ directories, (2) extract their HTTP path from the file location (Next.js convention: `app/api/cron/X/route.ts` → `/api/cron/X`), (3) parse the deployment config's schedule entries, (4) flag any cron route path not present in the schedule. For Vercel, parse `vercel.json` `crons[].path`. For Netlify, parse `netlify.toml` `[[scheduled_functions]]`. For GitHub Actions, search workflow files for `schedule:` triggers that `curl` the endpoint. Cross-reference against imports to avoid false positives from transitively-scheduled routes.
