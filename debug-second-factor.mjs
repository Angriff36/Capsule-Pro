...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    console.log(
      "First factors:",
      step1.supportedFirstFactors?.map((f) => ({
        s: f.strategy,
        id: f.emailAddressId,
      }))
    );
    console.log(
      "Second factors:",
      step1.supportedSecondFactors?.map((f) => ({
        s: f.strategy,
        id: f.emailAddressId,
      }))
    );
    ...
    console.log(
      ...
      si.firstFactorVerification?.status
    );
    console.log(
      "signIn.secondFactorVerification:",
      si.secondFactorVerification?.status
    );

    // Try password as second factor
    const pwdFactor = step1.supportedSecondFactors?.find(
