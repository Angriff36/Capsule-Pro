...
async function main() {
  ...
  try {
    ...
    const infos = consoleMessages.filter(
      (m) => m.type === "log" || m.type === "info"
    );

    // Count unique error messages (dedupe repeats)
    const uniqueErrors = [
      ...new Set(errors.map((e) => e.text.substring(0, 80))),
    ];
    // Count unique warning messages
    const uniqueWarnings = [
      ...new Set(warnings.map((w) => w.text.substring(0, 80))),
    ];
    ...
    const clerkMsgs = consoleMessages.filter(
      (m) =>
        m.text.includes("Clerk") ||
        m.text.includes("clerk") ||
        m.text.includes("auth")
    );
    const uniqueClerk = [
      ...new Set(clerkMsgs.map((m) => m.text.substring(0, 80))),
    ];
    console.log(
      "  Clerk auth messages:",
      clerkMsgs.length,
    ...
    const hasRepeatedSpam =
      uniqueErrors.length < errors.length * 0.3 && errors.length > 20;
    ...
    console.log(
      "  Massive console spam (>50 errors):",
      hasMassiveSpam ? "⚠ YES" : "✓ NO"
    );
    console.log(
      "  Repeated identical errors (>80% dupes):",
      hasRepeatedSpam ? "⚠ YES" : "✓ NO"
    );
    ...
    console.log(
      "L1 — No massive console spam:  " + (hasMassiveSpam ? "✗ FAIL" : "✓ PASS")
    );
    console.log(
      "L1 — No repeated subscriptions: " +
        (hasRepeatedSpam ? "⚠ SUSPECT" : "✓ PASS")
    );
    console.log(
      "L1 — Error rate reasonable:     " +
        (errorRate < 5 ? "✓ PASS" : "⚠ HIGH (" + errorRate.toFixed(1) + ")")
    );
    console.log(
      "L1 — API calls proportional:    " +
        (apiCallsPerNav < 30 ? "✓ PASS" : "⚠ HIGH (" + apiCallsPerNav + ")")
    );
    console.log(
      "L1 — Overall:                   " +
        (memoryLeak ? "⚠ MEMORY LEAK SUSPECTED" : "✓ PASS")
    );
    console.log("=".repeat(60));
  } finally {
    await browser.close();
