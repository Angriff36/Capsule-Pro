...
async function main() {
  ...
  try {
    ...
    const now = Date.now();
    const tomorrow9 = new Date(Date.now() + 9 * 3600000).toISOString();
    const tomorrow17 = new Date(Date.now() + 17 * 3600000).toISOString();
    ...
    const sch1Status = createShift.status === 200 ? "✓ PASS" : createShift.status === 404 ? "✗ NOT FOUND" : createShift.status === 422 ? "⚠ Guard (no tenant)" : "⚠ " + createShift.status;
    console.log(`    Status: ${createShift.status} | ${sch1Status}`);
    if (createShift.status !== 200) {
      console.log(`    Response: ${JSON.stringify(createShift.data)?.substring(0, 150)}`);
    } else {
    ...
    console.log(`    Status: ${listShifts.status} | Shifts count: ${shifts.length}`);
    ...
    const assignStaff = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
      startTime: tomorrow9,
      endTime: tomorrow17,
      role: "kitchen_staff",
    ...
    console.log(`    Status: ${assignStaff.status} | ${assignStaff.status === 200 ? "✓ PASS" : assignStaff.status === 422 ? "⚠ Guard" : "⚠ " + assignStaff.status}`);
    if (assignStaff.status === 200) {
      console.log(`    Staff assigned ✓`);
    }

    // Try bulk assignment
    const bulkAssign = await api(page, "POST", "/api/staff/shifts/bulk-assignment", {
      shiftId: "00000000-0000-0000-0000-000000000001",
      employeeIds: [USER_ID],
    });
    console.log(`\n  POST /api/staff/shifts/bulk-assignment:`);
    console.log(`    Status: ${bulkAssign.status} | ${bulkAssign.status === 200 ? "✓ PASS" : bulkAssign.status === 404 ? "✗ NOT FOUND" : "⚠ " + bulkAssign.status}`);
    ...
    // Create first shift
    const shift1 = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
      startTime: tomorrow9,
      endTime: tomorrow17,
      role: "sous-chef",
    ...
    const shift2 = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
    ...
    const isOverlapBlocked = shift2.status === 422 || shift2.status === 409 || shift2.status === 400;
    console.log(`  Overlap detected and blocked: ${isOverlapBlocked ? "✓ YES" : "⚠ NO (accepted overlap)"}`);
    if (shift2.data) {
      console.log(`  Validation message: ${JSON.stringify(shift2.data)?.substring(0, 200)}`);
    }

    // Non-overlapping shift (should succeed)
    const shift3 = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
      startTime: new Date(Date.now() + 25 * 3600000).toISOString(),
      endTime: new Date(Date.now() + 33 * 3600000).toISOString(),
      role: "kitchen_staff",
    });
    console.log(`  Shift 3 (non-overlapping next day): ${shift3.status} | ${shift3.status === 200 ? "✓ PASS" : shift3.status === 422 ? "⚠ Guard" : "⚠ " + shift3.status}`);
    ...
    const eventShift = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
      startTime: tomorrow9,
      endTime: tomorrow17,
      role: "head-chef",
    ...
    console.log(`    Status: ${eventShift.status} | ${eventShift.status === 200 ? "✓ PASS" : eventShift.status === 422 ? "⚠ Guard" : "⚠ " + eventShift.status}`);
    if (eventShift.data) {
      console.log(`    Response: ${JSON.stringify(eventShift.data)?.substring(0, 150)}`);
    }
    ...
    const shiftToDelete = await api(page, "POST", "/api/staff/shifts/commands/create", {
      employeeId: USER_ID,
      startTime: new Date(Date.now() + 48 * 3600000).toISOString(),
      endTime: new Date(Date.now() + 56 * 3600000).toISOString(),
      role: "kitchen_staff",
    });
    const shiftIdToDelete = shiftToDelete.data?.result || null;
    console.log(`  Created shift to delete: ${shiftToDelete.status} | ID: ${shiftIdToDelete || "unknown"}`);
    ...
    console.log(`    Status: ${removeShift.status} | ${removeShift.status === 200 ? "✓ PASS" : removeShift.status === 404 ? "✗ NOT FOUND" : "⚠ " + removeShift.status}`);
    if (removeShift.data) {
      console.log(`    Response: ${JSON.stringify(removeShift.data)?.substring(0, 150)}`);
    }

    // ══════════════════════════════════════════════════════════════
    // Summary
    // ══════════════════════════════════════════════════════════════
    const r = (s) => s === 200 ? "✓" : s === 404 ? "✗" : s === 422 ? "⚠" : s === 409 ? "⚠" : "?";

    console.log(`\n${"=".repeat(60)}`);
    console.log("📊 cp-089 — SCH1-5 Scheduling Module — RESULT");
    console.log("=".repeat(60));
    console.log("  SCH1 (Create shift):");
    console.log(`    create-shift:          ${r(createShift.status)} ${createShift.status}`);
    console.log("");
    console.log("  SCH2 (Assign staff):");
    console.log(`    assign-shift:         ${r(assignStaff.status)} ${assignStaff.status}`);
    console.log(`    bulk-assignment:       ${r(bulkAssign.status)} ${bulkAssign.status}`);
    console.log("");
    console.log("  SCH3 (Overlapping shift):");
    console.log(`    overlap-detection:     ${isOverlapBlocked ? "✓ BLOCKED" : "✗ ACCEPTED"}`);
    console.log(`    non-overlap-allowed:    ${shift3.status === 200 ? "✓ PASS" : "⚠ " + shift3.status}`);
    console.log("");
    console.log("  SCH4 (Connect to event):");
    console.log(`    shift-with-eventId:    ${r(eventShift.status)} ${eventShift.status}`);
    console.log("");
    console.log("  SCH5 (Delete shift):");
    console.log(`    remove-shift:          ${r(removeShift.status)} ${removeShift.status}`);
    console.log("");
    console.log("  Note: Jane has no orgId (orgs=[]) — tenant resolution fails for all");
