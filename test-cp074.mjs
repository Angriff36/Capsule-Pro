  ...
  try {
    ...
    for (const path of navPaths) {
      try {
        ...
        console.log("    Content:", bodyText.length > 10 ? bodyText.substring(0, 50) : "(blank/crashed)");
      } catch (e) {
        console.log("  " + path + ": ✗ Navigation error:", e.message.substring(0, 50));
      }
    }
    ...
    try {
      ...
      if (await searchInput.isVisible().catch(() => false)) {
        ...
        console.log("  Flow 2 (search interaction): ⚠ input not found, skipping");
      }
    } catch (e) {
    ...
    // Final verdict
    console.log("\n--- L3: Error Summary ---");
    console.log("  JS Exceptions:", jsExceptions.length === 0 ? "✓ NONE" : "⚠ " + jsExceptions.length);
    if (jsExceptions.length > 0) {
      jsExceptions.slice(0, 3).forEach(e => console.log("    -", e.substring(0, 80)));
    }
    console.log("  Unhandled rejections:", unhandledRejections.length === 0 ? "✓ NONE" : "⚠ " + unhandledRejections.length);
    if (unhandledRejections.length > 0) {
      unhandledRejections.slice(0, 3).forEach(e => console.log("    -", e.substring(0, 80)));
    }
    console.log("  Runtime overlays (5xx):", runtimeOverlays.length === 0 ? "✓ NONE" : "⚠ " + runtimeOverlays.length);
    if (runtimeOverlays.length > 0) {
      runtimeOverlays.slice(0, 3).forEach(e => console.log("    -", e.status, e.url.split("/").pop()));
    }
    ...
    console.log("L3 — No JS exceptions:       " + (jsExceptions.length === 0 ? "✓ PASS" : "✗ FAIL (" + jsExceptions.length + ")"));
    console.log("L3 — No unhandled rejections:  " + (unhandledRejections.length === 0 ? "✓ PASS" : "✗ FAIL (" + unhandledRejections.length + ")"));
    console.log("L3 — No runtime overlays:      " + (pass ? "✓ PASS" : "⚠ " + runtimeOverlays.length + " server errors (pages crash)"));
    console.log("=".repeat(60));
