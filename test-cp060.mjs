...
async function main() {
  ...
  try {
    ...
    console.log(`    ${stationR.status === 200 ? "✓ DEPLOYED" : stationR.status === 404 ? "✗ NOT DEPLOYED (404)" : "⚠ " + stationR.status}`);
    if (stationR.status !== 200 && stationR.status !== 404) {
      console.log(`    Response: ${JSON.stringify(stationR.data)?.substring(0, 200)}`);
    }

    // Missing taskId (guard)
    const stationGuardR = await apiFetch(page, "/api/station/assign-task", "POST", {
      id: "test-station-id",
      taskId: "",
      taskName: "",
    });
    ...
    if (stationGuardR.status === 422) {
      console.log(`    ✓ Guard correctly triggered: ${JSON.stringify(stationGuardR.data)?.substring(0, 100)}`);
    ...
    const staffR = await apiFetch(page, "/api/eventstaff/assign", "POST", {
      ...
      shiftStart: Date.now(),
      shiftEnd: Date.now() + 3600000,
    });
    console.log(`  POST /eventstaff/assign (valid):`);
    console.log(`    Status: ${staffR.status}`);
    console.log(`    ${staffR.status === 200 ? "✓ DEPLOYED" : staffR.status === 404 ? "✗ NOT DEPLOYED (404)" : "⚠ " + staffR.status}`);
    if (staffR.status !== 200 && staffR.status !== 404) {
      console.log(`    Response: ${JSON.stringify(staffR.data)?.substring(0, 200)}`);
    }
    ...
    if (staffGuardR.status === 422) {
      console.log(`    ✓ Guard correctly triggered: ${JSON.stringify(staffGuardR.data)?.substring(0, 100)}`);
    ...
    console.log(`    ${unassignR.status === 200 ? "✓ DEPLOYED" : unassignR.status === 404 ? "✗ NOT DEPLOYED (404)" : "⚠ " + unassignR.status}`);
    ...
    const rolePolicy = await page.evaluate(() => {
      ...
      return {
        ...
        roleDisplay: "Roles control which stations/staff a user can view and assign"
      };
    });
    ...
    console.log("    Station.assignTask:  " + (stationR.status !== 404 ? "✓ DEPLOYED" : "✗ NOT DEPLOYED") + " (guard: taskId required)");
    console.log("    EventStaff.assign:   " + (staffR.status !== 404 ? "✓ DEPLOYED" : "✗ NOT DEPLOYED") + " (guard: eventId, userId required)");
    console.log("    EventStaff.unassign: " + (unassignR.status !== 404 ? "✓ DEPLOYED" : "✗ NOT DEPLOYED"));
    console.log("");
    ...
    console.log("      Event: EventStaffAssigned → events.event-staff.assigned");
