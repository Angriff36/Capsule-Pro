...
async function main() {
  ...
  try {
    ...
    Object.keys(byStatus).forEach(s => console.log("  " + s + ":", byStatus[s].length, "tasks"));
    ...
    console.log("  Create task:", createR.status, createR.status === 200 ? "✓" : "✗", "id:", taskId);

    // Start it (pending → in_progress)
    const startR = await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, {
      status: "in_progress",
    });
    console.log("  Start task:", startR.status, startR.status === 200 ? "✓ PASS" : "✗ FAIL", JSON.stringify(startR.data)?.substring(0, 100));

    // Complete it (in_progress → done)
    const completeR = await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, {
      status: "done",
    });
    console.log("  Complete task:", completeR.status, completeR.status === 200 ? "✓ PASS" : "✗ FAIL");
    ...
    console.log("  Status:", invalid1R.status, invalid1R.status === 422 ? "✓ 422 returned" : "⚠ " + invalid1R.status);
    console.log("  Message:", JSON.stringify(invalid1R.data)?.substring(0, 150));
    ...
    console.log("  Status:", invalid2R.status, invalid2R.status === 422 ? "✓ 422 returned" : "⚠ " + invalid2R.status);
    console.log("  Message:", JSON.stringify(invalid2R.data)?.substring(0, 150));

    // === K1: Scenario 3 — Try to complete a PENDING task directly ===
    console.log("\n--- K1: Invalid Transition — PENDING → done (not allowed) ---");
    if (pendingTask) {
      const invalid3R = await api(page, "PATCH", "/api/kitchen/tasks/" + pendingTask.id, {
        status: "done",
      });
      console.log("  Status:", invalid3R.status, invalid3R.status === 422 ? "✓ 422 returned" : "⚠ " + invalid3R.status);
      console.log("  Message:", JSON.stringify(invalid3R.data)?.substring(0, 150));
    } else {
    ...
    const cancelR = await api(page, "POST", "/api/kitchen/tasks", { title: "QA K1 Cancel Test", priority: 5, complexity: 3 });
    const cancelId = cancelR.data?.result;
    if (cancelId) {
      await api(page, "PATCH", "/api/kitchen/tasks/" + cancelId, { status: "cancelled" });
      const invalid4R = await api(page, "PATCH", "/api/kitchen/tasks/" + cancelId, {
        status: "pending",
      });
      console.log("  Status:", invalid4R.status, invalid4R.status === 422 ? "✓ 422 returned" : "⚠ " + invalid4R.status);
      console.log("  Message:", JSON.stringify(invalid4R.data)?.substring(0, 150));
    }
    ...
    await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, { status: "in_progress" });
    await page.waitForTimeout(1000);
    const apiErrors = consoleErrors.filter(e => !e.includes("Warning") && !e.includes("warning"));
    console.log("  Console errors on 422:", apiErrors.length === 0 ? "✓ NONE (clean)" : "⚠ " + apiErrors.length + " errors: " + apiErrors.join("; "));

    // === K1: Verify UX message ===
    console.log("\n--- K1: UX Response Check ---");
    const uxR = await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, { status: "in_progress" });
    const hasMessage = uxR.data?.message || uxR.data?.error || uxR.data?.guardFailure;
    const hasSuggestion = uxR.data?.suggestion || uxR.data?.nextStep || uxR.data?.details;
    console.log("  Has error message:", hasMessage ? "✓ YES: " + JSON.stringify(hasMessage)?.substring(0, 80) : "✗ NO");
    console.log("  Has next step/suggestion:", hasSuggestion ? "✓ YES: " + JSON.stringify(hasSuggestion)?.substring(0, 80) : "⚠ NO (missing)");
    ...
    console.log("K1 — No console spam:          " + (noSpam ? "✓ PASS" : "⚠ FAIL"));
    console.log("K1 — Error message displayed:  " + (hasMessage ? "✓ PASS" : "✗ FAIL"));
    console.log("K1 — Next step shown:          " + (hasSuggestion ? "⚠ PRESENT" : "✗ MISSING"));
    console.log("=".repeat(60));
