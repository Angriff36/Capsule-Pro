...
## Event Details Tabs Follow-up TODOs (2026-02-11)

- Validate tab deep-link behavior in manual QA for shared links (`/events/[eventId]?tab=<value>`) across desktop and mobile navigation flows.
- Review empty states in `Copilot` and `Explore` tabs to ensure each state has a single primary CTA and de-emphasized secondary actions.
- Consider moving budget and prep task lane into a dedicated `Operations`-specific composition if product wants stricter separation from `Copilot` in a later iteration.
