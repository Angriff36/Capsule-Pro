...
async function main() {
    ...
    for (const tab of tabs) {
      ...
      if (result.pageErrors.length > 0) {
        result.pageErrors.forEach((e) =>
          console.log(`  ✗ PageError: ${e.substring(0, 100)}`)
        );
      }
      if (result.networkErrors.length > 0) {
        result.networkErrors.forEach((ne) =>
          console.log(`  ⚠ ${ne.status}: ${ne.url.substring(0, 80)}`)
        );
      }
    }
  } finally {
  ...
  const withErrors = results.filter(
    (r) => r.consoleErrors.length > 0 || r.pageErrors.length > 0
  ).length;
  ...
  console.log("");
  console.log(
    "  Root cause: Clerk orgs=[] → requireTenantId() fails → all server pages 500"
  );
  console.log("=".repeat(60));
