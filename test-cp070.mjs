...
async function api(page, method, path, body = null) {
  return await page.evaluate(async ({ m, p, b }) => {
    ...
    return { status: resp.status, data: json, headers: Object.fromEntries(resp.headers.entries()) };
...
async function main() {
  ...
  try {
    ...
    console.log("  Status:", alerts500.status, alerts500.status === 500 ? "✓ 500" : "⚠ " + alerts500.status);
    const responseText1 = JSON.stringify(alerts500.data)?.substring(0, 200);
    console.log("  Response:", responseText1);
    const leaked1 = responseText1.includes("password") || responseText1.includes("secret") || responseText1.includes("key") || responseText1.includes("token") || responseText1.includes("sk_");
    ...
    console.log("  Status:", invalidR.status, invalidR.status === 404 ? "404 (expected)" : "⚠ " + invalidR.status);

    // K2: Trigger 500 by sending malformed JSON or invalid body to a known endpoint
    console.log("\n--- K2: Malformed request body ---");
    const malformedR = await api(page, "POST", "/api/kitchen/tasks", { broken: "data" });
    console.log("  Status:", malformedR.status, malformedR.status >= 400 ? "✓ " + malformedR.status : "✗ unexpected");
    ...
    const pageErrors = consoleErrors.filter(e => !e.includes("Failed to load resource"));
    console.log("  Console errors on /events (expected 500):", pageErrors.length);
    if (pageErrors.length > 0) {
      console.log("  Error texts:", pageErrors.slice(0, 3).map(e => e.substring(0, 80)));
    }
    // Check if error boundary or generic error is shown
    const bodyText = await page.evaluate(() => document.body.innerText.substring(0, 200));
    console.log("  Page content:", bodyText.length > 0 ? bodyText.substring(0, 100) : "(empty/error boundary)");
    ...
    const secretPatterns = ["password", "secret", "sk_test", "pk_test", "token", "authorization", "Bearer", "neon", "clerk"];
    ...
    for (const data of allResponses) {
      ...
      for (const pattern of secretPatterns) {
        if (text.toLowerCase().includes(pattern)) {
          console.log("  ⚠ Possible leak:", pattern, "in:", text.substring(0, 100));
          leaked = true;
        }
      }
    }
    ...
    const stackHeaders = Object.keys(headers).filter(k => k.toLowerCase().includes("stack") || k.toLowerCase().includes("trace") || k.toLowerCase().includes("debug"));
    console.log("  Stack/trace headers:", stackHeaders.length === 0 ? "✓ NONE" : "⚠ " + stackHeaders.join(", "));

    // K2: Generic error UI check (via page screenshot)
    await page.goto(`${BASE}/events`, { waitUntil: "domcontentloaded", timeout: 15000 }).catch(() => {});
    await page.waitForTimeout(2000);
    const hasGenericError = await page.evaluate(() => {
      const text = document.body.innerText.toLowerCase();
      return text.includes("error") || text.includes("something went wrong") || text.includes("500") || text.includes("internal");
    });
    console.log("\n--- K2: Generic Error UI ---");
    console.log("  Error UI visible:", hasGenericError ? "✓ YES" : "✗ NO (page may be blank/error boundary)");

    console.log("\n" + "=".repeat(60));
    console.log("📊 cp-070 — K2: Force 500 Scenario — RESULT");
    console.log("=".repeat(60));
    console.log("K2 — 500 triggered:          " + (alerts500.status === 500 ? "✓ PASS" : "⚠ " + alerts500.status));
    console.log("K2 — Generic error UI:      " + (hasGenericError ? "✓ PASS" : "✗ FAIL (no error UI)"));
    console.log("K2 — No secrets leaked:      " + (!leaked ? "✓ PASS" : "✗ FAIL"));
    console.log("K2 — No stack trace headers: " + (stackHeaders.length === 0 ? "✓ PASS" : "⚠ FOUND"));
    console.log("=".repeat(60));
