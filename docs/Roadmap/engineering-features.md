...
| 2 | Read Replica Routing | **All analytics/reporting routes now read through `analyticsDatabase`** (`packages/database/analytics-database.ts`): 8 pure-read routes alias it; 2 mixed routes (events/advanced, events/profitability) keep the account-provisioning write on the primary and route only the OLAP reads to the replica. It transparently falls back to the primary when `ANALYTICS_DATABASE_URL` is unset. **To activate:** create a Neon **read replica** (a read-only compute on the MAIN branch — NOT a branch, which copy-on-write diverges from live data) and set `ANALYTICS_DATABASE_URL` to its connection string in Infisical/Vercel: `neon endpoints create --project-id <PROJECT> --branch-id <MAIN> --type read_only`. | **code fully wired 2026-06-24**; only the env var (replica URL) remains to activate |
