...
| 2 | Read Replica Routing | **All analytics/reporting routes now read through `analyticsDatabase`** (`packages/database/analytics-database.ts`): 8 pure-read routes alias it; 2 mixed routes (events/advanced, events/profitability) keep the account-provisioning write on the primary and route only the OLAP reads to the replica. It transparently falls back to the primary when `ANALYTICS_DATABASE_URL` is unset. **✅ Read replica CREATED 2026-06-24** on the main (prod) branch via `neon branches add-compute main --type read_only --name analytics-read-replica --project-id proud-term-94466127` → endpoint **`ep-square-moon-ahi87sh8`** (read-only host `ep-square-moon-ahi87sh8.c-3.us-east-1.aws.neon.tech`; reads the same prod data as `ep-divine-math`). Correcting an earlier wrong note: the CLI has NO `endpoints` command, but `branches add-compute --type read_only` (the **default** type) creates a read replica via the OAuth-authed CLI — **no API key needed** (`NEON_API_KEY` is a GitHub secret, NOT in Infisical — verified by recursive search of all envs). **Activation = set `ANALYTICS_DATABASE_URL`** in Infisical/Vercel to the prod `DATABASE_URL` with the host swapped to `ep-square-moon-ahi87sh8.c-3.us-east-1.aws.neon.tech` (`packages/database/keys.ts` auto-pooler-izes via `toNeonPoolerUrl`). | **✅ FULLY ACTIVATED 2026-06-24** — replica created + `ANALYTICS_DATABASE_URL` set in Infisical prod `/apps/capsule-pro/api` (pooled host `ep-square-moon-ahi87sh8-pooler…`, connection `SELECT 1` verified); live after Vercel picks up the synced secret (redeploy if Infisical→Vercel sync isn't automatic) |
...
- **OpenTelemetry Tracing Through Command Dispatcher** (`refactor`) — `manifest.command` spans with entity/command/tenant/actor attrs; nested dispatch nests as child spans. Files committed (stale claim resolved). **OTel backend decision (2026-06-24): Sentry is the OTel backend** — the Next.js SDK is OTel-based and exports these spans; no separate OTLP exporter (Jaeger/Honeycomb/Tempo) is wired because it would duplicate the existing Sentry tracing pipeline. Revisit only if a second tracing backend is adopted.
...
- **Read Replica Routing for Analytics/Reporting Queries** (`enhancement`) — Heavy OLAP reads route to `ANALYTICS_DATABASE_URL` when set via `analyticsDatabase`, else primary. **All 10 analytics routes wired 2026-06-24** (activity-feed / command-perf / reactions-log already used the replica client). *Activation = set `ANALYTICS_DATABASE_URL` to a Neon read replica (ops/env only; code is done).*
...
- **Optimistic UI with Governed Command Rollback** (`enhancement`) — `useOptimisticCommand` hook (optimistic → revert on failure). **`StatusTransitionBadge` wired to it 2026-06-24** (inline status changes render the target state instantly and revert with the runtime's friendly error on failure); `InlineEditField` already used it. Remaining adopters: other command-submitting surfaces (bulk actions now batch server-side, so they reconcile atomically rather than optimistically).
