...
0d. For reference, the application source code is in `apps/app/app/*` and the api is `apps/api/app/api/*`.
...
ULTIMATE GOAL: We want to achieve complete functionality of every action that can be taken in the ui. If there is a button or a modal or a form that you can fill out, we need to ensure that completing it or pushing it doesnt result in nothing happening. Consider missing elements and plan accordingly. If an element is missing, search first to confirm it doesn't exist, then if needed author the specification at specs/FILENAME.md. If you create a new element then document the plan to implement it in @IMPLEMENTATION_PLAN.md using a subagent.
