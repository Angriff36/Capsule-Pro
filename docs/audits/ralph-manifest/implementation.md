...
22. **Direct-write audit drift — the "0 governed violations" claim is STALE:** `pnpm manifest:audit-direct-writes` reports **6 governed-entity direct-write violations** (file-level, gate still exits 0 — report-only/baseline; was 7, `CrmScoringRule.create` migrated v0.12.285) across unrelated domains: `PaymentRefundAttempt.create`, `AdminChatThread.update`, `TrainingCompletion.upsert` (×2), `InventoryForecast`/`ReorderSuggestion` (forecast routes), `ProposalTemplate.updateMany` (×2), and `EventImport.create` (`events/actions.ts` bulk import — already a documented deferral). These are pre-existing tech debt accumulated since the v0.12.149 "0 violations" milestone, **NOT** introduced by the Event-hub increment (which removed a direct Event write). **Progress (v0.12.285):** `apps/api/app/api/crm/scoring/route.ts` POST migrated from `database.crmScoringRule.create` → `runManifestCommand("CrmScoringRule","create")` (GET `$queryRaw` read unchanged per §10); the live frontend create path already used the dispatcher (`crmScoringRuleCreate`), so no consumer contract changed. Conformance test: `apps/api/__tests__/crm/scoring-post-governed.test.ts` (4 tests; `@repo/database` mocked without `crmScoringRule.create` so a reverted bypass throws). Follow-up: migrate the remaining 6 to a Manifest command or add a justified `bypasses.json` entry, then refresh the §Governance "0 violations" claim.
