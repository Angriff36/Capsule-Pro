...
export function parseRowToRecord(row: Record<string, string>, dateColumn?: string): SalesRecord | null {
  // Find date column - use provided one or search for it
  let dateStr: string | undefined;

  if (dateColumn) {
    const normalizedDateColumn = dateColumn.toLowerCase().replace(/\s+/g, '_');
    dateStr = row[normalizedDateColumn] || row[dateColumn.toLowerCase()] || row[dateColumn];
  }
  ...
  if (!dateStr) {
    const dateCol = findColumn(row, [
      ...
      'record_date', 'Record Date', 'Record_Date',
      'entry_date', 'Entry Date', 'Entry_Date',
      'inquiry_date', 'Inquiry Date', 'Inquiry_Date',
      'lead_date', 'Lead Date', 'Lead_Date',
