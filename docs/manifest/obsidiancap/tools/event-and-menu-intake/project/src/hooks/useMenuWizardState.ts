...
import type { MenuFormData, DietaryFlag, CostDataProvider, DishCost } from '../types/menu';
...
export const MENU_WIZARD_STEPS: WizardStep[] = [
  { id: 'context', title: 'Event Context', subtitle: 'Occasion, season & scale', icon: 'Calendar' },
  { id: 'service-style', title: 'Service Style', subtitle: 'How should we serve?', icon: 'UtensilsCrossed' },
  { id: 'direction', title: 'Menu Direction', subtitle: 'Choose a culinary theme', icon: 'Compass' },
  { id: 'mains', title: 'Main Courses', subtitle: 'Select your proteins', icon: 'Beef' },
  { id: 'sides', title: 'Sides & More', subtitle: 'Accompaniments & appetizers', icon: 'Salad' },
  { id: 'dietary', title: 'Dietary Needs', subtitle: 'Coverage & guest counts', icon: 'ShieldCheck' },
  { id: 'addons', title: 'Add-Ons', subtitle: 'Dessert, late night & bar', icon: 'Sparkles' },
  { id: 'review', title: 'Review Menu', subtitle: 'Preview & export', icon: 'ClipboardCheck' },
];
...
export function useMenuWizardState(costProvider?: CostDataProvider) {
  ...
  const updateField = useCallback(<K extends keyof MenuFormData>(
    field: K,
  ...
  const toggleAddOn = useCallback((addOn: string) => {
    setFormData((prev: MenuFormData) => {
      const next = prev.addOnSelections.includes(addOn)
        ? prev.addOnSelections.filter((a: string) => a !== addOn)
  ...
  const toggleDietaryNeed = useCallback((flag: DietaryFlag) => {
    setFormData((prev: MenuFormData) => {
      const next = prev.dietaryCoverageNeeds.includes(flag)
        ? prev.dietaryCoverageNeeds.filter((f: DietaryFlag) => f !== flag)
  ...
  const loadCosts = useCallback(async (dishIds: string[]) => {
    if (!costProvider) return;
    const uncached = dishIds.filter((id: string) => !costCache[id]);
    ...
    setCostCache((prev: Record<string, DishCost>) => ({ ...prev, ...results }));
  ...
  const perPersonTotal = useMemo(() => {
    ...
    return items.reduce((sum: number, item: MenuCatalogItem) => sum + item.pricePerPerson, 0);
