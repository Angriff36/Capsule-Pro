...
export default function MenuWizardShell({ costProvider, pricingConfig, ownerViewConfig }: Props) {
  ...
  useEffect(() => {
    if (showCost && menu.formData.selectedItems.length > 0) {
      menu.loadCosts(menu.formData.selectedItems);
    }
  }, [showCost, menu.formData.selectedItems]);
  ...
  const canProceed = useMemo(() => {
    switch (menu.currentStep) {
      ...
      case 3: {
        ...
        const hasMain = menu.formData.selectedItems.some((id: string) => id.startsWith('main-'));
        return hasMain && v.errors.filter((e: string) => e.includes('main')).length === 0;
      }
      case 4: {
        const hasSide = menu.formData.selectedItems.some((id: string) => id.startsWith('side-'));
        return hasSide;
      }
      case 5: return true;
      case 6: return true;
  ...
  const handleGenerateStory = useCallback(async () => {
    menu.setIsGeneratingStory(true);
    try {
      const apiUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-menu-story`;
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({ formData: menu.formData }),
      });
      if (response.ok) {
        const result = await response.json();
        menu.setMenuStory(result.story || '');
      }
    } catch {
      // non-critical
    } finally {
      menu.setIsGeneratingStory(false);
    }
  }, [menu.formData]);
