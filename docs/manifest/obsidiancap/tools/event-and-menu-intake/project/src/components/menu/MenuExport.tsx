...
<div class="header">
  ...
  for (const cat of CATEGORY_ORDER) {
    ...
    for (const item of catItems) {
      const tags = item.dietaryFlags.map((f: DietaryFlag) => `<span class="tag">${getDietaryLabel(f)}</span>`).join('');
      html += `<div class="item"><div class="item-name">${item.name}${tags}</div><div class="item-desc">${item.description}</div></div>`;
    }
    html += `</div>`;
  }

  const usedFlags = new Set(items.flatMap((i: MenuItem) => i.dietaryFlags));
  if (usedFlags.size > 0) {
...
export default function MenuExport({ formData, menuStory }: Props) {
  ...
  const handleEmailCopy = () => {
    ...
    for (const cat of CATEGORY_ORDER) {
      ...
      for (const item of catItems) {
        const tags = item.dietaryFlags.length > 0 ? ` (${item.dietaryFlags.map((f: DietaryFlag) => getDietaryLabel(f)).join(', ')})` : '';
        text += `  ${item.name}${tags}\n  ${item.description}\n\n`;
      }
    }
