...
export default function MenuAddOnsStep({ formData, updateField, toggleMenuItem, showPrice, showCost, costCache }: Props) {
  ...
  const desserts = available.filter((item: MenuItem) => item.category === 'dessert');
  const lateNight = available.filter((item: MenuItem) => item.category === 'late-night');
