import type { MenuCatalogItem, MenuFormData, MenuConstraintResult, Season, DietaryFlag, ServiceStyle, MenuCategory } from '../types/menu';
...
export function getAvailableItems(
  ...
  return menuCatalog.filter((item: MenuCatalogItem) => {
    if (serviceStyle && !item.compatibleStyles.includes(serviceStyle as ServiceStyle)) return false;
...
export function getItemById(id: string): MenuCatalogItem | undefined {
  return menuCatalog.find((item: MenuCatalogItem) => item.id === id);
}

export function getItemsByIds(ids: string[]): MenuCatalogItem[] {
  return ids.map(id => getItemById(id)).filter((item): item is MenuCatalogItem => !!item);
}

export function validateMenuSelection(data: MenuFormData): MenuConstraintResult {
  const errors: string[] = [];
  ...
  for (const [category, minimum] of Object.entries(MINIMUM_BY_CATEGORY)) {
    ...
    if (count < minimum) {
      errors.push(`At least ${minimum} ${category} ${minimum === 1 ? 'item is' : 'items are'} required.`);
    }
  }
  ...
  if (data.serviceStyle) {
    for (const item of selectedItems) {
      if (!item.compatibleStyles.includes(data.serviceStyle as ServiceStyle)) {
        errors.push(`"${item.name}" is not compatible with ${data.serviceStyle.replace(/-/g, ' ')} service.`);
      }
    }
  }
  ...
  for (const item of selectedItems) {
    if (!item.seasons.includes(data.season)) {
      warnings.push(`"${item.name}" may not be available in ${data.season}. Our team will suggest alternatives.`);
    }
  }
  ...
  if (data.dietaryCoverageNeeds.length > 0) {
    for (const need of data.dietaryCoverageNeeds) {
      ...
      if (mainsCovering.length === 0) {
        errors.push(`No main course covers the "${need}" dietary need. Please add at least one ${need} main.`);
      }
    }
  }
...
export function groupByCategory(items: MenuCatalogItem[]): Record<string, MenuCatalogItem[]> {
