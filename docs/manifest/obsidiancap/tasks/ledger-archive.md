...
- No route handler may exist outside `app/api/` that performs data access without authentication.
- All exported functions in Next.js route files must be valid handler names.
- When a manifest-backed route replaces a legacy route, the legacy route must be deleted.
