...
-- RLS Policies (simplified without function dependencies)
...
CREATE INDEX admin_users_tenant_role_idx
  ON tenant_admin.admin_users(tenant_id, role_id) WHERE deleted_at IS NULL;
...
-- ============================================
-- UPDATE POLICIES WITH FUNCTION PERMISSIONS
-- ============================================

-- Update policies after functions are created
...
ALTER POLICY admin_audit_trail_select ON tenant_admin.admin_audit_trail
  USING (
    tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
    AND deleted_at IS NULL
    AND tenant_admin.fn_has_permission('reports.view'::text)
  );
...
ALTER POLICY admin_audit_trail_update ON tenant_admin.admin_audit_trail
  USING (
    tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
    AND deleted_at IS NULL
...
ALTER POLICY admin_audit_trail_delete ON tenant_admin.admin_audit_trail
  USING (
    tenant_id = (auth.jwt() ->> 'tenant_id')::uuid
    AND deleted_at IS NULL
...
-- ============================================
-- INSERT DEFAULT DATA (only when tenant exists)
-- ============================================
...
WHERE tenant_id = '00000000-0000-0000-0000-000000000000'::uuid AND deleted_at IS NULL
