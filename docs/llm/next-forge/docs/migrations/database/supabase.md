...
For authentication, see the [Supabase Auth migration guide](/en/migrations/authentication/supabase) to switch from Clerk to Supabase Auth with organization management, user roles, and more.

Here's how to switch from Neon to [Supabase](https://supabase.com) for your `next-forge` project.

## [1. Sign up to Supabase](#1-sign-up-to-supabase)
...
## [4. Replace the dependencies](#4-replace-the-dependencies)
...
## [5. Update the database package](#5-update-the-database-package)
...
## [6. Update the Prisma schema](#6-update-the-prisma-schema)
...
RLS policies use `auth.uid()` to get the authenticated user's ID from Supabase Auth. Make sure you've completed the [Supabase Auth migration](/en/migrations/authentication/supabase) first.

### [Enable RLS on your tables](#enable-rls-on-your-tables)
...
#### [Organization member policies](#organization-member-policies)
...
#### [Public read, authenticated write](#public-read-authenticated-write)
...
[1. Sign up to Supabase](#1-sign-up-to-supabase)[2. Create a Project](#2-create-a-project)[3. Update the environment variables](#3-update-the-environment-variables)[4. Replace the dependencies](#4-replace-the-dependencies)[5. Update the database package](#5-update-the-database-package)[6. Update the Prisma schema](#6-update-the-prisma-schema)[7. Set up Row Level Security (RLS)](#7-set-up-row-level-security-rls)[Enable RLS on your tables](#enable-rls-on-your-tables)[Create RLS policies](#create-rls-policies)[Organization policies](#organization-policies)[Organization member policies](#organization-member-policies)[Testing RLS policies](#testing-rls-policies)[Common RLS patterns](#common-rls-patterns)[User-owned data](#user-owned-data)[Tenant isolation](#tenant-isolation)[Public read, authenticated write](#public-read-authenticated-write)

[GitHubEdit this page on GitHub](https://github.com/vercel/next-forge/edit/main/docs/content/docs/migrations/database/supabase.mdx)Scroll to topGive feedbackCopy pageAsk AI about this pageOpen in chat
