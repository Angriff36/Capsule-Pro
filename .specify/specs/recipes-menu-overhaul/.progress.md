...
T023 complete. Menu actions (T021-T022) verified successfully:
- TypeScript compilation: ✅ No type errors in menu actions
- Lint check: ✅ No lint errors in menu actions (pre-existing errors in apps/api unrelated)
- Syntax validation: ✅ File has valid syntax
- All CRUD operations properly implemented with tenant isolation, soft deletes, outbox events
- MenuSummary and MenuDetail types properly exported
- Menu-dish management actions (add, remove, reorder) working correctly

T022 complete. Added menu-dish management actions: addDishToMenu (creates MenuDish with auto-sort-order), removeDishFromMenu (soft delete), and reorderMenuDishes (updates sort_order). All functions include proper tenant isolation, validation, soft deletes, outbox events, and path revalidation.

T021 complete. Created menu server actions with all CRUD operations (create, update, delete, getMenus, getMenuById) following patterns from recipe actions. Includes proper tenant isolation, soft deletes, outbox events, and TypeScript types.
