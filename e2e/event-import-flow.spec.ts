...
test.describe('Event Import Flow', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('http://127.0.0.1:2221/events/import', { waitUntil: 'networkidle' });
  });

  test('should upload PDF and CSV, create event with battle board and checklist', async ({ page }) => {
    // Check page loaded
    await expect(page.locator('text=Upload Files')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.locator('text=Khosravi Wedding Time & Attendance.csv')).toBeVisible();

    // Ensure generation options are checked
    const battleBoardChecked = await page.locator('#generateBattleBoard').isChecked();
    const checklistChecked = await page.locator('#generateChecklist').isChecked();

    if (!battleBoardChecked) {
      await page.locator('label:has-text("Generate Battle Board")').click();
    }
    if (!checklistChecked) {
      await page.locator('label:has-text("Generate Pre-Event Review Checklist")').click();
    }
    ...
    try {
      ...
      await page.waitForSelector('text=Import Complete, text=Import failed, text=Errors:', { timeout: 60000 });
      ...
      if (isVisible) {
        ...
        const battleBoardCreated = page.locator('text=Battle Board Created').first();
        const bbVisible = await battleBoardCreated.isVisible().catch(() => false);
        console.log(`✅ Battle Board: ${bbVisible ? 'Created' : 'Not Created'}`);
        ...
        const extractedEvent = page.locator('text=Extracted Event Data').first();
        ...
        if (eeVisible) {
          ...
          const clientText = await page.locator('dd:has-text("Northwest Orthopaedic")').textContent().catch(() => 'Not found');
          console.log(`✅ Client: ${clientText}`);
        }

        // Get any links to navigate to
        const viewBattleBoardBtn = page.locator('button:has-text("View Battle Board")').first();
        if (await viewBattleBoardBtn.isVisible().catch(() => false)) {
          console.log('✅ View Battle Board link available');
        }

        const viewChecklistBtn = page.locator('button:has-text("View Checklist")').first();
        ...
        console.log('✅ Verification complete - check /events for created event');

      } else {
        // Check for errors
        const errors = await page.locator('.text-destructive').textContent().catch(() => 'Unknown error');
