...
function shouldSkipClick(text: string, ariaLabel: string, role: string): boolean {
  const combined = `${text} ${ariaLabel} ${role}`.toLowerCase();
  return SKIP_PATTERNS.some((re) => re.test(combined));
}

test("App spider: authenticated crawl logs errors", async ({ page, baseURL }) => {
  test.setTimeout(180_000);
  expect(baseURL).toBeTruthy();

  const errors: Array<{ url: string; text: string }> = [];
  const warnings: Array<{ url: string; text: string }> = [];
  const failures: Array<{ url: string; status: number; method: string; body?: string }> = [];
  const failedRequests: Array<{ pageUrl: string; requestUrl: string; errorText: string }> = [];
  ...
  page.on("response", async (response) => {
    ...
    if (status >= 400 && isSameOrigin(response.url(), baseURL!)) {
      ...
      failures.push({ url: response.url(), status, method: response.request().method(), body });
    }
  });
  ...
  while (queue.length && visited.size < MAX_VISITS) {
    ...
    if (response && response.status() >= 400) {
      failures.push({ url: response.url(), status: response.status(), method: response.request().method() });
    }
    ...
    const clickables = await page
      .locator("button:visible, [role='button']:visible, [role='tab']:visible, a:visible")
      .all()
      .catch(() => []);
    ...
    for (const el of clickables) {
      ...
      const hasErrorText = await page.locator("text=/error|failed|invalid|required/i").count() > 0;
      if (hasErrorText) {
        const errorTexts = await page.locator("text=/error|failed|invalid|required/i").allTextContents();
        errors.push({ url: page.url(), text: `UI error after click: ${errorTexts.join(", ")}` });
      }
    ...
    const inputs = await page.locator("input:visible, select:visible").all().catch(() => []);
    ...
    for (const href of hrefs) {
      ...
      const normalized = new URL(href, baseURL!).pathname + (new URL(href, baseURL!).search || "");
  ...
  console.log(`[SPIDER] Visited ${visited.size} pages, attempted ${visitLog.reduce((sum, v) => sum + v.clicksAttempted, 0)} interactions`);
  
  if (errors.length || failures.length || failedRequests.length || warnings.length) {
    ...
    if (failedRequests.length) console.log("[SPIDER] Failed requests:", failedRequests);
  }
  ...
  expect(failedRequests, "Failed network requests (DNS/connection errors) found during crawl").toEqual([]);
});
