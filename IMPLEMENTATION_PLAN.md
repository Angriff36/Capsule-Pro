...
5. **Non-transactional writes in payroll — RESOLVED (base data source) 2026-06-10:** `PrismaPayrollDataSource.savePayrollRecords()` now wraps the run-header upsert + all N line-item upserts in a single `$transaction` (the constructor `Omit` was relaxed to expose `$transaction`). Unit tests (`packages/payroll-engine/tests/prisma-data-source.test.ts`) pin atomicity + fail-loud propagation. **Follow-up (still open):** the governed live path `apps/api/lib/payroll/manifest-payroll-data-source.ts` issues `PayrollRun.create` → `process` → N×`PayrollLineItem.create` as separate `runManifestCommandCore` invocations and **swallows** the `process` + per-line-item failures (logs + continues), so a partial payroll can still be reported as success — violates "fail loud" and is non-atomic across command invocations. Making it atomic needs a saga or a shared transactional command context (larger effort).
