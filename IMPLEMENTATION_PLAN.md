...
> **Last updated:** 2026-04-27 (thirty-first pass — closed remaining half of Tech-Debt #41 (email webhook rate limiting). Refactored `apps/api/lib/public-rate-limit.ts` to expose two limiter profiles via a shared `applyLimit` helper: `checkPublicRateLimit` (10/60s, unchanged — for human-driven mutations like contract signing / proposal responses) and new `checkWebhookRateLimit` (60/60s — for machine-driven webhook receivers where legitimate Resend bursts can exceed the human ceiling). Wired `checkWebhookRateLimit` into `apps/api/app/api/collaboration/notifications/email/webhook/route.ts` POST handler at the top — *before* `request.text()`, HMAC verification, and DB lookup — so a flood of unsigned/forged requests cannot exhaust webhook resources. HMAC signature verification (`verifyResendSignature`) remains the primary authentication mechanism; rate limit is defense-in-depth that caps abuse impact of a leaked secret or misbehaving sender. Both limiters fail open on Redis outage so a store outage cannot drop legitimate webhook events. Bucket key per IP (x-forwarded-for first → x-real-ip → "unknown") + HTTP method + URL path with dynamic UUID/long-token segments normalized to `:id` (prevents token cycling to evade the limit). Added 5 new unit tests in `apps/api/__tests__/lib/public-rate-limit.test.ts` covering: distinct sliding-window thresholds (10/60s vs 60/60s) verified via module-load-time snapshot of slidingWindow construction calls, webhook-vs-public limiter routing, 429 response shape with X-RateLimit-* + Retry-After headers, fail-open on rejection, and per-IP bucketing with method+endpoint encoded in the key. Renamed test mocks to `mockPublicLimiter`/`mockWebhookLimiter` and routed the `createRateLimiter` mock by `prefix`. TypeScript: 0 errors. Tests: **1076 passing** (+5), 0 failures, 1 skipped, 8 todo. Resolves the explicit "email webhook still TODO" note from the twenty-ninth pass.)
