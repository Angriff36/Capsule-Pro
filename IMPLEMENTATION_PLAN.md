...
- **Candidates remaining (parent → inferable field):** `RevenueRecognitionSchedule` (Invoice/Event → eventId/clientId), `PrepTask` (PrepList → eventId), `FacilityWorkOrder` (FacilityAsset → facilityId/areaId), `ScheduleShift` (Schedule → locationId — column non-nullable `@db.Uuid`, needs care vs empty seed), `TimelineTask` (Event → assignedTo — Prisma column is `assigneeId`/`assignee_id`, name mismatch with Manifest `assignedTo` → store-mapping check needed), `EventFollowup` (Event → assignedTo — column `assigned_to String?` nullable, verify property→column mapping), `EventContract` (Event → clientId — column non-nullable, also has a Client parent). (`Proposal`, `Shipment`, `WasteEntry`, `CateringOrder` migrated — see above.)
