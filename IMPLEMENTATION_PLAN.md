...
| **34 entities have transitions but 65 status entities lack them** | Incomplete state machine coverage. | IR analysis |
...
| **prisma-projection-options.mjs EventStaff table wrong** | `event_staff` vs actual table `event_staff_assignments`. | `manifest/scripts/prisma-projection-options.mjs` |
...
- **71 of 94 switch-case stores are pure boilerplate CRUD** -- **23 have custom logic**. GenericPrismaStore fallback EXISTS (233 LOC) but is NOT used at runtime.
...
21. **Permission guard is whitelist-based, not deny-by-default:** Commands NOT in `COMMAND_PERMISSION_MAP` pass through unrestricted.
...
47. **prisma-projection-options.mjs EventStaff table wrong:** Maps to `event_staff` but actual table is `event_staff_assignments`. Schema projection will produce wrong DDL. Task 2.5.
...
### 0.2 Fix build.mjs broken path, stale compilerVersion, and orphaned scripts
- **Done when:** `pnpm manifest:build` completes all 4 steps without ENOENT. Provenance shows correct compilerVersion. `generate-all-routes.mjs` has a `package.json` script entry or is removed. Dead `CODE_OUTPUT_DIR` variable in `build.mjs` removed.
...
### 2.8 Adopt `timestamps` entity modifier to eliminate datetime-as-number at the root (HIGHEST IMPACT NEW TASK)
- **Done when:** All 189 entities use the `timestamps` modifier. Hand-declared `createdAt`/`updatedAt` properties removed from source files. `pnpm manifest:compile` succeeds. Prisma projection generates `@default(now())` and `@updatedAt` for all entities. The 559+ datetime-as-number occurrences in event timestamps are systematically prevented for future entities.
...
- **Source to change:** `manifest/runtime/src/manifest-runtime-factory.ts`.
- **Subtasks:**
  - [ ] **7.4a -- RBAC:** `before-guard` middleware replaces `createPermissionGuard` proxy.
  - [ ] **7.4b -- Identity:** `before-policy` with `contextPatch` replaces `resolveUserRole`.
  - [ ] **7.4c -- Audit:** `after-emit` replaces post-hoc telemetry handler.
...
- **Done when:** Dead code removed. Barrel exports cleaned. Typecheck passes.
- **Why:** rules-engine/ (5 files, ~1000 LOC, 0 consumers), entity-graph/ (7 files, ~1400 LOC, 0 consumers, `buildGraphFromIR` is stub), packages/services/ (no package.json, no source, stale node_modules). Total: ~2400+ lines of dead code.
- **Backpressure:** `grep -r "RulesEngine\|EntityGraph\|@repo/services" apps/ packages/` returns 0 matches.
- **Source to change:** Remove `manifest/runtime/src/rules-engine/`, `manifest/runtime/src/entity-graph/`, `packages/services/`. Clean barrel exports from `manifest/runtime/src/index.ts`.
...
| IR source type bugs (datetime-as-number) | **559+ occurrences (UNIVERSAL)** | 559 | CONFIRMED across ALL domains |
...
| Prisma models | 226 total, **173 match IR**, 69 Prisma-only, **16 IR without model**, **15 wrong accessor** | 226/166/23 | CORRECTED: 7 more have models, 15 need accessor overrides |
...
7. **~~32~~ ~~23~~ 16 IR entities without Prisma models:** Reduced from 32->23->16. 7 more entities now have models (BankAccount->EmployeeBankAccount, EventImportWorkflow->EventImport, EventTimelineItem->EventTimeline, LogisticsRoute->DeliveryRoute, QACheck->QualityCheck, QACorrectiveAction->CorrectiveAction, QATemperatureLog->TemperatureLog). Additionally ~14 entities need accessor overrides. Task 0.3 + Task 0.1.
