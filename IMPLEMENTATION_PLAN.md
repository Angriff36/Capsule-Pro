...
- **~140 direct-write violations in API routes** + **~107 in server actions** = **~247 total**. 51 mutate handlers across 42 route files + battle-board server actions migrated to Manifest runtime (Task 8.2 batches 1-9 + Task 8.3 + Task 8.4). **7 hybrid files** remaining (down from 12). notifications package adds 9+ direct DB writes across 4 files.
...
- **Source to change:** `apps/api/app/api/` hand-written route files.
  ...
  - **Progress 2026-06-06 (batch 12):**
    ...
    - Total: 62 mutate handlers in 47 route files. 2689 tests pass, 0 typecheck errors.
  - **Total migrated across all batches:** 62 mutate handlers in 47 route files. Remaining: ~129 violations across ~35 files (most remaining are complex multi-entity transactions or infrastructure entities without Manifest IR definitions).
