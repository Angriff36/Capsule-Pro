...
- [x] **Test Fixes (Manifest HTTP Tests)** - Fixed 47 failing tests in manifest HTTP test files: added manifest runtime mocks to avoid complex database dependencies, updated test assertions to match actual response format (`data.data` instead of `data.result`), fixed outdated manifest-runtime.ts API expectations (changed `@repo/manifest` to `@angriff36/manifest`, `compileToIR` to `getCompiledManifestBundle`). All 675 tests now pass (2026-02-18)
