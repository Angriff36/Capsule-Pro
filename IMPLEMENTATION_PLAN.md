...
- **Source to change:** `apps/app/app/(authenticated)/**/actions*.ts` files.
  ...
  - **Drift blockers (cross-reference Task 0.4) — cannot migrate until IR↔schema reconciled:**
    ...
    - **AdminTask** (`administrative/kanban/actions.ts`): **PARTIALLY RESOLVED 2026-06-05** — `createAdminTask` migrated to governed `AdminTask.create` (batch 3). `updateAdminTaskStatus` is **DEFERRED** — still does `database.adminTask.update({status})`. Root conflict: the kanban UI uses statuses `backlog/in_progress/review/done` but the manifest state machine uses a different vocabulary (`backlog/todo/in_progress/done/cancelled`) with commands `moveToTodo/startProgress/complete/cancel/reopen` whose guards don't match kanban moves (e.g. `review` has NO command; `startProgress` requires `from: todo` but kanban goes backlog→in_progress; `complete` requires `from: in_progress` but kanban goes review→done; `reopen` requires `from: cancelled`). Proper fix (separate increment): reconcile AdminTask transitions + commands to the real kanban lifecycle, update `apps/api/app/api/administrative/tasks/[id]/route.ts` command-map, un-quarantine + update `apps/api/__tests__/administrative/admin-tasks.quarantine.test.ts`, then migrate `updateAdminTaskStatus`. Cross-surface change — kept out of the create migration increment per Rule 7 (surface conflicts, don't average). NOTE: because the file still contains `updateAdminTaskStatus`, the file-level audit count does NOT decrement until that write is also migrated.
...
- **Candidates remaining (parent → inferable field):** `CateringOrder` (Event → venueName/venueAddress), `RevenueRecognitionSchedule` (Invoice/Event → eventId/clientId), `PrepTask` (PrepList → eventId), `FacilityWorkOrder` (FacilityAsset → facilityId/areaId), `ScheduleShift` (Schedule → locationId — column non-nullable `@db.Uuid`, needs care vs empty seed), `TimelineTask` (Event → assignedTo — Prisma column is `assigneeId`/`assignee_id`, name mismatch with Manifest `assignedTo` → store-mapping check needed), `EventFollowup` (Event → assignedTo — column `assigned_to String?` nullable, verify property→column mapping), `EventContract` (Event → clientId — column non-nullable, also has a Client parent). (`Proposal`, `Shipment`, `WasteEntry` migrated — see above.)
