...
- [ ] **Followup**: wire `requireApiManager`/`requireApiAdmin` into remaining hand-written sensitive routes — `accounting/payments/[id]` PUT/POST, `accounting/invoices/[id]` PUT/PATCH/POST/DELETE, `accounting/chart-of-accounts/[id]` (auto-generated, needs IR generator support), `administrative/chat/threads/[threadId]` PATCH (note: user-self mgmt — may stay at session-only), `payroll/approvals/route.ts` POST (goes through manifest runtime — gate at manifest policy instead). Each accounting route has tests mocking `requireTenantId`; switching to role guards requires updating ~5 test files to also mock `requireCurrentUser`. Track as P1.AM-followup.
