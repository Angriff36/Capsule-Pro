...
- **Candidates remaining (parent → inferable field):** `CateringOrder` (Event → venueName/venueAddress), `RevenueRecognitionSchedule` (Invoice/Event → eventId/clientId), `Shipment` (Event → locationId), `WasteEntry` (Event → locationId), `PrepTask` (PrepList → eventId), `FacilityWorkOrder` (FacilityAsset → facilityId/areaId), `ScheduleShift` (Schedule → locationId), `TimelineTask` (Event → assignedTo), `EventFollowup` (Event → assignedTo), `EventContract` (Event → clientId). (`Proposal` migrated — see above.)
