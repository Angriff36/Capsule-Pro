...
| Routes | ✅ **DONE 2026-04-26** | CRUD works; `DeliveryRoute`/`RouteStop` schema already had every field the optimizer needs (totalDistance, totalDuration, optimizationScore, optimizationAlgorithm, distanceFromPrevious, timeFromPrevious — see `schema.prisma:5805-5897`). The 501 stub at `apps/api/app/api/logistics/routes/commands/optimize/route.ts` was replaced with a real nearest-neighbor TSP heuristic. Anchor = `stopNumber=1`; greedy NN over remaining stops by Haversine distance. Persists resequence inside `database.$transaction` with **two-phase renumber** (negative parking → final 1..N) to avoid `(route_id, stop_number)` unique-constraint violation. Status transitions to `optimized`; algorithm name `nearest-neighbor-v1`; score clamped to `[0, 100]`. Validates 400/404/409/422 paths (missing routeId, route not found, completed/cancelled, missing coordinates). 10 tests pass at `apps/api/__tests__/logistics/route-optimization.test.ts`. |
...
- **Tests:** ~~zero logistics tests~~ — **10 tests added 2026-04-26** at `apps/api/__tests__/logistics/route-optimization.test.ts` covering nearest-neighbor optimization across 400/404/409/422 paths plus 0/1/4-stop topologies and tenant-scoping.
...
- ~~`apps/api/app/api/logistics/routes/commands/optimize/route.ts`.~~ **RESOLVED 2026-04-26**: replaced 501 stub with nearest-neighbor TSP heuristic + 10 tests at `apps/api/__tests__/logistics/route-optimization.test.ts`.
...
~~Missing model (1): `prepTaskPlanWorkflow` — has routes but no Prisma model (fabrication issue)~~ **RESOLVED 2026-04-26**: Added `PrepTaskPlanWorkflow` model to `schema.prisma` with `@@id([tenantId, id])`, `@@map("prep_task_plan_workflows")`, `@@schema("tenant_kitchen")`, 30+ fields (JSON for arrays/objects, DateTime for timestamps, proper nullable fields). Added `Account.prepTaskPlanWorkflows` back-relation.
...
15. Logistics: real GPS/webhook integration; ~~add `Driver`, `Vehicle` models~~ ✅ (added 2026-04-26); ~~implement `/routes/commands/optimize`~~ ✅ (nearest-neighbor heuristic + 10 tests, 2026-04-26); create logistics manifests.
...
| `apps/api/app/api/public/proposals/[token]/respond/route.ts` | 38, 123-142 | ~~`notes`, `responderName`, `responderEmail` stored unsanitized~~ **FIXED 2026-04-26** — `sanitizeText`/`sanitizeEmail` applied in `apps/api/lib/sanitize.ts`; tests in `apps/api/__tests__/lib/sanitize.test.ts` (20 tests) |
...
| `apps/api/app/api/public/contracts/[token]/sign/route.ts` | 95-104 | No duplicate signature prevention; unlimited signatures per token | Attacker can flood contract with unlimited signatures (rate-limit added 2026-04-26 mitigates flood; duplicate prevention still TODO) |
| All public mutation endpoints | N/A | ~~No rate limiting~~ **FIXED 2026-04-26** — IP-based 10/min sliding window via `apps/api/lib/public-rate-limit.ts` applied to contracts/sign + proposals/respond; tests in `apps/api/__tests__/lib/public-rate-limit.test.ts` (8 tests). Email webhook still TODO. |
...
41. ~~**HIGH**: Add rate limiting middleware to all public mutation endpoints~~ **PARTIALLY FIXED 2026-04-26** — contract signing + proposal responding now rate-limited (IP-based, 10/min). Email webhook still pending.
42. ~~**HIGH**: Sanitize `notes`/`responderName` in proposal respond endpoint to prevent stored XSS.~~ **FIXED 2026-04-26** — `sanitizeText` applied to `responderName`/`notes`; `sanitizeEmail` applied to `responderEmail`. Tests added.
