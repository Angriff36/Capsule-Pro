...
## Persistence — Compile-to-Convex (Manifest projection)

**Architecture:** Manifest IR is **compiled** to Convex — not interpreted at runtime with a store adapter.

### Source of truth

1. **Domain:** `manifest/source/*.manifest`
2. **IR:** `manifest/ir/kitchen.ir.json`
3. **Backend:** `convex/` — schema, queries, mutations, crons, http, sagas (**all generated**)

### Hard rules

1. **Never hand-edit** files containing `GENERATED by the Manifest → Convex projection`.
2. **No store adapter, no `RuntimeEngine` in the request path**, no `@repo/database`, no Prisma.
3. **Governed writes** only through generated Convex mutations (policies/guards/reactions compiled in).
4. **Reactions** = inline fan-out in the same mutation transaction — not schedulers.
5. **Reads** = Convex queries; tenant-scoped; no domain semantics.

### Workflow

```
pnpm manifest:compile
pnpm manifest:generate-convex
pnpm manifest:check-convex-drift
npx convex dev
```

CI: `pnpm manifest:ci`

See `constitution.md` v5 and `manifest/convex-phase-out-registry.json`.
