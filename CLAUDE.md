...
2. **Verify table names against `packages/database/prisma/schema/*.prisma` before writing raw SQL.** Naming is **not** consistently snake_case in this repo. Examples that have burned past sessions:
   ...
   Always `rg -n "@@map|model <Name>" packages/database/prisma/schema/` for any model whose table you reference (the schema is now a **multi-file directory** `prisma/schema/*.prisma` — e.g. `manifest.prisma` is auto-generated by the Manifest Prisma projection, DO NOT hand-edit it). If the model has no `@@map`, the table name is the PascalCase model name verbatim.
...
4. **Never run `prisma db push`** (disabled in this repo) and **never run `prisma migrate reset`** without explicit user confirmation — it drops all data. (On the disposable dev DB, reset **with confirmation** is the standard recovery for a partially-applied failed migration — Prisma does NOT wrap migrations in a transaction, so a mid-migration failure leaves the DB half-migrated.)
5. **There is no "accepted drift" and no `db:repair` workflow.** Both were removed 2026-07-10 after `20260710142245_reconcile_schema_truth` reconciled migration history with the schema. `pnpm db:check` is strict: any live-DB↔schema diff fails. Never sanitize a diff, trim generated migration SQL, or maintain a drift allowlist — fix the source (Manifest `.manifest` / `infra.prisma`) or create a proper migration. One interim exception is documented in `docs/database/KNOWN_ISSUES.md` (uuid `SET DEFAULT ''` — pending `@angriff36/manifest` projection release).

### Standard workflow (from `docs/database/CONTRIBUTING.md`, copied here for visibility)

Use the repo's wrapper scripts — not raw `pnpm prisma`. **Migration CLI connection:** `packages/database/.env` (gitignored, loaded only by `packages/database/prisma.config.ts`) must hold `DATABASE_URL` pointing at the Neon **direct** endpoint (no `-pooler` in the host) — Neon's Prisma guidance is to never run migrations through PgBouncer. Runtime apps keep using the pooled URL from their own env. `SHADOW_DATABASE_URL` (in `packages/database/.env.local`) points at the `prisma_shadow` database on the same Neon instance; it is required for `prisma migrate diff --from-migrations` and used by `migrate dev` when present (otherwise Neon auto-provisions). It is **not** part of `@repo/database/keys` (app/runtime validation is **`DATABASE_URL` only**); build, `prisma generate`, `db:deploy`, `migrate:status`, and app startup do **not** require it.

```
1. pnpm db:check                                      # detect drift first — must be clean before you start
2. edit the SOURCE: manifest/source/*.manifest (Manifest-owned) or infra.prisma (hand-owned)
   — for Manifest changes, regenerate: manifest:compile → manifest generate -p prisma → metadata/client/ir:embed
3. pnpm db:dev --create-only --name X                 # generate + validate SQL via shadow DB
4. review the generated migration                     # verify it does ONLY what you intend
5. pnpm db:deploy                                     # apply to current DB
6. pnpm db:check                                      # confirm zero drift
7. pnpm prisma:check                                  # regenerate the Prisma Client (part of the normal workflow)
8. commit schema + migration folder + any docs in one commit
```

Custom SQL Prisma cannot express (partial indexes, CHECK constraints) goes into the migration created in step 3 **before** it is applied — that is the officially supported "customizing migrations" flow. Prisma's differ ignores partial indexes and CHECK constraints, so they do not show up as drift.

`pnpm migrate` runs the chain (`db:validate-migrations && db:check && prisma:check && db:dev`) but **omits `--create-only`**, so it auto-applies the generated migration. Use it only when you don't need a review-before-apply step. `pnpm migrate:status` is the wrapper for `prisma migrate status`; `pnpm migrate:resolve` for `prisma migrate resolve`. Do NOT run `prisma format` against the schema folder — it re-indents the generated `manifest.prisma` and breaks the projection drift gate (`manifest:schema:check`).

### Recovery cheatsheet (when things are already broken)

- **`P3009` "failed migrations in target database":** `pnpm migrate:resolve -- --rolled-back <name>`, fix the SQL, redeploy. ⚠ Prisma applies migrations WITHOUT a wrapping transaction — a failed migration may be **partially applied**; on the disposable dev DB the clean recovery is `prisma migrate reset --force` (with user confirmation) after fixing the SQL.
- **`_prisma_migrations` row exists but folder is missing:** **first** try to restore the folder. Check `git stash list` (folder may be in `stash@{N}^3` untracked tree, recover with `git show stash@{N}^3:<path> > <path>`). Also check unreachable git blobs (`git fsck --unreachable`) and other dev clones. **Restoring the file is always the preferred fix** — Prisma's official guidance is to repair migration histories by restoring/reverting migration files, not by editing `_prisma_migrations`.
