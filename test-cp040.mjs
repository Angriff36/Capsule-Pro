...
async function main() {
  ...
  try {
    ...
    const validR = await apiFetch(page, "/api/command-board/cards/commands/create", "POST", {
      boardId: BOARD_ID,
    ...
    if (validR.status === 200) {
      ...
      console.log(`  Response:`, JSON.stringify(validR.data)?.substring(0, 300));
    }

    // ── CommandBoardCard.create — Test Case 2: Missing title ─────
    console.log("\n═══ CommandBoardCard.create: MISSING TITLE (guard) ═══\n");
    const noTitleR = await apiFetch(page, "/api/command-board/cards/commands/create", "POST", {
      boardId: BOARD_ID,
      title: "",
      cardType: "task",
    });
    ...
    if (noTitleR.status === 422 || noTitleR.status === 400) {
      ...
      console.log(`  Error: ${JSON.stringify(noTitleR.data)?.substring(0, 200)}`);
    } else {
      console.log(`  Response:`, JSON.stringify(noTitleR.data)?.substring(0, 200));
    }

    // ── CommandBoardCard.create — Test Case 3: Missing boardId ───
    console.log("\n═══ CommandBoardCard.create: MISSING BOARD ID (guard) ═══\n");
    const noBoardR = await apiFetch(page, "/api/command-board/cards/commands/create", "POST", {
      boardId: "",
      title: "Some Title",
      cardType: "task",
    });
    ...
    if (noBoardR.status === 422 || noBoardR.status === 400) {
      console.log(`  Guard triggered: ✓ CORRECTLY REJECTED`);
      console.log(`  Error: ${JSON.stringify(noBoardR.data)?.substring(0, 200)}`);
    } else {
      console.log(`  Response:`, JSON.stringify(noBoardR.data)?.substring(0, 200));
    }

    // ── CommandBoardCard.create — Test Case 4: Invalid cardType ──
    console.log("\n═══ CommandBoardCard.create: INVALID CARD TYPE (constraint) ═══\n");
    const badTypeR = await apiFetch(page, "/api/command-board/cards/commands/create", "POST", {
      boardId: BOARD_ID,
      title: "QA Test",
    ...
    if (badTypeR.status === 422 || badTypeR.status === 400) {
      ...
      console.log(`  Error: ${JSON.stringify(badTypeR.data)?.substring(0, 200)}`);
    } else {
      console.log(`  Response:`, JSON.stringify(badTypeR.data)?.substring(0, 200));
    }

    // ── CommandBoardConnection.create — Self-connection guard ─────
    console.log("\n═══ CommandBoardConnection.create: SELF-CONNECTION (guard) ═══\n");
    const selfConnectR = await apiFetch(page, "/api/command-board/connections/commands/create", "POST", {
      boardId: BOARD_ID,
    ...
    if (selfConnectR.status === 422 || selfConnectR.status === 400) {
      console.log(`  Guard triggered: ✓ CORRECTLY REJECTED`);
      console.log(`  Error: ${JSON.stringify(selfConnectR.data)?.substring(0, 200)}`);
    } else {
      console.log(`  Response:`, JSON.stringify(selfConnectR.data)?.substring(0, 200));
    }

    // ── CommandBoard.update — Non-draft board guard ───────────────
    console.log("\n═══ CommandBoard.update: NON-DRAFT BOARD (guard) ═══\n");
    // The 'test' board is status='draft' so let's try to update it
    const updateR = await apiFetch(page, `/api/command-board/boards/commands/update`, "POST", {
      boardId: BOARD_ID,
    ...
    if (updateR.status === 200) {
      ...
      console.log(`  Response:`, JSON.stringify(updateR.data)?.substring(0, 200));
    }
    ...
    console.log("    Guard (boardId required):     " + (noBoardR.status !== 200 ? "✓ BLOCKED" : "✗ ALLOWED"));
    console.log("    Guard (title required):        " + (noTitleR.status !== 200 ? "✓ BLOCKED" : "✗ ALLOWED"));
    console.log("    Constraint (cardType valid):   " + (badTypeR.status !== 200 ? "✓ BLOCKED" : "✗ ALLOWED"));
    console.log("    Guard (no self-connection):     " + (selfConnectR.status !== 200 ? "✓ BLOCKED" : "✗ ALLOWED"));
    console.log("");
    ...
    console.log("    Policies: role-based access (kitchen_staff through admin)");
    console.log("    Events: emitted on success (CommandBoardCardCreated etc.)");
