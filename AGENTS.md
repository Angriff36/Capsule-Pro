...
- **`pnpm db:dev --create-only` gotcha (2026-07-12):** long-running (Neon shadow-DB provisioning + applying pending folder migrations) and often auto-backgrounded — don't pipe it through `tail` (buffers all output until exit → looks hung) and don't kill a mid-run invocation (it leaves the migration created AND applied; a second run then re-diffs to empty and creates an EMPTY duplicate migration). Run it once, let it finish, read the generated SQL. If an empty duplicate migration is created (and even applied to dev), just **delete its folder** — empty migrations have no schema effect, so dev stays consistent (`prisma migrate status` + `pnpm db:check` stay clean; verified 2026-07-12). Don't `migrate resolve --rolled-back` it (P3012 blocks that on a successfully-applied migration) and don't manually edit `_prisma_migrations` (README-forbidden; the leftover dev row is harmless).
- **`pnpm install` engine gotcha (2026-07-13):** a plain `pnpm install` (any form, incl. `--lockfile-only`) fails with `ERR_PNPM_UNSUPPORTED_ENGINE` — `packages/analytics` → `posthog-node@5.37.0` requires Node ≥22.22 but the local toolchain is 22.18. Workaround: `pnpm install --lockfile-only --config.engine-strict=false` (the lockfile resolution itself is Node-version-independent). Caveat: that regen can carry unrelated transitive churn (pnpm 10.24 re-resolves `@types/node` + a couple peer annotations), so dep-reclassification edits produce noisy lockfile diffs from this env — prefer code-only changes when possible.
...
- **Focused tests:** `pnpm --filter api test -- <path>` runs the WHOLE suite (pnpm's `--` passthrough doesn't filter). For a single file, run from the package: `cd apps/api && pnpm exec cross-env NODE_ENV=test vitest run <path>` (same for `app`). Same pattern for a single test: append `-t "<name>"`.
- **`git add` for `[id]` route dirs + Bash cwd drift (2026-07-14):** the Bash tool's working directory **persists across calls** — a prior `cd apps/api && ...` leaves later relative-path git commands running from `apps/api` (paths look doubled and silently fail to match). And Next.js bracket route dirs (`[id]`, `[eventId]`) are git pathspec **globs** (`[id]` = char-class 'i'|'d'), so they won't match literally. Fix both: check `pwd` first, and stage bracket paths with the per-pathspec literal magic — `git add ':(literal)apps/api/app/api/events/contracts/[id]/history/route.ts'` — or stage from the repo root with `git -C "$(git rev-parse --show-toplevel)"`.
