...
async function test_cp031(page, sessionToken) {
  ...
  console.log(
    "Create event:",
    createResult.status,
    JSON.stringify(createResult.data)?.substring(0, 200)
  );
  ...
  const inList =
    listResult.data?.data?.some((e) => e.title === title) ||
  ...
  const searchResult = await apiFetch(
    page,
    `/api/events?search=${encodeURIComponent(title)}`
  );
...
async function test_cp048(page, sessionToken) {
  ...
  const eventId =
    eventsResult.data?.data?.[0]?.id || eventsResult.data?.[0]?.id;
  ...
  console.log(
    `   Session A:   ${cardsA0} cards before, ${cardsA1} after refresh`
  );
...
async function test_cp049(page, sessionToken) {
  ...
  const inlineErrors = errors.filter(
    (e) => e.includes("error") || e.includes("Error") || e.includes("failed")
  );
  ...
  console.log("=".repeat(60));
  console.log(
    `   CB6 (CRUD):   ${cards1 > cards0 ? "✓ PASS (card added)" : "⚠ CAN'T VERIFY (no add btn)"}`
  );
  console.log(`   CB7 (Undo):   ${undoWorked ? "✓ PASS" : "⚠ NO EFFECT"}`);
  console.log(
    `   CB8 (Errors): ${inlineErrors.length > 0 ? "✓ INLINE ERRORS" : "⚠ NOT TESTED"}`
  );
  console.log("=".repeat(60));
...
async function main() {
  ...
  try {
    ...
    console.log(
      "cp-031 (Create Event):",
      cp031.inList && cp031.inSearch ? "✓ PASS" : "⚠ PARTIAL"
    );
