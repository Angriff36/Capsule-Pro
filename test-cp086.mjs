...
async function main() {
  ...
  try {
    ...
    const pageError =
      html.includes("Oops") || html.includes("Something went wrong");
    console.log(
      `  Event detail page: ${pageError ? "✗ 500 ERROR" : "✓ LOADED"}`
    );
    ...
    const cancelEndpoint = await apiFetch(
      page,
      `/api/event/${eventId}/cancel`,
      "POST"
    );
    console.log(
      `  POST /api/event/{id}/cancel: ${cancelEndpoint.status === 404 ? "✗ NOT FOUND (404)" : `⚠ STATUS ${cancelEndpoint.status}`}`
    );

    // Check cancel via PATCH
    const cancelPatch = await apiFetch(page, `/api/event/${eventId}`, "PATCH", {
      status: "cancelled",
    });
    console.log(
      `  PATCH /api/event/{id} status=cancelled: ${cancelPatch.status}`
    );
    ...
    const actionResult = await page.evaluate(
      async ({ id }) => {
        ...
        try {
          const r = await fetch("/events/actions", {
            method: "POST",
            body: fd,
            credentials: "include",
          });
    ...
    console.log(
      `  Server action (updateEvent status=cancelled): ${actionResult.error ? "ERROR: " + actionResult.error : "STATUS " + actionResult.status}`
    );
    ...
    const eventsAfter =
      eventsAfterResult.data?.data || eventsAfterResult.data || [];
    ...
    console.log(
      `  Prep tasks: ${taskError ? "ERROR: " + taskError : taskCount + " found"}`
    );
    ...
    console.log(
      `  Prep lists: ${prepError ? "ERROR: " + prepError : prepCount + " found"}`
    );
    ...
    console.log(
      `  Schedules: ${schedError ? "ERROR: " + schedError : schedCount + " found"}`
    );

    // Overall downstream assessment
    if (taskCount === 0 && prepCount === 0 && schedCount === 0) {
      console.log(
        "  Downstream: ⚠ No downstream records exist in DB for this tenant"
      );
    }
    ...
    console.log("    • UI blocked — event detail page returns 500");
    console.log(
      "    • STATUS: ⚠ PARTIALLY IMPLEMENTED (API works, UI blocked)"
    );
    console.log("");
