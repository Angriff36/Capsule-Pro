...
async function main() {
  ...
  try {
    ...
    const boardsMatch =
      JSON.stringify(boardsA.map((b) => b.id).sort()) ===
      JSON.stringify(boardsB.map((b) => b.id).sort());
    ...
    const hasCards =
      boardsA.some((b) => b.id === testBoardId) ||
      boardsB.some((b) => b.id === testBoardId);
    ...
    console.log(
      `   Session A cards on board ${testBoardId.substring(0, 8)}: ${cardsOnBoardA.length}`
    );
    console.log(
      `   Session B cards on board ${testBoardId.substring(0, 8)}: ${cardsOnBoardB.length}`
    );
    ...
    console.log(
      `   Same card IDs in both sessions: ${cardsMatch ? "✓ YES" : "✗ NO"}`
    );
    ...
    if (testCard) {
      console.log(
        `   Using card: ${testCard.id.substring(0, 8)} — "${testCard.title}"`
      );

      // Try update via card command endpoint
      const updateR = await pageA.evaluate(
      ...
      console.log(
        `   Card update API (POST /cards/commands/update): ${updateR.status === 200 ? "✓ PASS" : updateR.status === 404 ? "✗ NOT DEPLOYED (404)" : `⚠ STATUS ${updateR.status}`}`
      );

      if (updateR.status !== 200) {
        // Try alternative: board update
        const boardUpdateR = await pageA.evaluate(
          async ({ boardId }) => {
      ...
      console.log(
        "   ⚠ No cards on this board — cannot test update concurrency"
      );
    }
    ...
    await pageA.goto(`${BASE}/command-board/${testBoardId}`, {
    ...
    console.log(
      `   /command-board/{id}: ${htmlA.includes("Oops") ? "✗ 500 ERROR" : "✓ LOADED"}`
    );

    const battleBoardEvent =
      boardsA.find((b) => b.eventId)?.eventId ||
      "321bb0cf-a527-484c-9051-2b73c8dd6e76";
    await pageB.goto(`${BASE}/events/${battleBoardEvent}/battle-board`, {
      waitUntil: "domcontentloaded",
      timeout: 15_000,
    });
    ...
    console.log(
      `   /events/{id}/battle-board: ${htmlB.includes("Oops") ? "✗ 500 ERROR" : "✓ LOADED"}`
    );
    ...
    console.log(
      "    /command-board/{id}:         ✗ BLOCKED (500 - Clerk orgs=[])"
    );
    console.log(
      "    /events/{id}/battle-board:  ✗ BLOCKED (500 - Clerk orgs=[])"
    );
    console.log("");
    ...
    console.log(
      "    - State consistency verified (both sessions see same data)"
    );
    console.log("=".repeat(60));
  } finally {
    await browser.close();
