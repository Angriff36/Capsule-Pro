import type { QuarterlyMetrics, SalesRecord } from "../types";
import { daysBetween, formatMonthShort, getMonthsInRange } from "../utils/date";
...
function computeSalesCycleLength(
  records: SalesRecord[]
): QuarterlyMetrics["salesCycleLength"] {
  ...
  const durations = withCycle.map((r) =>
    daysBetween(r.proposalDate!, r.closeDate!)
  );
