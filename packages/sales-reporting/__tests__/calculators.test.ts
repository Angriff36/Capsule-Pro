...
describe("calculateWeeklyMetrics", () => {
  ...
  it("calculates revenue by event type", () => {
    ...
    expect(metrics.revenueByEventType.Wedding).toBe(25_000);
    expect(metrics.revenueByEventType.Corporate).toBe(25_000);
  });
  ...
  it("calculates lost opportunities", () => {
    const records = [
      createMockRecord({
        ...
        status: "lost",
        revenue: 5000,
        eventName: "Lost Deal 1",
      }),
      createMockRecord({
        date: new Date("2024-01-16"),
        status: "lost",
        revenue: 10_000,
        eventName: "Lost Deal 2",
      }),
      createMockRecord({
        date: new Date("2024-01-17"),
        status: "won",
    ...
    expect(metrics.lostOpportunities.totalValue).toBe(15_000);
  ...
  it("identifies top pending deals", () => {
    const records = [
      ...
      createMockRecord({
        ...
        status: "proposal_sent",
        revenue: 20_000,
        eventName: "Medium Deal",
      }),
      createMockRecord({
        date: new Date("2024-01-18"),
        status: "pending",
        revenue: 25_000,
        eventName: "Large Deal",
      }),
    ];
    ...
    expect(metrics.topPendingDeals[0].revenue).toBe(30_000);
    expect(metrics.topPendingDeals[0].eventName).toBe("Big Deal");
    expect(metrics.topPendingDeals[1].revenue).toBe(25_000);
    expect(metrics.topPendingDeals[2].revenue).toBe(20_000);
  });
...
describe("calculateMonthlyMetrics", () => {
  it("calculates total revenue for the month", () => {
    ...
    expect(metrics.totalRevenue).toBe(35_000);
  });
  ...
  it("calculates average event value", () => {
    ...
    expect(metrics.avgEventValue).toBeCloseTo(20_000, 0);
  });
  ...
  it("calculates pipeline forecast", () => {
    ...
    expect(metrics.pipelineForecast.pendingValue).toBe(65_000);
  ...
  it("calculates lead source breakdown", () => {
    ...
    expect(metrics.leadSourceBreakdown.Website).toEqual({
      count: 2,
      revenue: 20_000, // Only won revenue counted
...
describe("calculateQuarterlyMetrics", () => {
  it("calculates total revenue for the quarter", () => {
    ...
    expect(metrics.totalRevenue).toBe(60_000);
  });
...
describe("Calculator edge cases", () => {
  it("handles records with no matching event type", () => {
    ...
    expect(metrics.revenueByEventType.Other).toBe(15_000);
  });
