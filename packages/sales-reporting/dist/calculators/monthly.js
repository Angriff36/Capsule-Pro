...
function calculateMonthlyMetrics(allRecords, start, end) {
  ...
  const previousMonthRevenue =
    prevWon.length > 0 ? (0, shared_1.sumRevenue)(prevWon) : null;
  const yoyStart = (0, date_1.startOfMonth)(
    (0, date_1.subtractYears)(start, 1)
  );
  const yoyEnd = (0, date_1.endOfMonth)((0, date_1.subtractYears)(start, 1));
  const yoyRecords = (0, shared_1.filterByDateRange)(
    allRecords,
    yoyStart,
    yoyEnd
  );
  ...
  const yearOverYearRevenue =
    yoyWon.length > 0 ? (0, shared_1.sumRevenue)(yoyWon) : null;
  ...
  const winLossTrends = weeks.map((week) => {
    const weekRecords = (0, shared_1.filterByDateRange)(
      allRecords,
      week.start,
      week.end
    );
  ...
  const pipelineForecast = {
    ...
    weightedForecast:
      (0, shared_1.sumRevenue)(allPending) * (historicalConversion || 0.3),
