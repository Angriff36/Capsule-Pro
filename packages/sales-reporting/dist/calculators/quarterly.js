...
function computeSalesCycleLength(records) {
  const withCycle = records.filter(
    (r) => r.status === "won" && r.proposalDate && r.closeDate
  );
  if (withCycle.length === 0) {
    return { avg: 0, min: 0, max: 0, bySegment: {} };
  }
  const durations = withCycle.map((r) =>
    (0, date_1.daysBetween)(r.proposalDate, r.closeDate)
  );
  ...
  const bySegment = {};
  const segmented = (0, shared_1.groupBy)(
    withCycle,
    (r) => r.eventType || "Other"
  );
...
function generateRecommendations(metrics, records) {
  ...
  if (segments.length > 0) {
    ...
    recommendations.push(
      `"${best[0]}" events generated the highest revenue at ${(0, formatting_1.formatCurrencyFull)(best[1].revenue)} — prioritize marketing and capacity for this segment.`
    );
  }
  ...
  if (referrals.length > 0) {
    ...
    if (best[1].conversionRate > 0) {
      recommendations.push(
        `"${best[0]}" has the highest conversion rate at ${(0, formatting_1.formatPercent)(best[1].conversionRate)}. Consider increasing investment in this lead channel.`
      );
    }
  }
  ...
  if (wonCount + lostCount > 0) {
    ...
    if (rate < 0.4) {
      recommendations.push(
        `Win rate of ${(0, formatting_1.formatPercent)(rate)} is below target. Review qualification criteria and competitive positioning.`
      );
    }
  }
...
function computeGoals(metrics, records) {
  ...
  const wonRevenue = (0, shared_1.sumRevenue)(
    (0, shared_1.filterByStatus)(records, "won")
  );
  const avgDealSize = (0, shared_1.averageRevenue)(
    (0, shared_1.filterByStatus)(records, "won")
  );
  const dealsNeeded =
    avgDealSize > 0 ? Math.ceil(revenueTarget / avgDealSize) : 0;
  const leadTarget =
    conversionTarget > 0 ? Math.ceil(dealsNeeded / conversionTarget) : 0;
...
function calculateQuarterlyMetrics(allRecords, start, end) {
  ...
  const segmentGroups = (0, shared_1.groupBy)(
    records,
    (r) => r.eventType || "Other"
  );
  ...
  const pricingTrends = months.map((monthStart) => {
    ...
    const monthRecords = (0, shared_1.filterByDateRange)(
      allRecords,
      monthStart,
      monthEnd
    );
  ...
  const sourceGroups = (0, shared_1.groupBy)(
    records,
    (r) => r.leadSource || "Unknown"
  );
