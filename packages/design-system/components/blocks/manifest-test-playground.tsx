                ...
                selectedCommandDetail.parameters.length > 0 && (
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedCommandDetail.parameters.map((param) => (
                ...
                <Tabs defaultValue="output">
                  <TabsList className="grid w-full grid-cols-4">
                    ...
                    <TabsTrigger value="guards">
                      Guards
                      {executionResult.guards.filter((g) => !g.passed).length >
                        0 && (
                        <Badge className="ml-1 h-5 px-1" variant="destructive">
                          {
                            executionResult.guards.filter((g) => !g.passed)
                              .length
                          }
                        </Badge>
                      )}
                    ...
                    <TabsTrigger value="constraints">
                      Constraints
                      {executionResult.constraints.filter((c) => !c.passed)
                        .length > 0 && (
                        <Badge className="ml-1 h-5 px-1" variant="destructive">
                          {
                            executionResult.constraints.filter((c) => !c.passed)
                  ...
                  <TabsContent className="space-y-2" value="guards">
                    {executionResult.guards.length === 0 ? (
                      ...
                      executionResult.guards.map((guard, i) => (
                        <Card
                  ...
                  <TabsContent className="space-y-2" value="constraints">
                    {executionResult.constraints.length === 0 ? (
                      ...
                      executionResult.constraints.map((constraint, i) => (
                        <Card
                          className={`p-3 ${
                            constraint.passed
                              ? ""
