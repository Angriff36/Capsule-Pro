...
export function calculateCriticalPath(
  ...
  const taskMap = new Map<string, TaskForCPM>();
  const taskDurationMinutes = new Map<string, number>();
 ...
 */
export function validateTasksForCPM(tasks: TaskForCPM[]): void {
  if (tasks.length === 0) {
 ...
 */
export function getCriticalPathOrder(
  results: Map<string, CriticalPathResult>
): string[] {
  const criticalTasks: Array<{ id: string; earliestStart: Date }> = [];
