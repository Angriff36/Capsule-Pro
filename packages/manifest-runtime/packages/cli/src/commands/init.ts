...
function createConfigFromAnswers(answers: InitAnswers): ManifestConfig {
  ...
  if (
    answers.enableCodegen &&
    answers.projectionTarget &&
    answers.codeOutputDir
  ) {
    config.projections = {
      [answers.projectionTarget]: {
...
function showPostInit(answers: InitAnswers, config: ManifestConfig) {
  ...
  console.log(
    chalk.gray("   echo 'entity User { name: string }' > User.manifest")
  );
  console.log("");
  console.log("2. Compile to IR:");
  console.log(chalk.gray("   manifest compile User.manifest"));
  console.log(
    chalk.gray(`   manifest compile User.manifest -o ${config.output}user.json`)
  );
  console.log("");
  ...
  if (answers.enableCodegen && answers.projectionTarget) {
    ...
    const output =
      config.projections?.[answers.projectionTarget]?.output || "generated";
    console.log(
      chalk.gray(`   manifest generate ${config.output} -o ${output}`)
    );
    console.log(
      chalk.gray(
        `   manifest generate ${config.output} -p ${answers.projectionTarget}`
      )
    );
    console.log("");
  }
  ...
  console.log(
    chalk.gray("Edit manifest.config.yaml to customize paths and outputs.")
  );
  console.log("");
}
