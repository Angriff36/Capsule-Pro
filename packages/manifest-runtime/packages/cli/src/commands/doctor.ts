...
function summarizeDiff(diff: EntitySurfaceDiff): {
  errors: string[];
  warnings: string[];
} {
  const errors: string[] = [];
  ...
  if (diff.commands.missingInIR.length > 0)
    errors.push(
      `Commands missing in IR: ${diff.commands.missingInIR.join(", ")}`
    );
  if (diff.commands.extraInIR.length > 0)
    warnings.push(
      `Extra commands in IR: ${diff.commands.extraInIR.join(", ")}`
    );
  if (diff.properties.missingInIR.length > 0)
    errors.push(
      `Properties missing in IR: ${diff.properties.missingInIR.join(", ")}`
    );
  if (diff.properties.extraInIR.length > 0)
    warnings.push(
      `Extra properties in IR: ${diff.properties.extraInIR.join(", ")}`
    );
  if (diff.emits.missingInIR.length > 0)
    warnings.push(
      `Emitted events missing in IR: ${diff.emits.missingInIR.join(", ")}`
    );
  if (diff.emits.extraInIR.length > 0)
    warnings.push(
      `Extra emitted events in IR: ${diff.emits.extraInIR.join(", ")}`
    );
...
function filterDuplicates(
  ...
  return entries.filter((e) => {
    ...
    return (
      key.includes(needle) || kept.includes(needle) || dropped.includes(needle)
    );
  });
}
  ...
  try {
    ...
    if (ctx.source.files.length > 0) {
      for (const f of ctx.source.files) {
        console.log(
          `  - ${formatRelative(ctx.cwd, f.file)}${f.line ? `:${f.line}` : ""}`
        );
      }
    }
    console.log(
      tableLine(
        "Commands",
        ctx.source.commands.length ? ctx.source.commands.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "Properties",
        ctx.source.properties.length
          ? ctx.source.properties.join(", ")
          : "(none)"
      )
    );
    console.log(
      tableLine(
        "Emits/events",
        ctx.source.emits.length ? ctx.source.emits.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "Policies",
        ctx.source.policies.length ? ctx.source.policies.join(", ") : "(none)"
      )
    );

    if (
      ctx.source.parserFindings.length > 0 ||
      ctx.source.parserErrors.length > 0
    ) {
      console.log("");
      ...
      for (const err of ctx.source.parserErrors.slice(0, 10)) {
        const at = err.line
          ...
          : "";
        console.log(
          `  [${(err.severity || "error").toUpperCase()}] ${err.message}${at}`
        );
      }
      if (ctx.source.parserErrors.length > 10) {
        console.log(
          chalk.gray(
            `  ... ${ctx.source.parserErrors.length - 10} more parser diagnostics`
          )
        );
      }
    }
    ...
    for (const f of ctx.ir.files) {
      ...
      const compiledAt =
        typeof p.compiledAt === "string" ? p.compiledAt : "n/a";
      const compilerVersion =
        typeof p.compilerVersion === "string" ? p.compilerVersion : "n/a";
      console.log(
    ...
    console.log(
      tableLine(
        "Commands",
        ctx.ir.commands.length ? ctx.ir.commands.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "Properties",
        ctx.ir.properties.length ? ctx.ir.properties.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "Emits",
        ctx.ir.emits.length ? ctx.ir.emits.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "IR events",
        ctx.ir.events.length ? ctx.ir.events.join(", ") : "(none)"
      )
    );
    console.log(
      tableLine(
        "Policies",
        ctx.ir.policies.length ? ctx.ir.policies.join(", ") : "(none)"
      )
    );
    ...
    if (ctx.diff.hasDrift) {
      ...
      console.log(
        chalk.green("  No source-vs-IR drift detected for this entity.")
      );
    }
  } catch (error) {
    spinner.fail(
...
export async function diffSourceVsIRCommand(
  ...
  const spinner = createSpinner(
    `Diffing source vs IR for ${entityName}`,
    !options.json
  );
  try {
    const ctx = await buildEntityContext(entityName, options);
    ...
    if (options.json) {
      ...
      console.log("");
      console.log(
        tableLine("Source commands", ctx.source.commands.join(", ") || "(none)")
      );
      console.log(
        tableLine("IR commands", ctx.ir.commands.join(", ") || "(none)")
      );
      console.log(
        tableLine("Source props", ctx.source.properties.join(", ") || "(none)")
      );
      console.log(
        tableLine("IR props", ctx.ir.properties.join(", ") || "(none)")
      );
      console.log(
        tableLine("Source emits", ctx.source.emits.join(", ") || "(none)")
      );
  ...
  try {
    ...
    const payload = {
      ...
      summary: {
        ...
        knownDuplicates: entries.filter((e) => e.classification === "known")
          .length,
    ...
    for (const report of reports) {
      console.log(
        chalk.cyan(`  ${formatRelative(process.cwd(), report.file)}`)
      );
      ...
      for (const entry of filtered) {
        const color =
          entry.classification === "known" ? chalk.yellow : chalk.red;
        console.log(
          color(
            `    [${entry.classification.toUpperCase()}] ${entry.type} ${entry.key}`
          )
        );
    ...
    console.log(
      tableLine("Reports found", String(payload.summary.reportsFound))
    );
    console.log(tableLine("Entries", String(payload.summary.totalEntries)));
    console.log(
      tableLine("Known duplicates", String(payload.summary.knownDuplicates))
    );
    console.log(
      tableLine(
        "Suspicious duplicates",
        String(payload.summary.suspiciousDuplicates)
      )
    );
    ...
    spinner.fail(
      `duplicates failed: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
export async function runtimeCheckCommand(
  ...
  try {
    ...
    console.log(
      tableLine("Precompiled IR entity", ctx.ir.exists ? "yes" : "no")
    );
    console.log(
      tableLine("Precompiled IR command", irHasCommand ? "yes" : "no")
    );
    ...
    if (routeCheck.matches.length > 0) {
      for (const m of routeCheck.matches) {
        console.log(
          `  - ${m.method} ${m.routePath} (${formatRelative(ctx.cwd, m.manifestFile)})`
        );
      }
    }
    ...
    spinner.fail(
      `runtime-check failed: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
export async function cacheStatusCommand(
  ...
  const spinner = createSpinner(
    "Inspecting compiled IR timestamps",
    !options.json
  );
  try {
        ...
        return [
          {
            ...
            compilerVersion:
              typeof p.compilerVersion === "string" ? p.compilerVersion : null,
          },
        ];
      })
      .sort((a, b) =>
        String(b.compiledAt || "").localeCompare(String(a.compiledAt || ""))
      );
    ...
    console.log(
      chalk.yellow(
        "  Direct runtime cache introspection: unavailable (CLI is offline by design)"
      )
    );
    if (latest) {
      console.log(
        `  Latest compiled IR: ${formatRelative(process.cwd(), latest.file)}`
      );
    ...
    spinner.fail(
      `cache-status failed: ${error instanceof Error ? error.message : String(error)}`
    );
    process.exit(1);
  }
}
...
export async function doctorCommand(
  ...
  try {
    const cwd = process.cwd();
    const [sourceInspection, irInspection, duplicates, routeCheck] =
      await Promise.all([
        inspectSourceEntities({ cwd, srcPattern: options.src }),
        inspectCompiledIR({ cwd, irRoots: arrayify(options.irRoot) }),
    ...
    let sourceMerged: ReturnType<typeof mergeSourceEntityDefinitions> | null =
      null;
    let irMerged: ReturnType<typeof mergeIREntityDefinitions> | null = null;

    if (entityName) {
      sourceMerged = mergeSourceEntityDefinitions(
        sourceInspection.entities.get(entityName)
      );
      irMerged = mergeIREntityDefinitions(
        irInspection.entities.get(entityName)
      );
      entityDiff = diffEntitySurface({
        entityName,
        source: sourceMerged,
        ir: irMerged,
      });
    ...
    const filteredDuplicateEntries = filterDuplicates(
      duplicateEntries,
      entityName
    );
    ...
    console.log(
      tableLine(
        "Source manifests scanned",
        String(payload.summary.filesScanned.sourceManifests)
      )
    );
    console.log(
      tableLine(
        "Compiled IR files scanned",
        String(payload.summary.filesScanned.compiledIR)
      )
    );
    console.log(
      tableLine(
        "Merge reports found",
        String(payload.summary.filesScanned.mergeReports)
      )
    );
    console.log(
      tableLine(
        "Source parse-error files",
        String(payload.summary.parserErrorsInSourceFiles)
      )
    );
    if (payload.target) {
      console.log(
      ...
      if (payload.routeSurface)
        console.log(
          tableLine(
            "Route surface hit",
            payload.routeSurface.routeExists ? "yes" : "no"
          )
        );
    }
