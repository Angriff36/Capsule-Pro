...
program
  .name("manifest")
  .description(
    "Manifest language CLI - Compile, generate, and validate Manifest code"
  )
  .version("0.3.8");
...
program
  ...
  .option(
    "-g, --glob <pattern>",
    "Glob pattern for multiple files (use with output directory)"
  )
...
program
  ...
  .option(
    "-p, --projection <name>",
    "Projection name (nextjs, ts.types, ts.client)",
    "nextjs"
  )
  .option(
    "-s, --surface <name>",
    "Projection surface (route, command, types, client, all)",
    "all"
  )
  ...
  .action(async (ir, options = {}) => {
    ...
    const finalOptions = {
      ...
      auth:
        options.auth ||
        nextJsOptions.authImportPath ||
        nextJsOptions.authProvider ||
        "clerk",
      database:
        options.database ||
        nextJsOptions.databaseImportPath ||
        "@/lib/database",
      runtime:
        options.runtime ||
        nextJsOptions.runtimeImportPath ||
        "@/lib/manifest-runtime",
      response:
        options.response ||
        nextJsOptions.responseImportPath ||
...
program
  ...
  .option(
    "--schema <path>",
    "Schema path (default: docs/spec/ir/ir-v1.schema.json)"
  )
...
program
  .command("lint-routes")
  .description(
    "Scan for hardcoded route strings (canonical routes enforcement)"
  )
  .option("-f, --format <format>", "Output format (text, json)", "text")
...
async function isDirectExecution(): Promise<boolean> {
  ...
  if (argvEntry) {
    ...
    if (argvPath) {
      return (
        normalizeForComparison(modulePath) === normalizeForComparison(argvPath)
      );
    }
    ...
    if (
      normalizedArgv.includes("manifest") ||
      normalizedArgv.endsWith("index.js")
    ) {
      return true;
    }
