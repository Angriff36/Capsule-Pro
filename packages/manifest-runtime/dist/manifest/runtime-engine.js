...
export class RuntimeEngine {
    ...
    async _executeCommandInternal(commandName, input, options) {
        ...
        try {
            ...
            const policyResult = await this.checkPolicies(command, evalContext, options.entityName);
    ...
    async checkPolicies(command, evalContext, entityName) {
        ...
        if (entityName) {
            ...
            if (entity?.defaultPolicies && entity.defaultPolicies.length > 0) {
                ...
                const defaultPolicies = this.ir.policies.filter((p) => defaultPolicyNames.has(p.name));
