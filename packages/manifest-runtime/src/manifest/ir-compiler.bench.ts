 ...
 *
 * Benchmarks for the Manifest IR compiler to measure AST → IR transformation
...
// Large source (~2000 tokens) - multiple entities with vNext features
const largeSource = `
entity User {
  property required id: string = ""
  property required name: string = ""
  ...
  constraint idNotEmpty overrideable ok: self.id != "" "User ID cannot be empty"
  constraint nameNotEmpty: self.name != "" "Name cannot be empty"
  constraint emailNotEmpty: self.email != "" "Email cannot be empty"
  constraint emailValid: self.email contains "@" "Email must be valid"
  constraint roleValid: self.role in ["user", "admin", "moderator"] "Role must be valid"
...
entity Order {
  ...
  constraint customerIdNotEmpty overrideable warn: self.customerId != "" "Customer ID required"
  ...
  command create(id: string, customerId: string, amount: number) {
    constraint amount > 0 "Order amount must be positive"
    constraint customerId != "" "Customer must be specified"
