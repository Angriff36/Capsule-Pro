...
describe("Lexer", () => {
  describe("Keywords", () => {
    ...
    it("should tokenize all command-related keywords", () => {
      const source =
        "command on when then emit mutate compute guard publish persist";
      const tokens = new Lexer(source).tokenize();
    ...
    it("should tokenize all reserved words from KEYWORDS set", () => {
      ...
      for (const keyword of KEYWORDS) {
        ...
        const keywordToken = tokens.find(
          (t) => t.type === "KEYWORD" && t.value === keyword
        );
  ...
  describe("Complex Manifest Syntax", () => {
    it("should tokenize a simple entity declaration", () => {
      ...
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "entity")
      ).toBe(true);
      expect(
        tokens.some((t) => t.type === "IDENTIFIER" && t.value === "User")
      ).toBe(true);
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "property")
      ).toBe(true);
    });

    it("should tokenize a command with guards", () => {
      const source =
        'command updateName(id: string, name: string) when user.role == "admin"';
      const tokens = new Lexer(source).tokenize();

      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "command")
      ).toBe(true);
      expect(
        tokens.some((t) => t.type === "IDENTIFIER" && t.value === "updateName")
      ).toBe(true);
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "when")
      ).toBe(true);
    });
    ...
    it("should tokenize a constraint with severity", () => {
      ...
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "constraint")
      ).toBe(true);
      expect(tokens.some((t) => t.type === "KEYWORD" && t.value === "ok")).toBe(
        true
      );
    });
    ...
    it("should tokenize overrideable constraint", () => {
      ...
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "overrideable")
      ).toBe(true);
      expect(
        tokens.some((t) => t.type === "KEYWORD" && t.value === "block")
      ).toBe(true);
    });
  });
