...
export class Observable<T> {
  private readonly subs: Set<Subscriber<T>> = new Set();
  private _v: T;
  constructor(v: T) {
    this._v = v;
  }
...
export class EventEmitter<T extends Record<string, unknown>> {
  private readonly listeners: Map<keyof T, Set<(d: unknown) => void>> =
    new Map();
...
export class MemoryStore<T extends { id: string }> implements Store<T> {
  ...
  private readonly listeners: Set<(items: T[]) => void> = new Set();
