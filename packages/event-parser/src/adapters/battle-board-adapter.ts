 ...
 */
export function buildBattleBoardFromEvent(
  event: ParsedEvent,
  options: BattleBoardAdapterOptions = {}
  ...
  const battleBoard: BattleBoardData = {
 ...
 */
function convertStaffToBattleBoard(staffing: StaffShift[]): BattleBoardStaff[] {
 ...
 */
function convertTimelineToBattleBoard(
  timeline: EventTimelineEntry[],
  event: ParsedEvent
): BattleBoardTimeline[] {
  const result: BattleBoardTimeline[] = [];
 ...
 */
function buildDefaultLayouts(event: ParsedEvent): BattleBoardLayout[] {
  const layouts: BattleBoardLayout[] = [];
...
function mapPhaseToStyle(phase: TimelinePhase): TimelineStyle {
  switch (phase) {
    case "setup":
      return "setup";
    case "service":
...
function inferTeamFromPhase(phase: TimelinePhase): string {
  switch (phase) {
    case "setup":
      return "Ops Team";
    case "service":
...
function inferLocationFromEntry(entry: EventTimelineEntry): string {
...
function isHighlightEntry(entry: EventTimelineEntry): boolean {
  const label = entry.label.toLowerCase();
  const highlights = [
    "guest arrival",
    "service start",
 ...
 */
export function mergeStaffIntoBattleBoard(
  battleBoard: BattleBoardData,
  staffing: StaffShift[]
): BattleBoardData {
 ...
 */
export function createEmptyBattleBoard(
  eventName?: string,
  eventDate?: string
): BattleBoardData {
  return {
