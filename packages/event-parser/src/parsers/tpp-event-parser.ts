...
const INSTRUCTION_PREFIXES = [
  'MAKE', 'PORTION', 'ADD', 'PULL', 'MARINATE', 'SLICE', 'DICE', 'GRILL',
  'COOK', 'HEAT', 'WRAP', 'SEND', 'KEEP', 'PLACE', 'PACK', 'BUILD', 'TOSS',
  'PROOF', 'BAKE', 'CUT', 'REST', 'WARM', 'SET', 'PREP', 'LABEL', 'STORE',
  'COMBINE', 'ASSEMBLE', 'GARNISH', 'PLATE', 'HOLD', 'CHECK', 'PICK',
  'DELIVER', 'FINISH', 'REMOVE', 'CLEAN', 'BLEND', 'CHOP', 'WHISK',
  'POACH', 'BOIL', 'SAUTE', 'SIMMER',
];

const ACRONYM_KEEP = new Set([
  'BBQ', 'GF', 'DF', 'V', 'VG', 'VEG', 'DIY', 'VIP', 'AI', 'BLT', 'P',
  'GF/V', 'GF/VG', 'GF/DF', 'MTO',
]);
...
function selectPrimaryQuantity(
  details: MenuQuantityDetail[]
...
function deriveCategoryMetadata(category: string): {
  ...
  if (lower.includes('chef') || lower.includes('station')) badges.add('Chef Station');
  ...
  return { label, path: segments, group, serviceLocation, badges: [...badges], sortOrder };
}
...
function calculateConfidence(event: ParsedEvent, warnings: string[]): number {
  let score = 100;

  // Deduct for missing key data
  if (!event.client || event.client === 'Unknown Client') score -= 15;
  if (!event.date) score -= 15;
  if (!event.venue.name || event.venue.name === 'Unspecified Venue') score -= 10;
