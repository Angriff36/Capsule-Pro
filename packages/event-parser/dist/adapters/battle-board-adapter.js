...
export function buildBattleBoardFromEvent(event, options = {}) {
    ...
    if (timeline.length > 0) {
        ...
        warnings.push("No timeline data found. Timeline section will need manual entry.");
    }
    // Build layouts
    totalFields++;
...
function buildDefaultLayouts(event) {
    ...
    const hasKitchenService = event.menuSections?.some((item) => item.serviceLocation === "finish_at_kitchen" ||
    ...
    const hasBar = event.serviceStyle?.toLowerCase().includes("bar") ||
        event.menuSections?.some((item) => item.category.toLowerCase().includes("beverage") ||
...
function formatTo12Hour(time24) {
    ...
    if (time24.toLowerCase().includes("am") ||
...
export function mergeStaffIntoBattleBoard(battleBoard, staffing) {
    ...
    const existingNames = new Set(battleBoard.staff.map((s) => s.name.toLowerCase()));
    const newStaff = convertedStaff.filter((s) => !existingNames.has(s.name.toLowerCase()));
    return {
        ...battleBoard,
