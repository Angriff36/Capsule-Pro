...
export async function processDocument(fileContent, options) {
    ...
    if (fileType === "csv") {
        ...
        const csvContent = typeof fileContent === "string"
            ? fileContent
            : new TextDecoder().decode(fileContent instanceof ArrayBuffer
    ...
    if (fileType === "pdf") {
        ...
        warnings.push("PDF format not recognized as TPP. Manual data entry may be required.");
        return {
            id,
            fileName,
            fileType: "pdf",
...
export async function processMultipleDocuments(files) {
    ...
    const eventDoc = documents.find((doc) => doc.detectedFormat === "tpp" && doc.parsedEvent);
        ...
        if (doc.staffShifts) {
            ...
            if (mergedEvent) {
                ...
                for (const [eventName, shifts] of doc.staffShifts) {
                    const matches = eventNameVariations.some((variation) => eventName.toLowerCase().includes(variation.toLowerCase()) ||
