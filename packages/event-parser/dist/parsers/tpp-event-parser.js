...
export function parseTppEvent(rawLines, options) {
    ...
    const invoiceNumber = (sectionMap.get("invoice #:") ?? [])[0] ??
    ...
    const warnings = collectWarnings(event, { invoiceNumber, venueAddress, startTime, endTime }, diagnostics.warnings);
...
function isFooterLine(line) {
    ...
    return (normalized.startsWith("printed date") ||
        normalized === "page" ||
        normalized === "of" ||
...
function deriveServiceStyle(experience, menuItems) {
    ...
    if ([...categories].some((c) => c.includes("finish at event")) &&
...
function deriveAllergens(menuItems, notes) {
    ...
    const sources = [
        ...menuItems.flatMap((item) => item.specials.concat(item.preparationNotes ? [item.preparationNotes] : [])),
        ...notes,
    ];
...
function deriveKits(menuItems, lines) {
    ...
    const candidates = [
        ...
        ...menuItems.flatMap((item) => item.preparationNotes ? item.preparationNotes.split("\n") : []),
        ...lines,
    ];
...
function parseTimelineLine(line) {
    ...
    const rangeRegex = /(\d{1,2}(?::\d{2})?)\s*(a\.m\.|p\.m\.|am|pm)?\s*(?:-|\u2013|\u2014|to|through|until)\s*(\d{1,2}(?::\d{2})?)\s*(a\.m\.|p\.m\.|am|pm)?/i;
...
function categorizeTimelinePhase(text) {
    ...
    if (/(setup|set[-\s]?up|arrival|arrive|load\s?in|prep|staging|deliver|drop[-\s]?off|crew call|call time)/.test(value)) {
        return "setup";
    }
    if (/(ceremony|service|serve|dinner|lunch|meal|buffet|reception|cocktail|program|toast|bar|course|plated|dessert|hour)/.test(value)) {
        return "service";
    }
    if (/(teardown|tear[-\s]?down|load\s?out|strike|clean|cleanup|wrap|depart|end|breakdown|reset)/.test(value)) {
        return "teardown";
    }
    return "other";
}
function deriveTimes(timelineLines, parsedEntries) {
    ...
    if (parsedEntries && parsedEntries.length > 0) {
        for (const entry of parsedEntries) {
            ...
            if (typeof entry.endMinutes === "number" &&
    ...
    if (minutes.length === 0) {
        for (const line of timelineLines) {
            const rangeMatch = line.match(/(\d{1,2}(?::\d{2})?)(?:\s*(a\.m\.|p\.m\.|am|pm)?)\s*(?:-|\u2013|\u2014)\s*(\d{1,2}(?::\d{2})?)\s*(a\.m\.|p\.m\.|am|pm)?/i);
            if (rangeMatch) {
