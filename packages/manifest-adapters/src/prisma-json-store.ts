 ...
 */
export class PrismaJsonStore implements Store<EntityInstance> {
  ...
  async getById(id: string): Promise<EntityInstance | undefined> {
    try {
      ...
      console.error(
        `[PrismaJsonStore] getById("${id}") failed for entityType="${this.entityType}", tenant="${this.tenantId}":`,
        error
      );
      throw error;
    }
  ...
  async create(data: Partial<EntityInstance>): Promise<EntityInstance> {
    try {
      ...
      if (!id) {
        throw new Error(
          `[PrismaJsonStore] create() requires data.id for entityType="${this.entityType}"`
        );
      }
      ...
      console.error(
        `[PrismaJsonStore] create() failed for entityType="${this.entityType}", tenant="${this.tenantId}":`,
        error
      );
      throw error;
    }
  ...
  async update(
    ...
    try {
      ...
      console.error(
        `[PrismaJsonStore] update("${id}") failed for entityType="${this.entityType}", tenant="${this.tenantId}":`,
        error
      );
      throw error;
    }
  ...
  async delete(id: string): Promise<boolean> {
    try {
      ...
      console.error(
        `[PrismaJsonStore] delete("${id}") failed for entityType="${this.entityType}", tenant="${this.tenantId}":`,
        error
      );
      return false;
    }
  ...
  async clear(): Promise<void> {
    try {
      ...
      console.error(
        `[PrismaJsonStore] clear() failed for entityType="${this.entityType}", tenant="${this.tenantId}":`,
        error
      );
      throw error;
    }
 ...
 */
export function createPrismaJsonStoreProvider(
  prisma: PrismaClient,
  tenantId: string
): (entityName: string) => PrismaJsonStore {
  return (entityName: string) =>
    new PrismaJsonStore({ prisma, tenantId, entityType: entityName });
}
