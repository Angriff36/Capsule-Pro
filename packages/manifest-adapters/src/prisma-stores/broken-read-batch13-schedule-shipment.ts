  ...
  async create(data: Partial<EntityInstance>): Promise<EntityInstance> {
    ...
    const row = await this.prisma.scheduleShift.create({
      data: {
        ...
        shift_start:
          asNullableDate(data.shift_start ?? data.shiftStart) ?? new Date(),
        shift_end:
          asNullableDate(data.shift_end ?? data.shiftEnd) ?? new Date(),
        role_during_shift: asNullableString(
          data.role_during_shift ?? data.roleDuringShift
        ),
