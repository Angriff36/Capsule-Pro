 ...
 */
let unitCache: Map<number, Unit> | null = null;
...
async function loadConversions(
  ...
  const rows = await db.$queryRaw<UnitConversion[]>(
    Prisma.sql`
 ...
 */
export function clearUnitCaches(): void {
  unitCache = null;
...
export async function convertQuantity(
  ...
  if (fromUnit.unit_type === toUnit.unit_type) {
    ...
    const baseUnit = Array.from(units.values()).find(
      (u) => u.unit_type === fromUnit.unit_type && u.is_base_unit
    );
  ...
  throw new Error(
    `Cannot convert from ${fromUnit.code} (${fromUnit.unit_type}) to ${toUnit.code} (${toUnit.unit_type})`
  );
}
 ...
 */
export async function convertUnitToSystem(
  db: PrismaClient | Prisma.TransactionClient,
  quantity: number,
  fromUnitId: number,
  targetSystem: "metric" | "imperial"
): Promise<{ quantity: number; unitId: number }> {
  const units = await loadUnits(db);
  const fromUnit = units.get(fromUnitId);
  ...
  const targetUnit = Array.from(units.values()).find(
    (u) => u.unit_system === targetSystem && u.unit_type === fromUnit.unit_type
  );

  if (!targetUnit) {
    throw new Error(
      `No ${targetSystem} unit available for type ${fromUnit.unit_type}`
    );
  }

  const convertedQuantity = await convertQuantity(
    db,
    quantity,
    fromUnitId,
    targetUnit.id
 ...
 */
export async function getBestUnit(
  db: PrismaClient | Prisma.TransactionClient,
  quantity: number,
  unitType: "volume" | "weight" | "count" | "length",
  preferredSystem: "metric" | "imperial" = "metric"
): Promise<{ quantity: number; unitId: number; unit: Unit }> {
  const units = await loadUnits(db);
  const typeUnits = Array.from(units.values()).filter(
    (u) => u.unit_type === unitType && u.unit_system === preferredSystem
  );
  ...
  for (const unit of sortedUnits.reverse()) {
    if (unit.is_base_unit) continue;

    try {
      const converted = await convertQuantity(
        db,
        quantity,
        sortedUnits[0]?.id || unit.id,
        unit.id
      );
      if (converted >= 1) {
...
export async function scaleRecipe(
  ...
  if (targetYieldUnitId && targetYieldUnitId !== recipe.yield_unit_id) {
    ...
    const targetYieldInRecipeUnit = await convertQuantity(
      db,
      targetYieldQuantity,
      targetYieldUnitId,
  ...
  for (const ing of ingredients) {
    ...
    if (convertToSystem) {
      try {
        const converted = await convertUnitToSystem(
          db,
          scaledQuantity,
          ing.unit_id,
          convertToSystem
      ...
      try {
        ...
        if (originalUnit) {
          const preferredSystem =
            convertToSystem && originalUnit.unit_system !== "custom"
              ? convertToSystem
              : originalUnit.unit_system === "custom"
                ? "metric"
                : originalUnit.unit_system;
          const bestUnit = await getBestUnit(
            db,
            finalScaledQuantity,
            originalUnit.unit_type as "volume" | "weight",
            preferredSystem
          );
 ...
 */
export async function generatePrepListFromScaledRecipe(
  db: PrismaClient | Prisma.TransactionClient,
  tenantId: string,
...
> {
  ...
  const defaultStationMapping = new Map<string, string>([
    ["hot", "hot-line"],
    ["grill", "hot-line"],
    ["sauté", "hot-line"],
    ["cold", "cold-prep"],
 ...
 */
export async function batchScaleRecipes(
  db: PrismaClient | Prisma.TransactionClient,
  tenantId: string,
  recipes: Array<{
    recipeVersionId: string;
  ...
  for (const recipe of recipes) {
    try {
      const scaled = await scaleRecipe(
        ...
        recipe.targetYieldQuantity,
        recipe.targetYieldUnitId,
        recipe.convertToSystem
      );
      results.push(scaled);
    } catch (error) {
