 ...
 */
function calculateCaloriesFromFat(fatGrams) {
    return Math.round(fatGrams * 9);
}
/**
 * Calculate percent daily value
 */
function calculatePercentDV(amount, dailyValue, isProtein = false) {
    if (isProtein) {
 ...
 */
function getUnitToGramsFactor(unitId) {
 ...
 */
async function fetchRecipeIngredientNutrition(db, tenantId, recipeVersionId) {
 ...
 */
function calculateDailyValues(nutrients) {
    return {
        totalFat: calculatePercentDV(nutrients.totalFat, DAILY_VALUES.totalFat),
        saturatedFat: calculatePercentDV(nutrients.saturatedFat, DAILY_VALUES.saturatedFat),
        transFat: 0, // No DV for trans fat
        cholesterol: calculatePercentDV(nutrients.cholesterol, DAILY_VALUES.cholesterol),
        ...
        totalCarbohydrate: calculatePercentDV(nutrients.totalCarbohydrate, DAILY_VALUES.totalCarbohydrate),
        dietaryFiber: calculatePercentDV(nutrients.dietaryFiber, DAILY_VALUES.dietaryFiber),
        totalSugars: 0, // No DV for total sugars
        addedSugars: calculatePercentDV(nutrients.addedSugars, DAILY_VALUES.addedSugars),
...
function analyzeAllergens(ingredients) {
    ...
    for (const ing of ingredients) {
        for (const allergen of ing.allergens) {
            ...
            const majorAllergen = MAJOR_ALLERGENS.find((ma) => allergen.toLowerCase().includes(ma.toLowerCase()));
            if (majorAllergen) {
 ...
 */
function validateFDACompliance(nutrients, servingSize, servingsPerContainer) {
    const warnings = [];
 ...
 */
export async function generateNutritionLabel(db, tenantId, recipeVersionId, options = {}) {
    ...
    const ingredients = await fetchRecipeIngredientNutrition(db, tenantId, recipeVersionId);
    ...
    const servingsPerContainer = options.servingsPerContainer || Math.max(1, yieldQuantity);
    // Calculate per-serving values
    const perServing = {
        calories: Math.round(totalNutrition.calories / servingsPerContainer),
        caloriesFromFat: Math.round(totalNutrition.caloriesFromFat / servingsPerContainer),
        totalFat: Math.round((totalNutrition.totalFat / servingsPerContainer) * 10) / 10,
        saturatedFat: Math.round((totalNutrition.saturatedFat / servingsPerContainer) * 10) /
            10,
        transFat: Math.round((totalNutrition.transFat / servingsPerContainer) * 10) / 10,
        ...
        totalCarbohydrate: Math.round((totalNutrition.totalCarbohydrate / servingsPerContainer) * 10) / 10,
        dietaryFiber: Math.round((totalNutrition.dietaryFiber / servingsPerContainer) * 10) /
            10,
        totalSugars: Math.round((totalNutrition.totalSugars / servingsPerContainer) * 10) / 10,
        addedSugars: Math.round((totalNutrition.addedSugars / servingsPerContainer) * 10) / 10,
        protein: Math.round((totalNutrition.protein / servingsPerContainer) * 10) / 10,
    ...
    const servingSize = options.servingSize ||
        `${Math.round((yieldQuantity * 100) / servingsPerContainer)}g`;
    // Validate FDA compliance
    const fdaCompliance = validateFDACompliance(perServing, servingSize, servingsPerContainer);
 ...
 */
export async function batchGenerateNutritionLabels(db, tenantId, recipeVersionIds, options) {
    const results = [];
    for (const recipeVersionId of recipeVersionIds) {
        try {
            const label = await generateNutritionLabel(db, tenantId, recipeVersionId, options);
            results.push(label);
        }
        catch (error) {
            console.error(`Failed to generate nutrition label for recipe ${recipeVersionId}:`, error);
        }
    }
    return results;
}
 ...
 */
export async function getAllergenSummary(db, tenantId, recipeVersionId) {
    const ingredients = await fetchRecipeIngredientNutrition(db, tenantId, recipeVersionId);
