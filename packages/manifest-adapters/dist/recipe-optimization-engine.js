 ...
 */
async function findSubstitutions(db, tenantId, ingredientId, currentCost, category) {
    const substitutions = [];
    // Get original ingredient details
    const original = await db.$queryRaw(Prisma.sql `
    ...
    const alternatives = await db.$queryRaw(Prisma.sql `
      SELECT
        i.id,
        i.name,
        i.category,
    ...
    const cheaperAlternatives = alternatives.filter((alt) => Number(alt.unit_cost) < currentCost * 0.9);
    for (const alt of cheaperAlternatives) {
        ...
        const allergenChanges = alt.allergens.filter((a) => !original[0].allergens.includes(a));
 ...
 */
export async function optimizeRecipe(db, tenantId, recipeVersionId, targetYieldQuantity) {
    ...
    const inStockCount = ingredients.filter((ing) => ing.ingredient_cost > 0).length;
    const availabilityScore = ingredients.length > 0 ? (inStockCount / ingredients.length) * 100 : 100;
    ...
    for (const ing of ingredients) {
        ...
        if (ingCost <= 0)
            continue; // Skip ingredients without cost data
        const substitutions = await findSubstitutions(db, tenantId, ing.ingredient_id, ingCost, ing.category);
        if (substitutions.length > 0) {
            ...
            costOptimizations.push({
                ...
                priority: savings > currentCost * 0.05
                    ? "high"
                ...
                risks: bestSubstitution.qualityImpact === "negative"
                    ? [
    ...
    const highWasteIngredients = ingredients.filter((ing) => Number(ing.waste_factor) > 1.15);
    if (highWasteIngredients.length > 0) {
        const wasteSavings = highWasteIngredients.reduce((sum, ing) => {
            const excessWaste = (Number(ing.waste_factor) - 1.1) * Number(ing.ingredient_cost);
        ...
        if (wasteSavings > 0) {
            ...
            costOptimizations.push({
                ...
                substitutions: [],
                implementation: "Review prep methods and purchasing specifications to reduce waste",
    ...
    const nutritionalAnalysis = await calculateNutritionalAnalysis(db, tenantId, recipeVersionId, Number(recipe.yield_quantity));
    ...
    const overallScore = availabilityScore * 0.3 +
        qualityScore * 0.4 +
        nutritionalAnalysis.healthScore * 0.3;
    const optimizedCost = Math.max(0, currentCost - totalPotentialSavings);
    const optimizedCostPerYield = currentCostPerYield > 0 ? optimizedCost / Number(recipe.yield_quantity) : 0;
    return {
        ...
        totalPotentialSavingsPercentage: currentCost > 0 ? (totalPotentialSavings / currentCost) * 100 : 0,
        costOptimizations,
        nutritionalAnalysis,
        availabilityScore,
        qualityScore,
 ...
 */
async function calculateNutritionalAnalysis(db, tenantId, recipeVersionId, yieldQuantity) {
    ...
    if (perServing.sodium < 600)
        nutrientHighlights.push("Low sodium");
    if (perServing.calories < 400)
        nutrientHighlights.push("Low calorie");
 ...
 */
export async function batchOptimizeRecipes(db, tenantId, recipeVersionIds) {
    const results = [];
