...
export async function generateAiSuggestionsBatch(bottlenecks, context) {
    ...
    try {
        ...
        log.error("[bottleneck-ai-suggestions] Failed to generate AI suggestions batch", {
            error: error instanceof Error ? error.message : "Unknown error",
        });
        return [];
    }
...
// Prompt Builders
// ============================================================================
function buildAiPrompt(bottleneck, context) {
...
function buildBatchAiPrompt(bottlenecks, context) {
    const bottleneckList = bottlenecks
        .map((b, i) => `${i + 1}. [${b.severity.toUpperCase()}] ${b.title}
   - Category: ${b.category}
   - Type: ${b.type}
 ...
 */
export function prioritizeSuggestions(suggestions, maxCount = 5) {
    const priorityScore = {
        urgent: 100,
        high: 75,
        medium: 50,
