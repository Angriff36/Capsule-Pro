...
// ---------------------------------------------------------------------------
export class EventProfitabilityPrismaStore {
    prisma;
    tenantId;
    ...
    async create(data) {
        ...
        const row = await this.prisma.eventProfitability.create({
            data: {
                ...
                budgetedGrossMarginPct: toDecimalRequired(data.budgetedGrossMarginPct, 0),
    ...
    async update(id, data) {
        try {
            ...
            if (data.budgetedGrossMargin !== undefined)
                patch.budgetedGrossMargin = toDecimalRequired(data.budgetedGrossMargin, 0);
            if (data.budgetedGrossMarginPct !== undefined)
                patch.budgetedGrossMarginPct = toDecimalRequired(data.budgetedGrossMarginPct, 0);
            ...
            if (data.actualGrossMarginPct !== undefined)
                patch.actualGrossMarginPct = toDecimalRequired(data.actualGrossMarginPct, 0);
...
// ---------------------------------------------------------------------------
export class EventReportPrismaStore {
    prisma;
    tenantId;
    constructor(prisma, tenantId) {
        this.prisma = prisma;
