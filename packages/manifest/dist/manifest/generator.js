export class CodeGenerator {
    ...
    emitRuntime() {
        ...
        this.in();
        this.line('private listeners: Map<keyof T, Set<(d: any) => void>> = new Map();');
    ...
    genOutboxEvent(ev) {
        const payloadType = 'payload' in ev.payload && Array.isArray(ev.payload.fields)
    ...
    genExpose(x, _program) {
        if (x.protocol === 'rest') {
            ...
            if (ops.includes('list'))
                this.line(`async list(q?: Record<string, any>) { return ${x.entity}Store.getAll(); },`);
            if (ops.includes('get'))
    ...
    emitExports(p) {
        ...
        for (const x of p.exposures)
            exports.push(x.protocol === 'rest' ? `${x.name}API` : `create${x.entity}`);
    ...
    genExpr(e) {
        switch (e.type) {
            ...
            case 'UnaryOp': return `${e.operator === 'not' ? '!' : e.operator}${this.genExpr(e.operand)}`;
            case 'Call': return `${this.genExpr(e.callee)}(${e.arguments.map((a) => this.genExpr(a)).join(', ')})`;
            case 'MemberAccess': return `${this.genExpr(e.object)}.${e.property}`;
            case 'Conditional': return `(${this.genExpr(e.condition)} ? ${this.genExpr(e.consequent)} : ${this.genExpr(e.alternate)})`;
            case 'Array': return `[${e.elements.map((x) => this.genExpr(x)).join(', ')}]`;
            case 'Object': return `{ ${e.properties.map((p) => `${p.key}: ${this.genExpr(p.value)}`).join(', ')} }`;
            case 'Lambda': return `(${e.parameters.join(', ')}) => ${this.genExpr(e.body)}`;
            default: return '/* ? */';
        }
    }
    tsType(t) {
        const m = { string: 'string', number: 'number', boolean: 'boolean', any: 'any', void: 'void', list: 'Array', map: 'Map' };
