export class CodeGenerator {
  ...
  genOutboxEvent(ev) {
    const payloadType =
      "payload" in ev.payload && Array.isArray(ev.payload.fields)
        ? `{ ${ev.payload.fields.map((f) => `${f.name}: ${this.tsType(f.dataType)}`).join("; ")} }`
        : this.tsType(ev.payload);
