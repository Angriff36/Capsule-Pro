...
export class RuntimeEngine {
  ...
  async createInstance(entityName, data) {
    ...
    const constraintFailures = await this.validateConstraints(
      entity,
      mergedData
    );
    if (constraintFailures.length > 0) {
      ...
      console.warn(
        "[Manifest Runtime] Constraint validation failed:",
        constraintFailures
      );
      return undefined;
    }
    ...
    for (const constraint of entity.constraints) {
      ...
      if (shouldFire) {
        failures.push({
          ...
          message:
            constraint.message || `Constraint '${constraint.name}' failed`,
          resolved: await this.resolveExpressionValues(
            constraint.expression,
            evalContext
          ),
        });
   ...
   */
  async evaluateCommandConstraints(
    command,
    evalContext,
    overrideRequests,
    entityName
   ...
   */
  async emitOverrideAppliedEvent(
    constraint,
    overrideReq,
    outcome,
    commandName,
   ...
   */
  async emitConcurrencyConflictEvent(
    entityName,
    entityId,
    expectedVersion,
    actualVersion
