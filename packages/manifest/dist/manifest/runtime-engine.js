...
export class RuntimeEngine {
    ...
    async assertValidProvenance() {
        if (this.options.requireValidProvenance) {
            ...
            console.log("[Manifest Runtime] IR provenance verification is disabled. " +
     ...
     */
    async getInstanceByKey(entityName, id) {
    ...
    async createInstance(entityName, data) {
        ...
        if (entity.computedProperties.length > 0) {
            for (const computed of entity.computedProperties) {
                const computedValue = await this.evaluateComputedInternal(entity, instance, computed.name, new Set());
                if (computedValue !== undefined) {
                    instance[computed.name] = computedValue;
                }
            }
        }
        return instance;
    ...
    async validateConstraints(entity, instanceData) {
        ...
        // Use evaluateConstraint to build proper ConstraintOutcome objects with severity
