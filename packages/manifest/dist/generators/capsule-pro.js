...
export function generateCapsuleProRouteHandler(ir, options) {
    ...
    lines.push("//");
    lines.push("// Escape hatch: Create a manual route in a sibling directory and import");
...
function generateRouteHandlerMethod(entityName, operation) {
    ...
    lines.push("  if (!orgId) {");
    lines.push('    return manifestErrorResponse(new Error("Unauthorized"), 401);');
    lines.push("  }");
    lines.push("");
    ...
    switch (operation.operation) {
        ...
        default:
            ...
            lines.push('    return manifestErrorResponse(new Error("Not implemented"), 501);');
    }
 ...
 */
function generateGetOperation(entityName) {
    const lines = [];
    ...
    lines.push("    if (!item) {");
    lines.push(`      return manifestErrorResponse(new Error("Not found"), 404);`);
    lines.push("    }");
    lines.push(`    return manifestSuccessResponse({ ${camelCase(entityName)}: item });`);
    return lines.join("\n");
}
/**
...
function generateCreateOperation(entityName) {
    ...
    lines.push(`    const result = await runtime.createInstance("${entityName}", body);`);
    lines.push("    ");
    lines.push("    // Check for constraint failure (undefined return)");
    lines.push("    if (result === undefined) {");
    lines.push("      // Get constraint details");
    lines.push(`      const failures = await runtime.checkConstraints("${entityName}", body);`);
    lines.push("      ");
    lines.push("      // Return error response with constraint details");
    lines.push(`      return manifestErrorResponse(new Error("Constraint validation failed"), 400, {`);
...
function generateUpdateOperation(_entityName) {
    ...
    lines.push(`    const commandResult = await runtime.executeCommand(id, "update", {`);
    lines.push("      ...body,");
    lines.push("    });");
    lines.push("    ");
...
function generateCommandOperation(_entityName, operation) {
    ...
    lines.push(`    const result = await runtime.executeCommand(id, "${commandName}", body);`);
    lines.push("    ");
    lines.push("    // Check for blocking constraints");
