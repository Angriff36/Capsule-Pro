...
function generateAuthBody(options: NormalizedNextJsOptions): string {
  ...
  switch (authProvider) {
    case "clerk": {
      ...
      const guard = needsOrgId ? "!userId || !orgId" : "!userId";
...
export class NextJsProjection implements ProjectionTarget {
  ...
  private _command(
    ...
    // Commands may not have entity set (parser limitation) — find by name
    const command = ir.commands.find((c) => c.name === commandName);
    if (!command) {
      diagnostics.push({
        severity: "error",
        code: "COMMAND_NOT_FOUND",
        message: `Command "${commandName}" not found in IR. Available commands: ${ir.commands.map((c) => c.name).join(", ")}`,
        entity: entityName,
      });
      return { code: "", diagnostics };
    }
  ...
  private _generatePostCommandHandler(
    ...
    lines.push(
      "        return manifestErrorResponse(`Access denied: \x24{result.policyDenial.policyName}`, 403);"
    );
    lines.push("      }");
    lines.push("      if (result.guardFailure) {");
    lines.push(
      "        return manifestErrorResponse(`Guard \x24{result.guardFailure.index} failed: \x24{result.guardFailure.formatted}`, 422);"
    );
    lines.push("      }");
    lines.push(
