...
export class StandaloneGenerator {
  ...
  private genEntity(e: EntityNode) {
    ...
    this.in();
    this.line(
      "const obs = (this as Record<string, unknown>)[`_${String(prop)}`];"
    );
    this.line(
      'if (obs && typeof (obs as Observable<unknown>).subscribe === "function") {'
    );
    this.in();
  ...
  private genCommand(cmd: CommandNode) {
    ...
    this.line(
      `export async function ${cmd.name}(${params}): Promise<${returnType}> {`
    );
    this.in();
  ...
  private genOutboxEvent(ev: OutboxEventNode) {
    const payload = ev.payload as {
      fields?: { name: string; dataType: { name: string } }[];
    };
    ...
    this.line();
    this.line(
      `export const subscribe${ev.name} = (fn: (data: ${ev.name}Event) => void) => {`
    );
    this.in();
    this.line(
      `return EventBus.subscribe('${ev.channel}', fn as (d: unknown) => void);`
    );
    this.de();
    this.line("};");
  }
  ...
  private genExpr(e: ExpressionNode): string {
    switch (e.type) {
      ...
      case "Conditional": {
        const cond = this.genExpr(
          (e as { condition: ExpressionNode }).condition
        );
        const cons = this.genExpr(
          (e as { consequent: ExpressionNode }).consequent
        );
        const alt = this.genExpr(
          (e as { alternate: ExpressionNode }).alternate
        );
