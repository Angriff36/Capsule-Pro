...
function printUsage(): void {
  console.error(
    "Usage: manifest-generate <target> <surface> <manifest-file> [entity] [command] [options]"
  );
  console.error("");
  console.error("Arguments:");
  console.error('  target        Projection target name (e.g., "nextjs")');
  console.error(
    '  surface       Surface to generate (e.g., "nextjs.route", "nextjs.command", "ts.types")'
  );
  console.error("  manifest-file Path to .manifest source file");
  console.error(
    "  entity        Entity name (required for entity-scoped surfaces)"
  );
  console.error(
    "  command       Command name (required for nextjs.command surface)"
  );
  console.error("");
  ...
  console.error(
    "  --auth-provider <provider>   Auth provider: clerk, nextauth, custom, none"
  );
  console.error(
    "  --auth-import <path>         Import path for auth (default: @/lib/auth)"
  );
  console.error(
    "  --db-import <path>           Import path for database (default: @/lib/database)"
  );
  console.error(
    "  --runtime-import <path>      Import path for runtime factory (default: @/lib/manifest-runtime)"
  );
  console.error(
    "  --response-import <path>     Import path for response helpers (default: @/lib/manifest-response)"
  );
  console.error(
    "  --tenant-provider <fn>       Tenant lookup function name (e.g. getTenantIdForOrg)"
  );
  console.error(
    "  --tenant-import <path>       Import path for tenant lookup function"
  );
  console.error(
    "  --tenant-lookup-key <key>    Auth field to pass to tenant lookup: orgId or userId"
  );
  console.error("  --no-tenant-filter           Disable tenant filtering");
  console.error(
    "  --list                       List available projections and surfaces"
  );
...
function parseArgs(args: string[]): {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--help" || arg === "-h") {
      ...
      result.options.authProvider = args[
        ++i
      ] as NextJsProjectionOptions["authProvider"];
...
async function main(): Promise<number> {
    ...
    for (const diag of result.diagnostics) {
      const location =
        diag.line !== undefined
          ...
          : "";
      console.error(
        `  ${args.manifestPath}${location}: ${diag.severity} - ${diag.message}`
      );
    }
    return 1;
  }
  ...
  if (genResult.diagnostics.length > 0) {
    ...
    for (const diag of genResult.diagnostics) {
      ...
      if (diag.code === "MISSING_ENTITY" && !args.entity) {
        ...
        console.error(
          `    manifest-generate ${args.target} ${args.surface} ${args.manifestPath} <EntityName>`
        );
      }
    }
    if (hasErrors) {
  ...
  if (args.outputPath) {
    try {
      ...
      console.log(
        `Generated ${args.target} ${args.surface} -> ${args.outputPath}`
      );
    } catch (error) {
