 ...
 */
function getTaxByType(record: PayrollRecord, type: string): number {
 ...
 */
function sumDeductions(record: PayrollRecord, preTax: boolean): number {
...
function recordToRow(
  ...
  return [
    ...
    escapeCSV(formatDate(period.startDate, dateFormat)),
    escapeCSV(formatDate(period.endDate, dateFormat)),
  ];
}
...
export function exportSummaryToCSV(
  ...
  const lines = [
    ...
    [
      "Period Start",
      formatDate(period.startDate, options.dateFormat || "iso"),
    ].join(delimiter),
    [
      "Period End",
      formatDate(period.endDate, options.dateFormat || "iso"),
    ].join(delimiter),
