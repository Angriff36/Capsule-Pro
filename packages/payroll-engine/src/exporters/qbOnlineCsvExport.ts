 ...
 */
const DEFAULT_QBO_ACCOUNT_MAPPINGS: QBOnlineAccountMappings = {
 ...
 */
export function exportToQBOnlineCSV(
 ...
 */
export function exportToQBOnlineCSVAggregate(
  records: PayrollRecord[],
  period: PayrollPeriod,
  options: QBOnlineCsvExportOptions = {}
): string {
  ...
  const journalDate = formatDate(period.endDate, dateFormat);
  const memo = `Payroll ${formatDate(period.startDate, dateFormat)} to ${journalDate}`;
