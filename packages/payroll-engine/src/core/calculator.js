...
exports.verifyPayrollBalances = verifyPayrollBalances;
 ...
 */
function generatePayrollRecordId(periodId, employeeId) {
 ...
 */
function roundHours(hours, rule) {
  switch (rule) {
...
function calculateTipAllocation(
  ...
  let totalTips = currency_1.Currency.zero();
      ...
      case "by_hours":
        if (totalHoursAllEmployees > 0) {
          ...
          totalTips = totalTips.add(
            (0, currency_1.money)(pool.totalTips).multiply(share)
          );
        }
        break;
      case "by_headcount":
        if (employeeCount > 0) {
          totalTips = totalTips.add(
            (0, currency_1.money)(pool.totalTips).divide(employeeCount)
          );
        }
        break;
      case "fixed_shares":
        if (pool.fixedShares && pool.fixedShares[employeeId]) {
          ...
          totalTips = totalTips.add(
            (0, currency_1.money)(pool.totalTips).percentage(sharePercentage)
          );
        }
        break;
    }
...
function applyDeductions(
  ...
  const preTax = [];
  const postTax = [];
  let preTaxTotal = currency_1.Currency.zero();
  let postTaxTotal = currency_1.Currency.zero();
  ...
  for (const deduction of applicableDeductions) {
    ...
    if (deduction.percentage !== undefined && deduction.percentage > 0) {
      ...
      amount = (0, currency_1.money)(deduction.amount);
    } else {
...
function calculateEmployeePayroll(
  ...
  const regularRate = (0, currency_1.money)(role.baseRate);
  ...
  // 6. Calculate taxes
  const taxResult = (0, taxEngine_1.calculateTaxes)({
    grossPay,
  ...
  if (netPay.isNegative()) {
    ...
    if (availableForPostTax.isPositive()) {
      ...
      adjustedPostTax = postTax.map((d) => ({
        ...d,
        amount: (0, currency_1.money)(d.amount)
          .multiply(reductionRatio)
          .toNumber(),
      }));
      adjustedPostTaxTotal = availableForPostTax;
      finalNetPay = currency_1.Currency.zero();
    } else {
      // Can't cover all obligations - flag for review
      adjustedPostTax = [];
      adjustedPostTaxTotal = currency_1.Currency.zero();
...
function calculatePayroll(input) {
  ...
  summary.totalGrossPay = (0, currency_1.money)(
    summary.totalGrossPay
  ).toNumber();
  summary.totalNetPay = (0, currency_1.money)(summary.totalNetPay).toNumber();
  summary.totalTaxes = (0, currency_1.money)(summary.totalTaxes).toNumber();
  summary.totalDeductions = (0, currency_1.money)(
    summary.totalDeductions
  ).toNumber();
  summary.totalTips = (0, currency_1.money)(summary.totalTips).toNumber();
 ...
 */
function verifyPayrollBalances(records) {
  const discrepancies = [];
  for (const record of records) {
    const expectedNet = (0, currency_1.money)(record.grossPay)
