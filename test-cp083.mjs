...
async function main() {
  ...
  try {
    ...
    console.log("CRM1: Create client:", clientR.status, clientR.status === 200 ? "✓ PASS" : "✗ FAIL", JSON.stringify(clientR.data)?.substring(0, 100));
    const clientId = clientR.data?.result || clientR.data?.client?.id || clientR.data?.id;
    ...
    console.log("CRM1: Client list:", listClientsR.status, "count:", clientCount);
    ...
    console.log("CRM2: Create contact:", contactR.status, contactR.status === 200 ? "✓ PASS" : contactR.status === 404 ? "✗ 404 route" : "⚠ " + contactR.status, JSON.stringify(contactR.data)?.substring(0, 80));
    const contactId = contactR.data?.result || contactR.data?.contact?.id;

    // List contacts for this client
    if (clientId) {
      const listContactsR = await api(page, "GET", `/api/crm/clients/${clientId}/contacts`);
      console.log("CRM2: List contacts:", listContactsR.status, "count:", listContactsR.data?.contacts?.length || 0);
    }
    ...
    console.log("CRM3: Create deal:", dealR.status, dealR.status === 200 ? "✓ PASS" : dealR.status === 404 ? "✗ 404 route" : "⚠ " + dealR.status, JSON.stringify(dealR.data)?.substring(0, 80));
    const dealId = dealR.data?.result || dealR.data?.proposal?.id;

    // List deals
    const listDealsR = await api(page, "GET", "/api/crm/deals");
    console.log("CRM3: List deals:", listDealsR.status, listDealsR.status === 200 ? "✓ PASS" : "✗ " + listDealsR.status, "count:", listDealsR.data?.deals?.length || 0);
    ...
    const attachR = await api(page, "POST", `/api/crm/clients/${clientId}/events`, {
      eventId: EVENT_ID,
    });
    console.log("CRM4: Attach client to event:", attachR.status, attachR.status === 200 ? "✓ PASS" : attachR.status === 404 ? "✗ 404 route" : "⚠ " + attachR.status, JSON.stringify(attachR.data)?.substring(0, 80));
    ...
    console.log("CRM5: Duplicate email:", dupR.status, dupR.status === 409 ? "✓ 409 Conflict (merge flow)" : dupR.status === 422 ? "✓ 422 Validation error" : dupR.status === 200 ? "⚠ 200 Created duplicate (no merge)" : "⚠ " + dupR.status);
    ...
    if (contactId) {
      ...
      console.log("CRM5: Duplicate contact email:", dupContactR.status, dupContactR.status === 409 ? "✓ 409" : dupContactR.status === 422 ? "✓ 422" : dupContactR.status === 200 ? "⚠ Created" : "⚠ " + dupContactR.status);
    }

    console.log("\n" + "=".repeat(60));
    console.log("📊 cp-083 — CRM1-5: CRM module workflow — RESULT");
    console.log("=".repeat(60));
    console.log("CRM1 — Create customer:   " + (clientR.status === 200 ? "✓ PASS" : "✗ " + clientR.status));
    console.log("CRM2 — Create contact:   " + (contactR.status === 200 ? "✓ PASS" : contactR.status === 404 ? "✗ 404 route" : "⚠ " + contactR.status));
    console.log("CRM3 — Create deal:     " + (dealR.status === 200 ? "✓ PASS" : dealR.status === 404 ? "✗ 404 route" : "⚠ " + dealR.status));
    console.log("CRM4 — Attach to event: " + (attachR.status === 200 ? "✓ PASS" : attachR.status === 404 ? "✗ 404 route" : "⚠ " + attachR.status));
    console.log("CRM5 — Duplicate email:  " + (dupR.status === 409 ? "✓ PASS (409 conflict)" : dupR.status === 422 ? "✓ PASS (422 validation)" : "⚠ " + dupR.status));
    console.log("=".repeat(60));
