...
async function main() {
  ...
  const clerkCookies = allCookies.filter(
    (c) =>
      c.domain.includes("clerk") ||
      c.domain.includes("vercel") ||
  ...
  console.log(
    "\n__session cookie:",
    sessionCookie
      ? `${sessionCookie.value.substring(0, 40)}... (${sessionCookie.domain})`
      : "NOT FOUND"
  );
  ...
  page.on("response", (resp) => {
    if (resp.status() >= 400)
      responses.push(`${resp.status()} ${resp.url().substring(0, 100)}`);
  });
