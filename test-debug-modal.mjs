  ...
  try {
    ...
    for (const inp of await page.locator("input").all()) {
      try {
        ...
        const display = await page.evaluate(el => window.getComputedStyle(el).display, inp);
        if (name || id) console.log(`  [vis=${vis} display=${display}] name="${name}" id="${id}" type="${type}"`);
      } catch {}
    }

    // List all dialogs/modals
    for (const dlg of await page.locator('[role="dialog"], [aria-modal="true"]').all()) {
      try {
        const vis = await dlg.isVisible();
        const display = await page.evaluate(el => window.getComputedStyle(el).display, dlg);
        const cls = await dlg.getAttribute("class");
        console.log(`  Dialog[vis=${vis} display=${display}]: ${cls?.substring(0, 50)}`);
      } catch {}
    }
