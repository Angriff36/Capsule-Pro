 ...
 *   2. Adapter-derived? In `manifest-audit/schema-drift-allowlist.json` under
 ...
 *
 * Outputs:
 *   manifest-audit/schema-drift.json  — full structured report
 *   manifest-audit/schema-drift.md    — human-readable summary
 *
 * Usage:
...
const PATHS = {
  ...
  entitiesRegistry: path.join(ROOT, "manifest-registry/entities.json"),
  ...
  // duplicate-drop-allowlist.json. Outputs go to manifest-audit/ (gitignored).
  allowlist: path.join(ROOT, "scripts/manifest/schema-drift-allowlist.json"),
  outDir: path.join(ROOT, "manifest-audit"),
};
...
function checkEntity({
  ...
  for (const field of required) {
    ...
    violations.push({
      ...
      prismaType: field.prismaType,
      expectedManifestKind: expectedKind,
      recommendation: `Add \`${field.name}\` as a parameter to the manifest \`create\` command for ${entityName} (or as a property with a default), or document an adapter-derived rule in manifest-audit/schema-drift-allowlist.json under adapterDerived.${entityName}.${field.name}.`,
    });
  }
...
function renderMarkdown(summary, results) {
  ...
  lines.push("2. **Adapter-derived rule** — if the field is intentionally infrastructural (e.g. looked up from a parent entity), add an entry to `manifest-audit/schema-drift-allowlist.json` under `adapterDerived.<Entity>.<field>` with `{ \"rule\": \"...\", \"owner\": \"...\", \"reviewBy\": \"YYYY-MM-DD\" }`.");
...
function main() {
  ...
  console.log(
    "[schema-drift] Wrote manifest-audit/schema-drift.{json,md}"
  );
