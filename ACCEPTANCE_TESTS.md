...
| ------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- | ------- |
| AC-001 | specs/manifest-native/native-rewrite.md | WS0: zero `uuid … = ""` defaults remain in `manifest/source` (188 plan-time → **15 / 9 files** on 2026-07-12; IR-only sweep of nullable/param-seeded fields COMPLETE — incl. inventory `createdById`/`countedById` trusted-param port 2026-07-12. All 15 remaining are required DEFER cases needing schema/fork decisions) | `rg 'uuid.*= ""' manifest/source -g '*.manifest'` → 0 matches | PENDING |
