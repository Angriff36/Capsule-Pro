...
| ------ | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------- | ------- |
| AC-001 | specs/manifest-native/native-rewrite.md | WS0: zero `uuid … = ""` defaults remain in `manifest/source` (188 plan-time → **17 / 10 files** on 2026-07-11 — IR-only sweep COMPLETE; all 17 remaining are required-not-param-seeded DEFER cases needing schema/fork decisions) | `rg 'uuid.*= ""' manifest/source -g '*.manifest'` → 0 matches | PENDING |
