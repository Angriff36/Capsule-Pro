...
> **⚠ STAMP-INTEGRITY WARNING (added 2026-06-15) — the inline "▲ Dxx REMEDIATION — … COMPLETED 2026-06-14 (manifest@2.5.1)" blockquotes on D9–D12, D14, D16–D21, D25, and D26 are NOT trustworthy.** A ground-truth check on 2026-06-15 found their claimed deletions/changes still on disk on the current branch (`feat/openapi-docs-ci-mcp`): `manifest/scripts/ir-utils.mjs` (D11), `manifest/runtime/src/ir-contract.ts` (D14), `manifest/scripts/derive-prisma-options.mjs` (D16, and `prisma-options.config.json` does NOT exist), and the three regex scripts under D17 all still exist. The stamps also cite `manifest@2.5.1` while the installed version is `2.5.0`. Treat every remaining "COMPLETED" stamp here as **aspirational/unverified** until re-confirmed against the actual tree — do NOT skip the work on the strength of a stamp. **VERIFIED-DONE exceptions (real, re-confirmed against the tree): D15** (see that entry), and **D8 + D13** — both were falsely stamped on 2026-06-14 but were *actually* applied 2026-06-15 (outbox.ts deleted; the `"2.2.0"` provenance literal replaced) — see those entries.
...
> **▲ D8 REMEDIATION — the 2026-06-14 "COMPLETED" stamp was FALSE on this branch** (`outbox.ts` was still on disk). **GENUINELY COMPLETED 2026-06-15 (feat/openapi-docs-ci-mcp).** Deleted `apps/api/lib/manifest/outbox.ts` (both `ManifestOutboxEvent` interface and `writeManifestOutboxEvents` function) via `git rm`. Zero code importers confirmed by repo-wide grep (only this register, the `manifest/MANIFEST-DIVERGENCES.md` mirror, and the local untracked `tools/manifest-structure-audit.mjs` referenced it). Also removed the now-dangling `outbox` entry from that structure audit's `EXPECTED.apiManifestLib` required-files list so `pnpm check:manifest:structure` stays green (0 FAIL, 0 outbox refs in the regenerated report). `apps/api` had no importers so its typecheck is unaffected.
...
> **▲ D13 REMEDIATION — the 2026-06-14 "COMPLETED" stamp was FALSE on this branch** (the `"2.2.0"` literal was still at `loadManifests.ts:254`). **GENUINELY COMPLETED 2026-06-15 (feat/openapi-docs-ci-mcp).** Replaced the `compilerVersion: "2.2.0"` literal in `compileManifestSet` (`loadManifests.ts`) with `compiledIRs[0]?.provenance?.compilerVersion ?? "unknown"` — the same inherit-from-children pattern already used by `mergeIrDocuments`, so provenance no longer lies about which compiler produced the IR. Updated the two advisory strings in `audit-ir-drift.mjs` from `@angriff36/manifest@2.2.0+` to `2.5.0+`. Verified: `@repo/manifest-runtime` typecheck green; conformance-index + station-capacity tests (14) pass (the compile/load path still produces a loadable IR). No test asserted the old literal.
