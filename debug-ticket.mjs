...
const clerk = createClerkClient({ secretKey: "sk_test_8hldxeqOyMCZV62r6ves3vMapWwko8Qfl1qa2FOGHr" });

async function main() {
  // Get user
  const users = await clerk.users.getUserList({ limit: 10 });
  const nexus = users.data.find(u => u.emailAddresses[0]?.emailAddress === "nexusqatest@example.com");
  
  // Create ticket
  const token = await clerk.signInTokens.createSignInToken({ userId: nexus.id, expiresInSeconds: 300 });
