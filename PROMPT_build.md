...
Your job in THIS invocation is to perform EXACTLY ONE atomic fix. **JUST FOCUS
ON FIXING THE ANY TYPES IN ALL THE APPS AND PACKAGES DIRECTORIES** Rules
(mandatory):
...
5. After ONE issue is successfully fixed and verified, you MUST EXIT
   immediately.
6. Do NOT fix additional issues.
7. Do NOT continue improving, refactoring, or scanning for more problems. **JUST
   FOCUS ON FIXING THE ANY TYPES IN ALL THE APPS AND PACKAGES DIRECTORIES** Exit
   protocol:
