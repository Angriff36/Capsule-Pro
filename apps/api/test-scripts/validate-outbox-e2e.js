...
async function cleanup() {
  ...
  await database_1.database.outboxEvent.deleteMany({
    where: { tenantId: TEST_TENANT_ID },
  });
}
async function validateCreateOutboxEvent() {
  console.log("\n1. Testing createOutboxEvent helper...");
  const event = await (0, realtime_1.createOutboxEvent)(database_1.database, {
...
async function validateOutboxQuery() {
  ...
  const pendingEvents = await database_1.database.outboxEvent.findMany({
...
async function validateEnvelopeStructure(event) {
...
async function simulatePublish(eventId) {
  ...
  const published = await database_1.database.outboxEvent.update({
    where: { id: eventId },
    data: {
      status: "published",
...
async function validatePayloadSize(envelope) {
