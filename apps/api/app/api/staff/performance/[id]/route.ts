// Auto-generated Next.js API detail route for StaffPerformance
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const staffPerformance = await database.staffPerformance.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!staffPerformance) {
      return manifestErrorResponse({ error: "StaffPerformance not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ staffPerformance });
  } catch (error) {
    ...
    console.error("Error fetching staffPerformance:", error);
