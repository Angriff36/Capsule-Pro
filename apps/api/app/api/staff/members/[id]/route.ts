// Auto-generated Next.js API detail route for StaffMember
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const staffMember = await database.staffMember.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!staffMember) {
      return manifestErrorResponse({ error: "StaffMember not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ staffMember });
  } catch (error) {
    ...
    console.error("Error fetching staffMember:", error);
