// Auto-generated Next.js API detail route for OnboardingTask
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const onboardingTask = await database.onboardingTask.findFirst({
      where: {
        id,
        tenant_id: tenantId,
        deleted_at: null
    ...
    if (!onboardingTask) {
      return manifestErrorResponse({ error: "OnboardingTask not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ onboardingTask });
  } catch (error) {
    ...
    console.error("Error fetching onboardingTask:", error);
