// Auto-generated Next.js API detail route for OpenShift
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const openShift = await database.open_shifts.findFirst({
      where: {
        id,
        tenant_id: tenantId
      },
    ...
    if (!openShift) {
      return manifestErrorResponse({ error: "OpenShift not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ openShift });
  } catch (error) {
    ...
    console.error("Error fetching openShift:", error);
