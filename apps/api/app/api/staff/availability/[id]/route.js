...
async function GET(request, context) {
  ...
  return server_2.NextResponse.json({ availability: availability[0] });
}
/**
...
async function PATCH(request, context) {
  ...
  const { availability: existingAvail, error: verifyError } = await (0,
  validation_1.verifyAvailability)(tenantId, id);
  ...
  if (
    ...
    const existingRecord = await database_1.database.$queryRaw(database_1.Prisma
      .sql`
  ...
  try {
    ...
    const result = await database_1.database.$queryRaw(
      database_1.Prisma.raw(`UPDATE tenant_staff.employee_availability
    ...
    return server_2.NextResponse.json({ availability: result[0] });
  } catch (error) {
...
async function DELETE(request, context) {
  ...
  const { id } = await context.params;
  // Verify availability exists
  const { error: verifyError } = await (0, validation_1.verifyAvailability)(
    tenantId,
    id
  );
  if (verifyError) {
  ...
  try {
    ...
    return server_2.NextResponse.json({ deleted: result[0].id });
  } catch (error) {
