...
function validateShipmentItemData(item) {
  if (!item.item_id) {
...
async function GET(request, { params }) {
  try {
    ...
    const items = await database_1.database.shipmentItem.findMany({
      where: {
        tenantId,
        shipmentId: id,
        deletedAt: null,
    ...
    return server_2.NextResponse.json({ data: mappedItems });
  } catch (error) {
...
async function POST(request, { params }) {
  try {
    ...
    for (const item of items) {
      ...
      const created = await database_1.database.shipmentItem.create({
        data: {
          tenantId,
          shipmentId: id,
          itemId: item.item_id,
    ...
    return server_2.NextResponse.json({ data: createdItems }, { status: 201 });
  } catch (error) {
