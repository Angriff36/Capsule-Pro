...
async function GET(request, { params }) {
  try {
    ...
    return server_2.NextResponse.json(mappedShipment);
  } catch (error) {
...
async function PUT(request, { params }) {
  try {
    ...
    (0, validation_1.validateUpdateShipmentRequest)(body);
    ...
    if (
      ...
      const duplicate = await database_1.database.shipment.findFirst({
        where: {
          tenantId,
    ...
    `;
    const updated = await database_1.database.shipment.findFirst({
      where: { tenantId, id, deletedAt: null },
    });
    if (!updated) {
      return server_2.NextResponse.json(
