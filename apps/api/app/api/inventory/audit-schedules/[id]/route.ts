// Auto-generated Next.js API detail route for AuditSchedule
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const auditSchedule = await database.auditSchedule.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!auditSchedule) {
      return manifestErrorResponse({ error: "AuditSchedule not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ auditSchedule });
  } catch (error) {
    ...
    console.error("Error fetching auditSchedule:", error);
