// Auto-generated Next.js API detail route for InventoryTransferItem
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const inventoryTransferItem = await database.inventoryTransferItem.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!inventoryTransferItem) {
      return manifestErrorResponse({ error: "InventoryTransferItem not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ inventoryTransferItem });
  } catch (error) {
    ...
    console.error("Error fetching inventoryTransferItem:", error);
