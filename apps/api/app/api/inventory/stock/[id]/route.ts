// Auto-generated Next.js API detail route for InventoryStock
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const inventoryStock = await database.inventoryStock.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!inventoryStock) {
      return manifestErrorResponse({ error: "InventoryStock not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ inventoryStock });
  } catch (error) {
    ...
    console.error("Error fetching inventoryStock:", error);
