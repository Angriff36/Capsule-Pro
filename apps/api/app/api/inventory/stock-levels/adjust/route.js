...
async function POST(request) {
  try {
    ...
    (0, validation_1.validateCreateAdjustmentRequest)(body);
    const {
      inventoryItemId,
      storageLocationId,
      quantity,
    ...
    if (storageLocationId) {
      const location = await database_1.database.$queryRaw`
        SELECT id
    ...
    return server_2.NextResponse.json(response, { status: 201 });
  } catch (error) {
