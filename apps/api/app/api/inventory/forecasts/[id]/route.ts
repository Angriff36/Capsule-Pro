// Auto-generated Next.js API detail route for InventoryForecast
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const inventoryForecast = await database.inventoryForecast.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!inventoryForecast) {
      return manifestErrorResponse({ error: "InventoryForecast not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ inventoryForecast });
  } catch (error) {
    ...
    console.error("Error fetching inventoryForecast:", error);
