// Auto-generated Next.js API detail route for VendorContact
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const vendorContact = await database.vendorContact.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!vendorContact) {
      return manifestErrorResponse({ error: "VendorContact not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ vendorContact });
  } catch (error) {
    ...
    console.error("Error fetching vendorContact:", error);
