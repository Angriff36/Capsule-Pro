...
async function GET(request, context) {
  try {
    ...
    const item = await database_1.database.inventoryItem.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null,
    ...
    return server_2.NextResponse.json(itemWithStatus);
  } catch (error) {
...
async function PUT(request, context) {
  try {
    ...
    (0, validation_1.validateUpdateInventoryItemRequest)(body);
    // Check if updating to a duplicate item_number
    if (body.item_number && body.item_number !== existing.item_number) {
      const duplicate = await database_1.database.inventoryItem.findFirst({
        where: {
          tenantId,
    ...
    const updatedItem = await database_1.database.inventoryItem.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null,
    ...
    const itemWithStatus = {
      ...
      fsa_status: updatedItem.fsa_status ?? "unknown",
