// Auto-generated Next.js API detail route for InventoryTransfer
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const inventoryTransfer = await database.inventoryTransfer.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!inventoryTransfer) {
      return manifestErrorResponse({ error: "InventoryTransfer not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ inventoryTransfer });
  } catch (error) {
    ...
    console.error("Error fetching inventoryTransfer:", error);
