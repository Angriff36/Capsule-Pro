// Auto-generated Next.js API detail route for StorageLocation
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const storageLocation = await database.storage_locations.findFirst({
      where: {
        id,
        tenant_id: tenantId,
        deleted_at: null
    ...
    if (!storageLocation) {
      return manifestErrorResponse({ error: "StorageLocation not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ storageLocation });
  } catch (error) {
    ...
    console.error("Error fetching storageLocation:", error);
