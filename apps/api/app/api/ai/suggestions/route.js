...
async function getContextData(tenantId, timeframe = "week") {
  ...
  // Fetch events
  const upcomingEvents = await database_1.database.event.findMany({
    where: {
      tenantId,
      deletedAt: null,
  ...
  const dishes =
    dishIds.length > 0
      ? await database_1.database.dish.findMany({
          where: {
            tenantId,
            id: { in: dishIds },
            deletedAt: null,
  ...
  const prepTasks = await database_1.database.prepTask.findMany({
    where: {
      tenantId,
      deletedAt: null,
  ...
  const inventoryAlertsRaw = await database_1.database.inventoryAlert.findMany({
    where: {
      tenantId,
      resolved_at: null,
      deleted_at: null,
  ...
  const staffAssignments =
    await database_1.database.eventStaffAssignment.findMany({
      where: {
        tenantId,
        deletedAt: null,
      },
  ...
  const staffEvents =
    staffEventIds.length > 0
      ? await database_1.database.event.findMany({
          where: {
            id: { in: staffEventIds },
            eventDate: { gte: today, lte: endDate },
            deletedAt: null,
...
async function generateAISuggestions(tenantId, context, maxSuggestions = 5) {
  // Build system prompt
...
Generate ${maxSuggestions} prioritized suggestions based on this state.`;
  try {
    ...
    const suggestions = (aiResponse.suggestions || []).map((s, index) => {
      const config =
...
function generateFallbackSuggestions(tenantId, context, maxSuggestions) {
  const suggestions = [];
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  if (criticalAlerts.length > 0) {
    suggestions.push({
      ...
      category: "inventory",
      priority: "high",
      title: `${criticalAlerts.length} critical inventory alert${criticalAlerts.length > 1 ? "s" : ""}`,
  ...
  if (context.highVolumeDays.length > 0) {
    suggestions.push({
      ...
      category: "scheduling",
      priority: "medium",
      title: `${context.highVolumeDays.length} high-volume day${context.highVolumeDays.length > 1 ? "s" : ""} detected`,
