// Auto-generated Next.js API detail route for CommandBoard
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const commandBoard = await database.commandBoard.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!commandBoard) {
      return manifestErrorResponse({ error: "CommandBoard not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ commandBoard });
  } catch (error) {
    ...
    console.error("Error fetching commandBoard:", error);
