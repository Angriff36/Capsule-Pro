...
# Manifest reference for `apps/api/app/api/command-board/simulations`

**Why this file:** Board simulation apply/merge does atomic multi-entity writes — model as a native saga with compensation (or a single command spanning children), not raw createMany/updateMany in a $transaction.

**Relevant divergences:** D3 — see `manifest/MANIFEST-DIVERGENCES.md` for the full remediation detail.
