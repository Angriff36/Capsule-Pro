// Auto-generated Next.js API detail route for BoardProjection
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const boardProjection = await database.boardProjection.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!boardProjection) {
      return manifestErrorResponse({ error: "BoardProjection not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ boardProjection });
  } catch (error) {
    ...
    console.error("Error fetching boardProjection:", error);
