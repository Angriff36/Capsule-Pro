// Auto-generated Next.js API detail route for BoardAnnotation
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const boardAnnotation = await database.boardAnnotation.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!boardAnnotation) {
      return manifestErrorResponse({ error: "BoardAnnotation not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ boardAnnotation });
  } catch (error) {
    ...
    console.error("Error fetching boardAnnotation:", error);
