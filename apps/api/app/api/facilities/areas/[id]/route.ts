// Auto-generated Next.js API detail route for FacilityArea
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const facilityArea = await database.facilityArea.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!facilityArea) {
      return manifestErrorResponse({ error: "FacilityArea not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ facilityArea });
  } catch (error) {
    ...
    console.error("Error fetching facilityArea:", error);
