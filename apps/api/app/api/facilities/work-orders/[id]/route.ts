// Auto-generated Next.js API detail route for FacilityWorkOrder
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const facilityWorkOrder = await database.facilityWorkOrder.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!facilityWorkOrder) {
      return manifestErrorResponse({ error: "FacilityWorkOrder not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ facilityWorkOrder });
  } catch (error) {
    ...
    console.error("Error fetching facilityWorkOrder:", error);
