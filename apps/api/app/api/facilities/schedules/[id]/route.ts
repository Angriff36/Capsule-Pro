// Auto-generated Next.js API detail route for FacilitySchedule
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const facilitySchedule = await database.facilitySchedule.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!facilitySchedule) {
      return manifestErrorResponse({ error: "FacilitySchedule not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ facilitySchedule });
  } catch (error) {
    ...
    console.error("Error fetching facilitySchedule:", error);
