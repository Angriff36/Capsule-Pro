// Auto-generated Next.js API detail route for MaintenanceWorkOrder
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const maintenanceWorkOrder = await database.maintenanceWorkOrder.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!maintenanceWorkOrder) {
      return manifestErrorResponse({ error: "MaintenanceWorkOrder not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ maintenanceWorkOrder });
  } catch (error) {
    ...
    console.error("Error fetching maintenanceWorkOrder:", error);
