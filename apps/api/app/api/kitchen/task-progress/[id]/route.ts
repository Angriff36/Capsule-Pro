// Auto-generated Next.js API detail route for KitchenTaskProgress
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const kitchenTaskProgress = await database.kitchenTaskProgress.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!kitchenTaskProgress) {
      return manifestErrorResponse({ error: "KitchenTaskProgress not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ kitchenTaskProgress });
  } catch (error) {
    ...
    console.error("Error fetching kitchenTaskProgress:", error);
