  ...
  try {
    ...
    const result = await database.$transaction(async (tx) => {
      ...
      if (
        ...
        // Get current recipe data using Prisma (replaces raw SQL)
        const currentRecipe = await database.recipe.findFirst({
          where: {
            tenantId,
            id: recipeId,
            deletedAt: null,
      ...
      // 2. Get latest version data for defaults using Prisma (replaces raw SQL)
      ...
      const versionResult = await runtime.runCommand(
        "create",
        {
          ...
          cuisineType: body.cuisineType ?? prev.cuisineType ?? "",
          description: body.description ?? prev.description ?? "",
          tags: body.tags ?? prev.tags ?? [],
          versionNumber: newVersionNumber,
          yieldQuantity: body.yieldQuantity ?? Number(prev.yieldQuantity),
          yieldUnitId: body.yieldUnitId ?? prev.yieldUnitId,
          yieldDescription:
            body.yieldDescription ?? prev.yieldDescription ?? "",
          prepTimeMinutes: body.prepTimeMinutes ?? prev.prepTimeMinutes ?? 0,
          cookTimeMinutes: body.cookTimeMinutes ?? prev.cookTimeMinutes ?? 0,
          restTimeMinutes: body.restTimeMinutes ?? prev.restTimeMinutes ?? 0,
          difficultyLevel: body.difficultyLevel ?? prev.difficultyLevel ?? 1,
