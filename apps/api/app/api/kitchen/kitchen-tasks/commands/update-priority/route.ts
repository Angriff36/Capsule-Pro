// Auto-generated Next.js command handler for KitchenTask.updatePriority
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, priority, ...commandArgs } = body;
    ...
    console.log("[kitchen-task/update-priority] Pre-command state:", {
      taskId: id,
    ...
    if (!result.success) {
      console.error("[kitchen-task/update-priority] Command failed:", {
      ...
      if (result.guardFailure) {
        return manifestErrorResponse(
          `Guard ${result.guardFailure.index} failed: ${result.guardFailure.formatted} (task.status=${taskRecord?.status}, priority=${priority})`,
          422
        );
      }
    ...
    console.error("[kitchen-task/update-priority] Error:", error);
