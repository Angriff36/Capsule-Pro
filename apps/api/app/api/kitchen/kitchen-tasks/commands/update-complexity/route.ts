// Auto-generated Next.js command handler for KitchenTask.updateComplexity
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { id, complexity, ...commandArgs } = body;
    ...
    console.log("[kitchen-task/update-complexity] Pre-command state:", {
      taskId: id,
    ...
    if (!result.success) {
      console.error("[kitchen-task/update-complexity] Command failed:", {
      ...
      if (result.guardFailure) {
        return manifestErrorResponse(
          `Guard ${result.guardFailure.index} failed: ${result.guardFailure.formatted} (task.status=${taskRecord?.status}, complexity=${complexity})`,
          422
        );
      }
    ...
    console.error("[kitchen-task/update-complexity] Error:", error);
