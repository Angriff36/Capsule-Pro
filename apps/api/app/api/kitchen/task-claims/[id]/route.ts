// Auto-generated Next.js API detail route for KitchenTaskClaim
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const kitchenTaskClaim = await database.kitchenTaskClaim.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!kitchenTaskClaim) {
      return manifestErrorResponse({ error: "KitchenTaskClaim not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ kitchenTaskClaim });
  } catch (error) {
    ...
    console.error("Error fetching kitchenTaskClaim:", error);
