// Auto-generated Next.js API detail route for TemperatureReading
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const temperatureReading = await database.temperatureReading.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!temperatureReading) {
      return manifestErrorResponse({ error: "TemperatureReading not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ temperatureReading });
  } catch (error) {
    ...
    console.error("Error fetching temperatureReading:", error);
