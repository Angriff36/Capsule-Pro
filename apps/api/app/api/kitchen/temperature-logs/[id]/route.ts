// Auto-generated Next.js API detail route for TemperatureLog
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const temperatureLog = await database.temperatureLog.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!temperatureLog) {
      return manifestErrorResponse({ error: "TemperatureLog not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ temperatureLog });
  } catch (error) {
    ...
    console.error("Error fetching temperatureLog:", error);
