// Auto-generated Next.js API detail route for MethodVideo
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const methodVideo = await database.method_videos.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!methodVideo) {
      return manifestErrorResponse({ error: "MethodVideo not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ methodVideo });
  } catch (error) {
    ...
    console.error("Error fetching methodVideo:", error);
