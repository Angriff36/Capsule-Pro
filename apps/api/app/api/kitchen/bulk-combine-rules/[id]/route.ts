// Auto-generated Next.js API detail route for BulkCombineRule
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const bulkCombineRule = await database.bulk_combine_rules.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!bulkCombineRule) {
      return manifestErrorResponse({ error: "BulkCombineRule not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ bulkCombineRule });
  } catch (error) {
    ...
    console.error("Error fetching bulkCombineRule:", error);
