// Auto-generated Next.js API detail route for PrepListImport
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const prepListImport = await database.prep_list_imports.findFirst({
      where: {
        id,
        tenant_id: tenantId
      },
    ...
    if (!prepListImport) {
      return manifestErrorResponse({ error: "PrepListImport not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ prepListImport });
  } catch (error) {
    ...
    console.error("Error fetching prepListImport:", error);
