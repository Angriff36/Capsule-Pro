// Auto-generated Next.js API detail route for QATemperatureLog
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const qATemperatureLog = await database.temperatureLog.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!qATemperatureLog) {
      return manifestErrorResponse({ error: "QATemperatureLog not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ qATemperatureLog });
  } catch (error) {
    ...
    console.error("Error fetching qATemperatureLog:", error);
