// Auto-generated Next.js API detail route for QACorrectiveAction
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const qACorrectiveAction = await database.qACorrectiveAction.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!qACorrectiveAction) {
      return manifestErrorResponse({ error: "QACorrectiveAction not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ qACorrectiveAction });
  } catch (error) {
    ...
    console.error("Error fetching qACorrectiveAction:", error);
