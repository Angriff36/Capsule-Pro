// Auto-generated Next.js API detail route for QualityCheckItem
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const qualityCheckItem = await database.qualityCheckItem.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!qualityCheckItem) {
      return manifestErrorResponse({ error: "QualityCheckItem not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ qualityCheckItem });
  } catch (error) {
    ...
    console.error("Error fetching qualityCheckItem:", error);
