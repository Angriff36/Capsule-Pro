// Auto-generated Next.js API detail route for QACheck
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const qACheck = await database.qACheck.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!qACheck) {
      return manifestErrorResponse({ error: "QACheck not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ qACheck });
  } catch (error) {
    ...
    console.error("Error fetching qACheck:", error);
