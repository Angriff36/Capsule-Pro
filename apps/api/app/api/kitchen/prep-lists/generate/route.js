...
exports.generatePrepList = generatePrepList;
exports.savePrepListToProductionBoard = savePrepListToProductionBoard;
exports.savePrepListToDatabase = savePrepListToDatabase;
exports.POST = POST;
...
async function generatePrepList(input) {
  ...
  const eventDishes = await database_1.database.$queryRaw(database_1.Prisma.sql`
      SELECT
        ed.dish_id,
        d.name AS dish_name,
        d.recipe_id,
  ...
  const dishes = eventDishes.map((dish) => {
...
async function getAllRecipeIngredients(_tenantId, dishesWithRecipes) {
  ...
  const results = await database_1.database.$queryRaw(database_1.Prisma.sql`
      SELECT
        d.id AS dish_id,
        d.name AS dish_name,
...
function aggregateIngredients(allIngredients, dishes, batchMultiplier) {
  const map = new Map();
...
function calculateScaledQuantity(baseQuantity, batchMultiplier, dish) {
...
function populateStations(ingredientMap, stationMap, dietaryRestrictions) {
...
function createStationTasks(stationMap, dishes, eventDate) {
...
function assignIngredientToStation(category) {
  if (!category) {
    return "prep-station";
  }
...
function applyDietaryRestrictions(allergens, dietaryRestrictions) {
  const substitutions = [];
...
async function savePrepListToProductionBoard(eventId, prepList) {
  const { orgId, userId } = await (0, server_1.auth)();
 ...
 */
async function savePrepListToDatabase(eventId, prepList, name) {
...
async function POST(request) {
  try {
    ...
    return server_2.NextResponse.json(prepList);
  } catch (error) {
