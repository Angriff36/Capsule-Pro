...
async function GET(request) {
  ...
  try {
    ...
    const stationMetrics = await database_1.database.$queryRawUnsafe(
      `
      SELECT
    ...
    const [prepListsSync, allergenWarnings, wasteAlerts, timeToCompletion] =
      await Promise.all([
        ...
        database_1.database.$queryRawUnsafe(
          `
        SELECT
          COUNT(*)::int as total,
        ...
        database_1.database.allergenWarning.count({
          where: {
            tenantId,
            isAcknowledged: false,
            deletedAt: null,
            ...(locationId ? { tenantId: locationId } : {}),
          },
        }),
        // Waste entries in period (as alerts)
        database_1.database.wasteEntry.count({
          where: {
            tenantId,
            deletedAt: null,
    ...
    const stationTrends = await database_1.database.$queryRawUnsafe(
      `
      SELECT
    ...
    const topPerformers = await database_1.database.$queryRawUnsafe(
      `
      SELECT
        u.id as employee_id,
        u.first_name,
