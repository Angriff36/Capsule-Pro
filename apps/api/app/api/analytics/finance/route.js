...
async function GET(request) {
  ...
  try {
    ...
    const currentPeriodMetrics = await database_1.database.$queryRawUnsafe(
      `
      SELECT
    ...
    const previousPeriodMetrics = await database_1.database.$queryRawUnsafe(
      `
      SELECT
        COUNT(*)::int as total_events,
        COALESCE(SUM(ep.budgeted_revenue), 0)::numeric as budgeted_revenue,
    ...
    const ledgerData = await database_1.database.$queryRawUnsafe(
      `
      SELECT
        (
    ...
    // Get budget alerts
    const budgetAlerts = await database_1.database.budgetAlert.findMany({
      where: {
        tenantId,
        resolvedAt: null,
      },
    ...
    if (financeAlerts.length === 0) {
      financeAlerts.push(
        { message: "Review event budgets before next cycle.", severity: "Low" },
        {
