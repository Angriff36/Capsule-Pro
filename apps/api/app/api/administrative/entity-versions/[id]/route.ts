// Auto-generated Next.js API detail route for EntityVersion
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const entityVersion = await database.entityVersion.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!entityVersion) {
      return manifestErrorResponse({ error: "EntityVersion not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ entityVersion });
  } catch (error) {
    ...
    console.error("Error fetching entityVersion:", error);
