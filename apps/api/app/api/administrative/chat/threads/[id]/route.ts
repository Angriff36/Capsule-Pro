// Auto-generated Next.js API detail route for AdminChatThread
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const adminChatThread = await database.adminChatThread.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!adminChatThread) {
      return manifestErrorResponse({ error: "AdminChatThread not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ adminChatThread });
  } catch (error) {
    ...
    console.error("Error fetching adminChatThread:", error);
