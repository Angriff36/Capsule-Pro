// Auto-generated Next.js API detail route for AdminChatMessage
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const adminChatMessage = await database.adminChatMessage.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!adminChatMessage) {
      return manifestErrorResponse({ error: "AdminChatMessage not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ adminChatMessage });
  } catch (error) {
    ...
    console.error("Error fetching adminChatMessage:", error);
