// Auto-generated Next.js API detail route for RouteStop
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const routeStop = await database.routeStop.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!routeStop) {
      return manifestErrorResponse({ error: "RouteStop not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ routeStop });
  } catch (error) {
