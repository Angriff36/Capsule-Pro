// Auto-generated Next.js API detail route for LogisticsRoute
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const logisticsRoute = await database.logisticsRoute.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!logisticsRoute) {
      return manifestErrorResponse({ error: "LogisticsRoute not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ logisticsRoute });
  } catch (error) {
    ...
    console.error("Error fetching logisticsRoute:", error);
