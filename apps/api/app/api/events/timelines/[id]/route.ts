// Auto-generated Next.js API detail route for EventTimeline
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const eventTimeline = await database.eventTimeline.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!eventTimeline) {
      return manifestErrorResponse({ error: "EventTimeline not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ eventTimeline });
  } catch (error) {
    ...
    console.error("Error fetching eventTimeline:", error);
