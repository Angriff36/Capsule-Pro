// Auto-generated Next.js API detail route for EventStaffAssignment
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const eventStaffAssignment = await database.eventStaffAssignment.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!eventStaffAssignment) {
      return manifestErrorResponse({ error: "EventStaffAssignment not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ eventStaffAssignment });
  } catch (error) {
