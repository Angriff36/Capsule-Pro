// Auto-generated Next.js API detail route for TimelineTask
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const timelineTask = await database.timelineTask.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!timelineTask) {
      return manifestErrorResponse({ error: "TimelineTask not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ timelineTask });
  } catch (error) {
    ...
    console.error("Error fetching timelineTask:", error);
