// Auto-generated Next.js API detail route for EventTimelineItem
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const eventTimelineItem = await database.eventTimeline.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!eventTimelineItem) {
      return manifestErrorResponse({ error: "EventTimelineItem not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ eventTimelineItem });
  } catch (error) {
    ...
    console.error("Error fetching eventTimelineItem:", error);
