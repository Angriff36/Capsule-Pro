// Auto-generated Next.js API detail route for AiEventSetupSession
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const aiEventSetupSession = await database.aiEventSetupSession.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!aiEventSetupSession) {
      return manifestErrorResponse({ error: "AiEventSetupSession not found", diagnostics: [] }, 404);
    }
