// Auto-generated Next.js API detail route for CrmScoringRule
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const crmScoringRule = await database.crmScoringRule.findFirst({
      where: {
        id,
        tenantId,
      },
    ...
    if (!crmScoringRule) {
      return manifestErrorResponse({ error: "CrmScoringRule not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ crmScoringRule });
  } catch (error) {
    ...
    console.error("Error fetching crmScoringRule:", error);
