// Auto-generated Next.js API detail route for InteractionAttachment
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const interactionAttachment = await database.interactionAttachment.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!interactionAttachment) {
      return manifestErrorResponse({ error: "InteractionAttachment not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ interactionAttachment });
  } catch (error) {
    ...
    console.error("Error fetching interactionAttachment:", error);
