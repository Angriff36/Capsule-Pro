...
async function GET(request, { params }) {
  try {
    ...
    const eventCount = await database_1.database.event.count({
      where: {
        AND: [{ tenantId }, { clientId: id }, { deletedAt: null }],
      },
    });
...
async function PUT(request, { params }) {
  try {
    ...
    (0, validation_1.validateUpdateClientRequest)(body);
    ...
    return server_2.NextResponse.json({ data: updatedClient });
  } catch (error) {
    if (error instanceof invariant_1.InvariantError) {
      return server_2.NextResponse.json(
        { message: error.message },
