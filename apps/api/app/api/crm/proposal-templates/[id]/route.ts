// Auto-generated Next.js API detail route for ProposalTemplate
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const proposalTemplate = await database.proposalTemplate.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!proposalTemplate) {
      return manifestErrorResponse({ error: "ProposalTemplate not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ proposalTemplate });
  } catch (error) {
    ...
    console.error("Error fetching proposalTemplate:", error);
