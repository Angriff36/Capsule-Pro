// Auto-generated Next.js command handler for Proposal.withdraw
...
export async function POST(request: NextRequest) {
  try {
    ...
    log.info("[proposal/withdraw] Executing command:", {
      entityName: "Proposal",
      command: "withdraw",
    ...
    const result = await runtime.runCommand("withdraw", body, {
      entityName: "Proposal",
      ...(body.id ? { instanceId: String(body.id) } : {}),
    });

    if (!result.success) {
      log.error("[proposal/withdraw] Command failed:", {
    ...
    log.error("[proposal/withdraw] Error:", error);
