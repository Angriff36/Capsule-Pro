...
# Manifest reference for `apps/api/app/api/cron`

**Why this file:** Cron schedules are hand-bound in vercel.json + hand-written route handlers. Declare a `schedule` (cron/interval) inside the entity block in .manifest source so the IR emits a schedules array, then generate the crons + route via the native nextjs.schedule projection. (See the "Scheduled commands" section of the embedded Workflows page.)

**Relevant divergences:** U8 — see `manifest/MANIFEST-DIVERGENCES.md` for the full remediation detail.
