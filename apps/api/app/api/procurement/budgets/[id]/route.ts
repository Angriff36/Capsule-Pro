// Auto-generated Next.js API detail route for ProcurementBudget
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const procurementBudget = await database.procurementBudget.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!procurementBudget) {
      return manifestErrorResponse({ error: "ProcurementBudget not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ procurementBudget });
  } catch (error) {
    ...
    console.error("Error fetching procurementBudget:", error);
