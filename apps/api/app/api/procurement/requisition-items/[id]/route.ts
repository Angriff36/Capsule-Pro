...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const purchaseRequisitionItem = await database.purchaseRequisitionItem.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!purchaseRequisitionItem) {
      return manifestErrorResponse({ error: "PurchaseRequisitionItem not found", diagnostics: [] }, 404);
    }
    ...
    console.error("Error fetching purchaseRequisitionItem:", error);
