...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const purchaseRequisition = await database.purchaseRequisition.findFirst({
      where: {
        id,
        tenantId,
        deletedAt: null
    ...
    if (!purchaseRequisition) {
      return manifestErrorResponse({ error: "PurchaseRequisition not found", diagnostics: [] }, 404);
    }
