...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!["approved", "rejected"].includes(action)) {
      return manifestErrorResponse("Invalid action. Must be 'approved' or 'rejected'", 400);
    }
    ...
    const currentPO = currentPOs[0] as { id: string; status: string; po_number: string };
    ...
    `, orderId, action, userId, action, notes || null, tenantId);
