// Auto-generated Next.js API detail route for BankAccount
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const bankAccount = await database.employeeBankAccount.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!bankAccount) {
      return manifestErrorResponse({ error: "BankAccount not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ bankAccount });
  } catch (error) {
