// Auto-generated Next.js API detail route for PaymentRefundAttempt
...
export async function GET(
  ...
  if (!tenantId) {
    ...
    const paymentRefundAttempt = await database.paymentRefundAttempt.findFirst({
      where: {
        id,
        tenantId
      },
    ...
    if (!paymentRefundAttempt) {
      return manifestErrorResponse({ error: "PaymentRefundAttempt not found", diagnostics: [] }, 404);
    }

    return manifestSuccessResponse({ paymentRefundAttempt });
  } catch (error) {
    ...
    console.error("Error fetching paymentRefundAttempt:", error);
