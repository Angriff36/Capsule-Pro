// Auto-generated Next.js API detail route for CollectionPaymentPlan
...
export async function GET(
  ...
  try {
    ...
    const collectionPaymentPlan =
      await database.collectionPaymentPlan.findFirst({
        where: {
          id,
          tenantId,
        },
    ...
    if (!collectionPaymentPlan) {
      return manifestErrorResponse(
        { error: "CollectionPaymentPlan not found", diagnostics: [] },
        404
      );
    }

    return manifestSuccessResponse({ collectionPaymentPlan });
  } catch (error) {
    ...
    console.error("Error fetching collectionPaymentPlan:", error);
    return manifestErrorResponse(
      {
