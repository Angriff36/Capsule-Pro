...
export async function GET(request: Request) {
  ...
  const opened = emailLogs.filter((l) => l.status === "opened" || l.status === "delivered").length;
  ...
  const conversionRate = totalLeads > 0 ? (convertedLeads / totalLeads) * 100 : null;
  ...
  const workflowEmailCounts = emailLogs.reduce<Record<string, { sent: number; opened: number }>>(
    (acc, log) => {
      const wId = log.workflowId || "unlinked";
      if (!acc[wId]) acc[wId] = { sent: 0, opened: 0 };
      acc[wId].sent++;
      if (log.status === "opened" || log.status === "delivered") acc[wId].opened++;
      return acc;
    },
    {},
  );
