...
describe("Admin Extended API", () => {
  ...
  describe("Admin Chat Participant", () => {
      ...
      describe(label, () => {
        ...
        it("should return 200 on successful command", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: { id: "result-id", archived: true },
            emittedEvents: [{ type: "archived" }],
          });
        ...
        it("should pass correct command and body to runtime", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: {},
            emittedEvents: [],
          ...
          expect(mockRunCommand).toHaveBeenCalledWith(commandName, body, {
            entityName: "AdminChatParticipant",
          });
        });
        ...
        it("should return 403 on policy denial", async () => {
          ...
          const respBody = await response.json();
          expect(respBody.success).toBe(false);
          expect(respBody.message).toContain("Access denied");
          expect(respBody.message).toContain("AdminOnly");
        });

        it("should return 422 on guard failure", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            guardFailure: { index: 2, formatted: "Participant not found" },
          });
          ...
          const respBody = await response.json();
          expect(respBody.success).toBe(false);
          expect(respBody.message).toContain("Guard 2 failed");
          expect(respBody.message).toContain("Participant not found");
        });
        ...
        it("should return 400 on general command failure", async () => {
          ...
          const respBody = await response.json();
          expect(respBody.success).toBe(false);
          expect(respBody.message).toBe("Command execution failed");
        });
    ...
    testManifestRoute("POST archive", archiveParticipant, "archive", {
      instanceId: "participant-001",
    });

    testManifestRoute("POST clear-history", clearHistory, "clearHistory", {
      instanceId: "participant-001",
    });

    testManifestRoute("POST unarchive", unarchiveParticipant, "unarchive", {
      instanceId: "participant-001",
    });
  });
  ...
  // =====================================================================
  // AI EVENT SETUP SESSION (manifest runtime commands)
  // =====================================================================
    ...
    function testSessionCommand(
      ...
      describe(label, () => {
        ...
        it("should return 200 on success", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: { sessionId: "session-001", status: "confirmed" },
            emittedEvents: [{ type: "session.confirmed" }],
          });
        ...
        it("should pass correct command and entity to runtime", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: {},
            emittedEvents: [],
        ...
        it("should return 403 on policy denial", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            policyDenial: { policyName: "SessionOwnerOnly" },
          });
        ...
        it("should return 422 on guard failure", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            guardFailure: { index: 1, formatted: "Session already completed" },
          });
          ...
          const respBody = await response.json();
          expect(respBody.message).toContain("Guard 1 failed");
        });
        ...
        it("should return 400 on command failure", async () => {
          ...
          const respBody = await response.json();
          expect(respBody.message).toBe("Invalid session state");
        });
        ...
        it("should use correct tenantId for isolation", async () => {
          ...
          expect(callArg.user.tenantId).toBe(TEST_TENANT_ID);
        });
      });
    }

    testSessionCommand("POST cancel", sessionCancel, "cancel", {
      instanceId: "session-001",
    });

    testSessionCommand("POST confirm", sessionConfirm, "confirm", {
      instanceId: "session-001",
    });

    testSessionCommand("POST mark-created", sessionMarkCreated, "markCreated", {
      instanceId: "session-001",
    });

    testSessionCommand("POST parse", sessionParse, "parse", {
      instanceId: "session-001",
  ...
  describe("Alerts Config", () => {
      ...
      describe(label, () => {
        ...
        it("should return 200 on success", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: { id: "alert-001" },
            emittedEvents: [{ type: "alert.created" }],
          });
        ...
        it("should return 422 on guard failure", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            guardFailure: { index: 1, formatted: "Duplicate alert name" },
          });
        ...
        it("should return 400 on command failure", async () => {
          ...
          const respBody = await response.json();
          expect(respBody.message).toBe("Invalid config");
        });
    ...
    testAlertsCommand("POST create", alertsConfigCreate, "create", {
      name: "Low Stock Alert",
      type: "inventory",
      threshold: 10,
    });

    testAlertsCommand("POST update", alertsConfigUpdate, "update", {
      instanceId: "alert-001",
      threshold: 20,
    });

    testAlertsCommand("POST remove", alertsConfigRemove, "remove", {
      instanceId: "alert-001",
    });
  });
  ...
  describe("Allergen Warning", () => {
    ...
    function testAllergenCommand(
      ...
      describe(label, () => {
        ...
        it("should return 200 on success", async () => {
          mockRunCommand.mockResolvedValue({
            success: true,
            result: { id: "allergen-001" },
            emittedEvents: [{ type: "allergen.created" }],
          });
        ...
        it("should return 403 on policy denial", async () => {
          ...
          const respBody = await response.json();
          expect(respBody.message).toContain("Access denied");
          expect(respBody.message).toContain("ManagerOnly");
        });

        it("should return 422 on guard failure", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            guardFailure: { index: 0, formatted: "Allergen already resolved" },
          });
          ...
          const respBody = await response.json();
          expect(respBody.message).toContain("Guard 0 failed");
          expect(respBody.message).toContain("Allergen already resolved");
        });

        it("should return 400 on command failure with error message", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
          ...
          const respBody = await response.json();
          expect(respBody.message).toBe("Allergen warning not found");
        });

        it("should return 400 with default error when no error message", async () => {
          mockRunCommand.mockResolvedValue({
            success: false,
            error: undefined,
          });
          ...
          const respBody = await response.json();
          expect(respBody.message).toBe("Command failed");
        });
    ...
    testAllergenCommand("POST resolve", allergenResolve, "resolve", {
      instanceId: "allergen-001",
      resolution: "Ingredient substituted",
    });

    testAllergenCommand("POST soft-delete", allergenSoftDelete, "softDelete", {
      instanceId: "allergen-001",
    });
  });
});
