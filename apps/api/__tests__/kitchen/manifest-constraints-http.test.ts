...
describe("Manifest HTTP Constraint Enforcement - Recipe Commands", () => {
  ...
  it("should handle valid update requests", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "recipe-001", name: "Updated Recipe" }, events: [] })
    );
    const response = await dispatch("Recipe", "update")(makeRequest({ id: "recipe-001", name: "Updated Recipe" }));
...
describe("Manifest HTTP Constraint Enforcement - Dish Commands", () => {
  ...
  it("should reject unauthorized update-lead-time with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("Dish", "update-lead-time")(makeRequest({ id: "dish-001", prepTimeMinutes: 45, cookTimeMinutes: 30 }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
...
describe("Manifest HTTP Constraint Enforcement - Menu Commands", () => {
  ...
  it("should handle valid menu update requests", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "menu-001", name: "Updated Menu" }, events: [] })
    );
    const response = await dispatch("Menu", "update")(makeRequest({ id: "menu-001", name: "Updated Menu" }));
    expect(response.status).toBe(200);
  });
  ...
  it("should reject unauthorized menu activate with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("Menu", "activate")(makeRequest({ id: "menu-001" }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
  ...
  it("should reject unauthorized menu deactivate with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("Menu", "deactivate")(makeRequest({ id: "menu-001" }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
...
describe("Manifest HTTP Constraint Enforcement - PrepTask Commands", () => {
  ...
  it("should reject unauthorized start with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("PrepTask", "start")(makeRequest({ id: "task-001", userId: "user-001" }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
  ...
  it("should reject unauthorized complete with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("PrepTask", "complete")(makeRequest({ id: "task-001", quantityCompleted: 10, userId: "user-001" }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
...
describe("Manifest HTTP Constraint Enforcement - PrepListItem Commands", () => {
  ...
  it("should handle valid quantity update with WARN constraint", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "item-001", baseQuantity: 15, scaledQuantity: 35 }, events: [] })
    );
    const response = await dispatch("PrepListItem", "update-quantity")(makeRequest({ id: "item-001", newBaseQuantity: 15, newScaledQuantity: 35, newBaseUnit: "kg", newScaledUnit: "kg" }));
    const data = await response.json();
    expect(response.status).toBe(200);
  ...
  it("should handle normal quantity update without warning", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "item-001", baseQuantity: 12, scaledQuantity: 24 }, events: [] })
    );
    const response = await dispatch("PrepListItem", "update-quantity")(makeRequest({ id: "item-001", newBaseQuantity: 12, newScaledQuantity: 24, newBaseUnit: "kg", newScaledUnit: "kg" }));
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data).toHaveProperty("success", true);
  });
  ...
  it("should handle station update to same station without warning", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "item-001", stationId: "station-001", stationName: "Hot Prep Station" }, events: [] })
    );
    const response = await dispatch("PrepListItem", "update-station")(makeRequest({ id: "item-001", newStationId: "station-001", newStationName: "Hot Prep Station" }));
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data).toHaveProperty("success", true);
  });
...
describe("Manifest HTTP Constraint Enforcement - RecipeVersion Commands", () => {
  ...
  it("should reject unauthorized create with 401", async () => {
    vi.mocked(requireCurrentUser).mockRejectedValueOnce(new InvariantError("Unauthenticated"));
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", difficulty: 3 }));
    const data = await response.json();
    expect(response.status).toBe(401);
    expect(data).toHaveProperty("success", false);
  });
  ...
  it("should reject invalid difficulty with 422 (BLOCK)", async () => {
    ...
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", difficulty: 6 }));
    const data = await response.json();
  ...
  it("should reject negative times with 422 (BLOCK)", async () => {
    ...
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", prepTime: -30 }));
    const data = await response.json();
    expect(response.status).toBe(422);
    expect(data).toHaveProperty("success", false);
  });
  ...
  it("should allow high difficulty with WARN constraint", async () => {
    vi.mocked(runManifestCommand).mockResolvedValueOnce(
      manifestSuccessResponse({ result: { id: "version-001", difficulty: 4 }, events: [] })
    );
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", difficulty: 4 }));
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data).toHaveProperty("success", true);
  });
  ...
  it("should allow long recipe with WARN constraint", async () => {
    ...
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", prepTime: 180, cookTime: 300, restTime: 60 }));
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data).toHaveProperty("success", true);
  });
  ...
  it("should handle valid recipe creation without warnings", async () => {
    ...
    const response = await dispatch("RecipeVersion", "create")(makeRequest({ recipeId: "recipe-001", yieldQty: 10, yieldUnit: 1, prepTime: 30, cookTime: 60, restTime: 10, difficulty: 2 }));
    const data = await response.json();
    expect(response.status).toBe(200);
    expect(data).toHaveProperty("success", true);
  });
