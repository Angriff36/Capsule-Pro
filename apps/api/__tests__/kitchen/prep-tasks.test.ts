...
function findManyAndClauses(): unknown[] {
  ...
  if (!call) throw new Error("prepTask.findMany was not called");
  const arg = call[0] as { where?: { AND: unknown[] } };
  if (!arg?.where?.AND) throw new Error("prepTask.findMany was not called with a where.AND clause");
  return arg.where.AND;
}

function mockRunSuccess(result: Record<string, unknown> = { id: TEST_PREP_TASK_ID }) {
  mocks.runManifestCommand.mockResolvedValue(
    new Response(JSON.stringify({ success: true, result, events: [{ type: "PrepTaskEvent", entityId: result.id }] }), { status: 200 })
  );
}
...
describe("Prep Tasks API", () => {
  ...
  describe("GET /api/kitchen/prep-tasks (filtered list)", () => {
    ...
    it("returns paginated results with default page=1, limit=20", async () => {
      mocks.prepTaskFindMany.mockResolvedValue([{ id: TEST_PREP_TASK_ID, tenantId: TEST_TENANT_ID }]);
    ...
    it("scopes results to tenant + non-deleted", async () => {
      ...
      expect(ands.some((c) => (c as Record<string, unknown>).tenantId === TEST_TENANT_ID)).toBe(true);
      expect(ands.some((c) => (c as Record<string, unknown>).deletedAt === null)).toBe(true);
    });
    ...
    it("threads status filter", async () => {
      ...
      const ands = findManyAndClauses();
      expect(ands.some((c) => (c as Record<string, unknown>).status === "in_progress")).toBe(true);
    });
    ...
    it("threads priority filter (parsed as int)", async () => {
      ...
      const ands = findManyAndClauses();
      expect(ands.some((c) => (c as Record<string, unknown>).priority === 3)).toBe(true);
    });
    ...
    it("applies search filter as case-insensitive contains on name + notes", async () => {
      ...
      const ands = findManyAndClauses();
      const orClause = ands.find((c) => (c as Record<string, unknown>).OR !== undefined) as { OR: Array<Record<string, { contains: string; mode: string }>> };
    ...
    it("applies isOverdue filter excluding done/completed/canceled", async () => {
      ...
      const overdueClause = ands.find((c) => {
        const inner = (c as Record<string, unknown>).AND as unknown[] | undefined;
      ...
      const statusClause = overdueClause.AND.find((x) => (x as Record<string, unknown>).status) as { status: { notIn: string[] } };
      expect(statusClause.status.notIn).toEqual(["done", "completed", "canceled"]);
    });
    ...
    it("respects custom page/limit", async () => {
      ...
      expect(mocks.prepTaskFindMany).toHaveBeenCalledWith(expect.objectContaining({ take: 10, skip: 10 }));
    });
    ...
    it("clamps limit to 100", async () => {
      ...
      expect(mocks.prepTaskFindMany).toHaveBeenCalledWith(expect.objectContaining({ take: 100 }));
    });
    ...
    it("applies orderBy [priority desc, dueByDate asc, startByDate asc]", async () => {
      ...
      const { GET } = await import("@/app/api/kitchen/prep-tasks/route");
      await GET(makeGET("/api/kitchen/prep-tasks"));
      expect(mocks.prepTaskFindMany).toHaveBeenCalledWith(expect.objectContaining({
        orderBy: [{ priority: "desc" }, { dueByDate: "asc" }, { startByDate: "asc" }],
      }));
    });
  ...
  describe("GET /api/kitchen/prep-tasks/list", () => {
    ...
    it("clamps limit to MAX_LIMIT=200", async () => {
      ...
      expect(mocks.prepTaskFindMany).toHaveBeenCalledWith(expect.objectContaining({ take: 200 }));
    });
  ...
  describe("GET /api/kitchen/prep-tasks/[id]", () => {
    it("returns 401 when unauthenticated", async () => {
      ...
      const res = await GET(makeGET(`/api/kitchen/prep-tasks/${TEST_PREP_TASK_ID}`), { params: Promise.resolve({ id: TEST_PREP_TASK_ID }) });
      expect(res.status).toBe(401);
    });
    ...
    it("returns 200 with prepTask payload", async () => {
      mocks.prepTaskFindFirst.mockResolvedValue({ id: TEST_PREP_TASK_ID, tenantId: TEST_TENANT_ID });
      const { GET } = await import("@/app/api/kitchen/prep-tasks/[id]/route");
      const res = await GET(makeGET(`/api/kitchen/prep-tasks/${TEST_PREP_TASK_ID}`), { params: Promise.resolve({ id: TEST_PREP_TASK_ID }) });
      expect(res.status).toBe(200);
      const body = await res.json();
  ...
  const COMMANDS: CommandSpec[] = [
    { name: "cancel", runtimeName: "cancel", sampleBody: { id: TEST_PREP_TASK_ID, reason: "no longer needed" } },
    { name: "claim", runtimeName: "claim", sampleBody: { id: TEST_PREP_TASK_ID, claimedBy: TEST_USER_ID } },
    { name: "complete", runtimeName: "complete", sampleBody: { id: TEST_PREP_TASK_ID, quantityCompleted: 10 } },
    { name: "create", runtimeName: "create", sampleBody: { eventId: TEST_EVENT_ID, name: "Dice onions", taskType: "prep", quantityTotal: 10, priority: 1 } },
    { name: "reassign", runtimeName: "reassign", sampleBody: { id: TEST_PREP_TASK_ID, assigneeId: "another-user" } },
    { name: "release", runtimeName: "release", sampleBody: { id: TEST_PREP_TASK_ID } },
    { name: "start", runtimeName: "start", sampleBody: { id: TEST_PREP_TASK_ID, startedBy: TEST_USER_ID } },
    { name: "unclaim", runtimeName: "unclaim", sampleBody: { id: TEST_PREP_TASK_ID } },
    { name: "update-assignment", runtimeName: "updateAssignment", sampleBody: { id: TEST_PREP_TASK_ID, assigneeId: "user-2" } },
    { name: "update-due-date", runtimeName: "updateDueDate", sampleBody: { id: TEST_PREP_TASK_ID, dueByDate: "2026-05-02T12:00:00Z" } },
    { name: "update-priority", runtimeName: "updatePriority", sampleBody: { id: TEST_PREP_TASK_ID, priority: 3 } },
    { name: "update-quantity", runtimeName: "updateQuantity", sampleBody: { id: TEST_PREP_TASK_ID, quantityTotal: 25 } },
    { name: "update-status", runtimeName: "updateStatus", sampleBody: { id: TEST_PREP_TASK_ID, status: "in_progress" } },
  ];

  describe.each(COMMANDS)("POST PrepTask.$name", ({ name, runtimeName, sampleBody }) => {
    it(`returns 401 when unauthenticated [${name}]`, async () => {
      ...
      const res = await manifestDispatch(makePOST(sampleBody), { params: Promise.resolve({ entity: "PrepTask", command: name }) });
      expect(res.status).toBe(401);
    });
    ...
    it(`forwards correct command name + entity to runtime [${name}]`, async () => {
      ...
      await manifestDispatch(makePOST(sampleBody), { params: Promise.resolve({ entity: "PrepTask", command: name }) });
