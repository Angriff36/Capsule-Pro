...
// Import routes
const { GET: getStationsList } = await import(
  "@/app/api/kitchen/stations/route"
);
  ...
  describe("GET /api/kitchen/stations", () => {
    it("returns paginated stations with task counts", async () => {
      ...
      const res = await getStationsList(
        makeGET("http://localhost:3000/api/kitchen/stations")
      );
      expect(res.status).toBe(200);
      const body = await res.json();
    ...
    it("filters by isActive status", async () => {
      ...
      await getStationsList(
        makeGET("http://localhost:3000/api/kitchen/stations?isActive=true")
      );
      expect(mocks.stationFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
    ...
    it("filters by search term", async () => {
      ...
      await getStationsList(
        makeGET("http://localhost:3000/api/kitchen/stations?search=grill")
      );
      expect(mocks.stationFindMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
    ...
    it("handles pagination", async () => {
      ...
      const res = await getStationsList(
        makeGET("http://localhost:3000/api/kitchen/stations?page=3&limit=10")
      );
      const body = await res.json();
      expect(body.pagination).toEqual({
        page: 3,
