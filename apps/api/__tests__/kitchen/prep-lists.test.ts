...
function findManyAndClauses(): unknown[] {
  ...
  if (!arg?.where?.AND) {
    throw new Error("prepList.findMany not called with where.AND");
  }
  ...
  mocks.runManifestCommand.mockResolvedValue(
    new Response(
      JSON.stringify({
        success: true,
        result,
        events: [{ type: "PrepListEvent", entityId: result.id }],
      }),
      { status: 200 }
    )
  );
...
describe("Prep Lists API", () => {
  ...
  describe("GET /api/kitchen/prep-lists", () => {
    ...
    it("filters by station via prepListItem lookup", async () => {
      ...
      const { GET } = await import("@/app/api/kitchen/prep-lists/route");
      const res = await GET(
        makeGET("/api/kitchen/prep-lists?station=station-1")
      );
      expect(res.status).toBe(200);
    ...
    it("returns empty result when station has no prep lists", async () => {
      ...
      const { GET } = await import("@/app/api/kitchen/prep-lists/route");
      const res = await GET(
        makeGET("/api/kitchen/prep-lists?station=empty-station")
      );
      expect(res.status).toBe(200);
      const body = await res.json();
    ...
    it("respects custom page and limit", async () => {
      ...
      expect(mocks.prepListFindMany).toHaveBeenCalledWith(
  ...
  describe("POST /api/kitchen/prep-lists", () => {
    it("delegates to runManifestCommand with PrepList.create", async () => {
      mocks.runManifestCommand.mockResolvedValue(
        new Response(
          JSON.stringify({ success: true, result: { id: TEST_PREP_LIST_ID } }),
          { status: 200 }
        )
      );
      ...
      await POST(
        makePOST("/api/kitchen/prep-lists", { eventId: TEST_EVENT_ID })
      );
  ...
  describe("GET /api/kitchen/prep-lists/[id]", () => {
    it("returns 401 when unauthenticated", async () => {
      ...
      const res = await GET(
        makeGET(`/api/kitchen/prep-lists/${TEST_PREP_LIST_ID}`),
  ...
  describe("PATCH /api/kitchen/prep-lists/[id]", () => {
    it("delegates to runManifestCommand with PrepList.update", async () => {
      ...
      const req = makePOST(`/api/kitchen/prep-lists/${TEST_PREP_LIST_ID}`, {
        name: "Updated",
      });
