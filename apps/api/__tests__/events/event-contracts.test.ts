...
describe("EventContract Command Routes", () => {
  ...
  describe("Authentication gating", () => {
    it("create returns 401 when unauthenticated", async () => {
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/create", {
          method: "POST",
    ...
    it("update returns 401 when unauthenticated", async () => {
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/update", {
          method: "POST",
    ...
    it("sign returns 401 when unauthenticated", async () => {
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/sign", {
          method: "POST",
          body: JSON.stringify({ id: TEST_CONTRACT_ID }),
        })
      );
  ...
  describe("POST /api/eventcontract/update -- success", () => {
    it("updates a contract and returns 200", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        ok({
          result: { id: TEST_CONTRACT_ID, title: "Updated", status: "sent" },
          events: [],
        })
      );
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/update", {
          method: "POST",
          body: JSON.stringify({ id: TEST_CONTRACT_ID, title: "Updated" }),
        })
      );
  ...
  describe("POST /api/eventcontract/send -- success", () => {
    it("sends a contract and returns 200", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        ok({
          result: { id: TEST_CONTRACT_ID, status: "sent" },
          events: [{ type: "ContractSent" }],
        })
      );

      const POST = createHandler("send");
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/send", {
          method: "POST",
          body: JSON.stringify({ id: TEST_CONTRACT_ID }),
        })
      );
  ...
  describe("POST /api/eventcontract/sign -- success", () => {
    it("signs a contract and returns 200", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        ok({
          result: { id: TEST_CONTRACT_ID, status: "signed" },
      ...
      expect(res.status).toBe(200);
      expect(runManifestCommand).toHaveBeenCalledWith(
        expect.objectContaining({ command: "sign" })
      );
    });
  });
  ...
  describe("POST /api/eventcontract/cancel -- success", () => {
    it("cancels a contract and returns 200", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        ok({
          result: { id: TEST_CONTRACT_ID, status: "cancelled" },
          events: [],
        })
      );
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/cancel", {
          method: "POST",
          body: JSON.stringify({
            id: TEST_CONTRACT_ID,
  ...
  describe("POST /api/eventcontract/expire -- success", () => {
    it("expires a contract and returns 200", async () => {
      ...
      const res = await POST(
        createMockRequest("http://localhost:3000/api/eventcontract/expire", {
          method: "POST",
          body: JSON.stringify({ id: TEST_CONTRACT_ID }),
        })
      );
      ...
      expect(res.status).toBe(200);
      expect(runManifestCommand).toHaveBeenCalledWith(
        expect.objectContaining({ command: "expire" })
      );
    });
  });
  ...
  describe("POST /api/eventcontract/markViewed -- success", () => {
    it("marks contract as viewed and returns 200", async () => {
      ...
      expect(res.status).toBe(200);
      expect(runManifestCommand).toHaveBeenCalledWith(
        expect.objectContaining({ command: "markViewed" })
      );
    });
  });
  ...
  describe("POST /api/eventcontract/softDelete -- success", () => {
    it("soft-deletes a contract and returns 200", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        ok({
          result: { id: TEST_CONTRACT_ID, deletedAt: "2026-02-01" },
          events: [],
        })
      );
      ...
      expect(res.status).toBe(200);
      expect(runManifestCommand).toHaveBeenCalledWith(
        expect.objectContaining({ command: "softDelete" })
      );
    });
  });
  ...
  describe("Policy denial (403)", () => {
    it("returns 403 when policy denies access on create", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        fail(403, "Access denied: contractManagerOnly")
      );
    ...
    it("returns 403 when policy denies sign access", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        fail(403, "Access denied: authorizedSignerOnly")
      );
  ...
  describe("Guard failure (422)", () => {
    it("returns 422 when guard rejects on send", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        fail(422, "Guard failed: wrong status")
      );
    ...
    it("returns 422 when guard rejects on cancel", async () => {
      vi.mocked(runManifestCommand).mockResolvedValue(
        fail(422, "Guard failed: cannot cancel signed")
      );
