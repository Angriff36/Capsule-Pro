...
const proposal = ir.entities.find((e: { name: string }) => e.name === "Proposal");
const createCmd = ir.commands.find(
  (c: { entity: string; name: string }) => c.entity === "Proposal" && c.name === "create"
);
...
const PARENT_OWNED = ["clientId", "eventDate", "eventType", "venueName", "venueAddress"] as const;
...
describe("Proposal create — does not require event-owned input", () => {
  ...
  it("declares the event-owned snapshot fields it inherits (so they can be stored)", () => {
    const props = new Set(proposal.properties.map((p: { name: string }) => p.name));
