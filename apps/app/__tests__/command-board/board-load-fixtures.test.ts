...
function createMockConnection(index: number, projections: BoardProjection[]): DerivedConnection {
	...
	const relationshipTypes = ["client_to_event", "event_to_task", "event_to_employee", "generic"];
...
function createMediumFixtures() {
	const entityTypes: EntityType[] = ["event", "prep_task", "client", "inventory_item"];
	...
	const connections = Array.from({ length: 25 }, (_, i) => createMockConnection(i, projections));
...
function createLargeFixtures() {
	...
	const connections = Array.from({ length: 100 }, (_, i) => createMockConnection(i, projections));
...
describe("Performance Budget Validation", () => {
	it("documents performance budget thresholds for empty boards", () => {
		expect(PERFORMANCE_BUDGETS.projectionNormalization.empty).toBeLessThan(0.01);
		expect(PERFORMANCE_BUDGETS.entityMapConstruction.empty).toBeLessThan(0.01);
		expect(PERFORMANCE_BUDGETS.connectionTransformation.empty).toBeLessThan(0.01);
	...
	it("documents performance budget thresholds for medium boards", () => {
		expect(PERFORMANCE_BUDGETS.projectionNormalization.medium).toBeLessThan(0.01);
		expect(PERFORMANCE_BUDGETS.entityMapConstruction.medium).toBeLessThan(0.1);
		expect(PERFORMANCE_BUDGETS.connectionTransformation.medium).toBeLessThan(0.01);
	...
	it("documents performance budget thresholds for large boards", () => {
		expect(PERFORMANCE_BUDGETS.projectionNormalization.large).toBeLessThan(0.01);
		expect(PERFORMANCE_BUDGETS.entityMapConstruction.large).toBeLessThan(0.1);
		expect(PERFORMANCE_BUDGETS.connectionTransformation.large).toBeLessThan(0.01);
