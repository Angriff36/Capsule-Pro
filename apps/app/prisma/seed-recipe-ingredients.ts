...
async function main() {
  ...
  const recipeVersions = await prisma.$queryRaw<
    Array<{ id: string; recipe_id: string; name: string }>
  >`
  ...
  // Get existing units
  const units = await prisma.$queryRaw<Array<{ id: number; code: string }>>`
  ...
  for (const ing of commonIngredients) {
    ...
    if (existing) {
      ...
      try {
        const newIng = await prisma.$queryRaw<Array<{ id: string }>>`
