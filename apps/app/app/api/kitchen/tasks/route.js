...
async function GET(request) {
  ...
  return server_2.NextResponse.json({ tasks: tasksWithUsers });
}
async function POST(request) {
  ...
  const task = await database_1.database.kitchenTask.create({
    data: {
      tenantId,
      title: body.title,
  ...
  return server_2.NextResponse.json({ task }, { status: 201 });
}
