...
import type { AvailabilityListResponse, CreateAvailabilityInput } from "./types";
...
export async function POST(request: Request) {
  ...
  if (!(body.employeeId && body.dayOfWeek !== undefined && body.startTime && body.endTime)) {
    return NextResponse.json(
      {
        message: "Employee ID, day of week, start time, and end time are required",
      },
      { status: 400 }
    );
  }
  ...
  const effectiveUntil = body.effectiveUntil ? new Date(body.effectiveUntil) : null;
  if (effectiveUntil) {
  ...
  const { employee, error: employeeError } = await verifyEmployee(tenantId, body.employeeId);
  if (employeeError) {
    return employeeError;
  }

  // Check for overlapping availability
  const { hasOverlap, overlappingAvailability } = await checkOverlappingAvailability(
    tenantId,
    body.employeeId,
    body.dayOfWeek,
    body.startTime,
