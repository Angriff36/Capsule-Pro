...
export async function POST(request: Request) {
  ...
  if (!(body.employeeId && body.patterns && body.patterns.length > 0)) {
    return NextResponse.json(
      {
        message: "Employee ID and at least one availability pattern are required",
      },
      { status: 400 }
    );
  }
  ...
  const errors: Array<{ pattern: { dayOfWeek: number; startTime: string; endTime: string }; error: string }> = [];
