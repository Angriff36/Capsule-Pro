...
import type { EmployeesAvailabilityQuery } from "../types";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const availabilityData = await database.$queryRaw<
      Array<{
        employee_id: string;
        ...
        WHERE e.tenant_id = ${tenantId}
          ...
          ${
            employeeIds && employeeIds.length > 0
              ? Prisma.sql`AND e.id = ANY(${employeeIds}::uuid[])`
              : Prisma.empty
          }
    ...
    if (includeTimeOff) {
      const timeOffData = await database.$queryRaw<
        Array<{
          employee_id: string;
          time_off_id: string;
          start_date: Date;
      ...
      >(
        Prisma.sql`
          ...
          WHERE tor.tenant_id = ${tenantId}
            ...
            ${
              employeeIds && employeeIds.length > 0
                ? Prisma.sql`AND tor.employee_id = ANY(${employeeIds}::uuid[])`
                : Prisma.empty
            }
