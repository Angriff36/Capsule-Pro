...
import type { CreateTimeOffRequestInput, TimeOffRequestsListResponse } from "../types";
...
export async function POST(request: Request) {
  ...
  if (!(body.employeeId && body.startDate && body.endDate && body.requestType)) {
  ...
  const { hasOverlap, overlappingRequests } = await checkOverlappingTimeOffRequests(
    tenantId,
    body.employeeId,
    startDate,
    endDate
  ...
  try {
    ...
    const result = await database.$queryRaw<
      Array<{ id: string; employee_id: string; status: string; start_date: Date; end_date: Date }>
    >(
      Prisma.sql`
