  ...
  try {
    ...
    if (resolved && !overrideReason) {
      return NextResponse.json({
        error: 'overrideReason is required when resolving an allergen warning'
      }, { status: 400 })
    }
    ...
    if (!warning) {
      return NextResponse.json({ error: 'Allergen warning not found' }, { status: 404 })
    }

    // Update the warning using the correct composite key format
    const updatedWarning = await database.allergenWarning.update({
      where: { tenantId_id: { tenantId: tenantId, id: warningId } },
      data: {
        isAcknowledged: true,
