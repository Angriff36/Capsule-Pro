    ...
    <div className="flex flex-1 flex-col p-4">
      ...
      {events.length === 0 && !isLoading ? (
        ...
        <div className="space-y-3">
          {events.map((event) => {
            // Determine link target: first prep list if only one, prep lists tab otherwise
            const linkTarget =
              event.prepListIds.length === 1
                ? `/kitchen/mobile/prep-lists/${event.prepListIds[0]}`
                : `/kitchen/mobile/prep-lists?eventId=${event.id}`;

            return (
              <Link className="block" href={linkTarget} key={event.id}>
                <div
