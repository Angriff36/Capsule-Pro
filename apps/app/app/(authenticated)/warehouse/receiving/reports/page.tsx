...
export default function ReceivingReportsPage() {
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Receiving Reports & Supplier Performance</h1>
        <p className="text-muted-foreground">Track receiving metrics and supplier performance trends</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className="text-2xl font-bold">{receivingSummary.total_pos_received}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className="text-2xl font-bold">{receivingSummary.total_items_received}</div>
              <div className="flex items-center gap-1 text-green-600 text-xs">
                {getTrendIcon(true)}
                <span>+8%</span>
              </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex items-center gap-2">
              <div className={`text-2xl font-bold ${getScoreColor(receivingSummary.average_quality_score)}`}>
        ...
        <CardContent>
          <div className="space-y-4">
            {supplierMetrics.map((supplier) => (
              <div key={supplier.supplier_name} className="rounded-lg border p-4 space-y-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {supplier.total_orders} orders • ${supplier.total_spent.toLocaleString()} total
                    </p>
                  </div>
                  <div className="text-right">
                    {supplier.discrepancy_rate > 5 ? (
                      <div className="flex items-center gap-1 text-red-600 text-sm">
                        ...
                        <span>{supplier.discrepancy_rate}% discrepancy rate</span>
                      </div>
                    ) : (
                ...
                <div className="grid gap-3 sm:grid-cols-4">
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">On-Time Deliveries</p>
                  ...
                  <div>
                    ...
                    <p className={`text-sm font-semibold ${getScoreColor(supplier.quality_score)}`}>
                      {supplier.quality_score.toFixed(1)} / 5.0
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground mb-1">Discrepancy Rate</p>
                    <p className={`text-sm font-semibold ${supplier.discrepancy_rate > 5 ? 'text-red-600' : 'text-green-600'}`}>
