...
const AdministrativeOverviewBoardsPage = async () => {
  ...
  const [
    ...
    >(
      Prisma.sql`
        ...
        WHERE e.tenant_id = ${tenantId}::uuid
          AND e.deleted_at IS NULL
          AND e.event_date >= ${lastWeek.start}
          AND e.event_date <= ${lastWeek.end}
        ORDER BY e.event_date ASC, e.created_at ASC
      `
    ),
    database.$queryRaw<
    ...
    >(
      Prisma.sql`
        ...
        WHERE tenant_id = ${tenantId}::uuid
          AND deleted_at IS NULL
          AND created_at >= ${lastWeek.start}
          AND created_at <= ${lastWeek.end}
      `
    ),
    database.$queryRaw<Array<{ open_shifts: bigint }>>(
      Prisma.sql`
    ...
    database.$queryRaw<Array<{ alerts: bigint }>>(
      Prisma.sql`
    ...
    database.$queryRaw<Array<{ overdue: bigint }>>(
      Prisma.sql`
    ...
    database.$queryRaw<Array<{ avg_minutes: string | null }>>(
      Prisma.sql`
  ...
  const executiveActions = executiveTasks.map((task) => ({
    title: task.title,
    owner:
  ...
  const boardSnapshots = [
    {
      ...
      value: `${eventIssues.length} event${eventIssues.length === 1 ? "" : "s"} need review`,
      trend: `${eventIssues.length - lastWeekIssues >= 0 ? "+" : ""}${eventIssues.length - lastWeekIssues} vs. last week`,
