...
exports.calculateEventProfitability = calculateEventProfitability;
exports.getHistoricalProfitability = getHistoricalProfitability;
exports.getEventProfitabilityList = getEventProfitabilityList;
require("server-only");
const server_1 = require("@repo/auth/server");
const database_1 = require("@repo/database");
const tenant_1 = require("../../../../lib/tenant");
async function calculateEventProfitability(eventId) {
  ...
  const actualFoodCostResult = await database_1.database.$queryRawUnsafe(
    `
  ...
  const actualLaborCostResult = await database_1.database.$queryRawUnsafe(
    `
    SELECT COALESCE(SUM(
      CASE
  ...
  const marginTrendResult = await database_1.database.$queryRawUnsafe(
    `
    SELECT
...
async function getHistoricalProfitability(months = 12) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) {
    throw new Error("Unauthorized");
  }
...
async function getEventProfitabilityList(limit = 50) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) {
    throw new Error("Unauthorized");
  }
