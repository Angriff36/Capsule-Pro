...
const board_card_1 = require("./components/board-card");
const canvas_viewport_1 = require("./components/canvas-viewport");
const grid_layer_1 = require("./components/grid-layer");
const viewport_controls_1 = require("./components/viewport-controls");
...
function BoardCanvas({
  ...
  const { updateCursor, updateSelectedCard, updateDragging, clearPresence } =
    (0, collaboration_1.useCommandBoardPresence)();
  ...
  const handleAddCard = (0, react_1.useCallback)(async () => {
    ...
    if (result.success && result.card) {
      ...
      sonner_1.toast.success("Card added successfully");
      broadcast({ type: "CARD_ADDED", cardId: newCard.id });
    } else {
      sonner_1.toast.error(result.error || "Failed to add card");
    }
  ...
  const handlePositionChange = (0, react_1.useCallback)(
    async (cardId, position) => {
      ...
      if (result.success) {
        ...
        sonner_1.toast.error(result.error || "Failed to save card position");
      }
    },
  ...
  const handleDrag = (0, react_1.useCallback)(
    (e) => {
      ...
      const newX =
        dragStateRef.current.cardStartPosition.x + deltaX / state.viewport.zoom;
      const newY =
        dragStateRef.current.cardStartPosition.y + deltaY / state.viewport.zoom;
      setState((prev) => ({
        ...prev,
  ...
  const handleDragEnd = (0, react_1.useCallback)(async () => {
    ...
    if (result.success) {
      sonner_1.toast.success("Card position saved");
      broadcast({
        type: "CARD_MOVED",
  ...
  const handleDeleteCard = (0, react_1.useCallback)(
    async (cardId) => {
      ...
      if (result.success) {
        ...
        sonner_1.toast.success("Card deleted successfully");
        broadcast({ type: "CARD_DELETED", cardId });
      } else {
        sonner_1.toast.error(result.error || "Failed to delete card");
      }
    },
    [canEdit, broadcast]
  );
