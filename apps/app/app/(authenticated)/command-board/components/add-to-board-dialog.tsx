...
import { createEntityCard } from "../actions/entity-cards";
...
export function AddToBoardDialog({
  ...
  const handleAddToExistingBoard = async () => {
    ...
    try {
      const result = await createEntityCard(selectedBoardId, {
        entityType,
        entityId,
        title: "", // Will be fetched from entity
        content: undefined,
        position: { x: 100, y: 100, width: 280, height: 180, zIndex: 1 },
      });
  ...
  const handleCreateNewBoard = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.board) {
        // Add the entity card to the new board
        await createEntityCard(result.board.id, {
          entityType,
          entityId,
          title: "", // Will be fetched from entity
          content: undefined,
