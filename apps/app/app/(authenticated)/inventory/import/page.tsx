...
import { ArrowLeft, Download, FileUp, AlertCircle, CheckCircle2 } from "lucide-react";
...
const CSV_HEADERS = ["item_number", "name", "category", "unit_cost", "quantity_on_hand", "reorder_level", "tags", "fsa_status"];
...
export default function InventoryImportPage() {
  ...
  const handleFileSelect = useCallback((selectedFile: File) => {
    if (!selectedFile.name.endsWith(".csv")) {
      setResult({ success: 0, errors: [{ row: 0, message: "Please upload a CSV file" }] });
      return;
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult({ success: 0, errors: [{ row: 0, message: errorData.error || `HTTP ${res.status}` }] });
        return;
      }
      ...
      setResult({ success: 0, errors: [{ row: 0, message: err instanceof Error ? err.message : "Upload failed" }] });
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 pt-0">
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {CSV_HEADERS.filter((h) => h !== "item_number" && h !== "name").map((header) => (
                    <span
                      key={header}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {[
                    ...
                    "dry_goods", "frozen", "beverages", "supplies", "equipment", "other",
                  ].map((cat) => (
                    <span key={cat} className="rounded bg-blue-50 px-1.5 py-0.5 text-xs text-blue-700 dark:bg-blue-950 dark:text-blue-400">
                      {cat}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {["unknown", "requires_review", "compliant", "non_compliant", "exempt"].map((status) => (
                    <span key={status} className="rounded bg-green-50 px-1.5 py-0.5 text-xs text-green-700 dark:bg-green-950 dark:text-green-400">
                      {status.replace("_", " ")}
                    </span>
                  ))}
                </div>
      ...
      {result && (
        <Card className={result.success > 0 && result.errors.length === 0 ? "border-green-200 dark:border-green-900" : ""}>
          <CardHeader>
