...
export default async function PrepTaskPlanWorkflowsPage() {
	...
	const [
		...
		database.prepTaskPlanWorkflow.count({
			where: { ...baseWhere, status: { in: ["generating", "generation_completed"] } },
		}),
		database.prepTaskPlanWorkflow.count({
			where: { ...baseWhere, status: { in: ["reviewing", "review_completed"] } },
		}),
		database.prepTaskPlanWorkflow.count({
			...
			<CommandBand>
				<CommandBandHeader>
					<div className="space-y-4">
						...
						<CommandBandLede>
							Track prep task plan workflows from generation through approval
							to instantiation. Monitor lifecycle state, review generated tasks,
