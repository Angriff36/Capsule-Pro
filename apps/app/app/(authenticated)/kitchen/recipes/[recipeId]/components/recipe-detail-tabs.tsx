...
type RecipeTabKey = "nutrition" | "costing" | "history";

interface RecipeDetailTabsProps {
  recipe: RecipeDetailRow;
  recipeVersionId: string | null;
  // Restrict which panels render. Defaults to all three power features.
  tabs?: RecipeTabKey[];
  // "sections" lays the nutrition/costing/history panels out simultaneously
  // (side-by-side on wide screens) instead of hiding them behind tabs.
  variant?: "tabs" | "sections";
}

const ALL_RECIPE_TABS: RecipeTabKey[] = ["nutrition", "costing", "history"];
...
function CompareVersionsDialog({
  ...
  useEffect(() => {
    ...
    setCompareFrom((prev) => prev ?? first.id);
    setCompareTo((prev) => prev ?? versions[1]?.id ?? first.id);
  }, [versions]);
      ...
      <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-3xl">
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <p className="font-medium text-muted-foreground text-sm">From</p>
              <Select
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {compareData.changes.steps.added.length === 0 &&
                    compareData.changes.steps.removed.length === 0 &&
                    compareData.changes.steps.changed.length === 0 && (
                      <p className="text-muted-foreground">No step changes.</p>
                    )}
                </div>
              </div>
            </div>
...
const restoreButtonLabel = (isCurrent: boolean, isRestoring: boolean) => {
  if (isCurrent) {
    return "Current Version";
  }
  return isRestoring ? "Restoring..." : "Restore This Version";
};
...
function HistoryTabContent({
  ...
  const requestRestore = () => {
    if (!viewingDetail || viewingDetail.id === currentVersionId) {
      return;
    }
      ...
      try {
        const response = await apiFetch(
          kitchenRecipeCompositeRestore(recipeId),
          {
            ...
            body: JSON.stringify({ sourceVersionId: detail.id }),
          }
        );
    ...
    <>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Restore version {viewingDetail?.versionNumber}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              A new version will be created from this snapshot — nothing is
              deleted.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export function RecipeDetailTabs({
  ...
  // Everything visible at once — no tabs. Nutrition (narrow facts panel) and
  // costing share a row on wide screens; history spans below.
  if (variant === "sections") {
    return (
      <div className="space-y-6">
        <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
          {visibleTabs.includes("nutrition") && nutritionContent}
          {visibleTabs.includes("costing") && (
            <div className="space-y-4">{costingContent}</div>
          )}
        </div>
        {visibleTabs.includes("history") && historyContent}
      </div>
    );
  }
