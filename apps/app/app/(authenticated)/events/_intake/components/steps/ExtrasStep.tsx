...
const BAR_OPTIONS = [
  { value: 'none', label: 'No Bar Service', description: 'Food only', icon: <CircleOff className="w-4 h-4" /> },
  { value: 'beer-wine', label: 'Beer & Wine', description: 'Curated beer and wine selection', icon: <Beer className="w-4 h-4" /> },
  { value: 'full-bar', label: 'Full Bar', description: 'Standard spirits, beer, and wine', icon: <GlassWater className="w-4 h-4" /> },
  { value: 'premium-bar', label: 'Premium Bar', description: 'Top-shelf spirits and craft cocktails', icon: <Martini className="w-4 h-4" /> },
  { value: 'byob-service', label: 'BYOB Service', description: 'You supply, we bartend', icon: <Wine className="w-4 h-4" /> },
];
...
export default function ExtrasStep({ formData, updateField, toggleArrayItem }: Props) {
  return (
        ...
        <div className="flex flex-wrap gap-2">
          {RENTAL_OPTIONS.map(rental => (
            <ChipToggle
              ...
              onClick={() => toggleArrayItem('rentalsNeeded', RENTAL_KEYS[rental])}
            />
          ))}
        </div>
      </div>
