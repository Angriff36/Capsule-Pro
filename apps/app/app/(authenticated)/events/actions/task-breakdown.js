...
exports.generateTaskBreakdown = generateTaskBreakdown;
exports.saveTaskBreakdown = saveTaskBreakdown;
const openai_1 = require("@ai-sdk/openai");
const database_1 = require("@repo/database");
const ai_1 = require("ai");
const tenant_1 = require("../../../lib/tenant");
const AI_MODEL = "gpt-4o-mini";
async function generateTaskBreakdown({ eventId, customInstructions }) {
  ...
  const similarEvents = await database_1.database.$queryRaw(database_1.Prisma
    .sql`
...
Please generate a complete task breakdown following the system prompt guidelines. Return ONLY valid JSON, no markdown formatting.`;
  try {
    ...
    const now = Date.now();
    const transformTask = (task, section, index) => ({
 ...
 */
function getFallbackTasks(event, dishesData) {
...
async function saveTaskBreakdown(eventId, breakdown) {
  const tenantId = await (0, tenant_1.requireTenantId)();
  const allTasks = [
    ...breakdown.prep.map((t) => ({ ...t, taskType: "prep" })),
    ...breakdown.setup.map((t) => ({ ...t, taskType: "setup" })),
    ...breakdown.cleanup.map((t) => ({ ...t, taskType: "cleanup" })),
  ];
  const locationResult = await database_1.database.$queryRaw(database_1.Prisma
    .sql`
  ...
  for (const task of allTasks) {
    ...
    await database_1.database.prepTask.create({
      data: {
        tenantId,
        id: task.id,
        eventId,
