...
exports.createBatchAvailability = createBatchAvailability;
  ...
  const [availability, totalCount] = await Promise.all([
    database_1.database.$queryRaw(database_1.Prisma.sql`
        ...
        WHERE ea.tenant_id = ${tenantId}
          ...
          ${hasEmployeeId ? database_1.Prisma.sql`AND ea.employee_id = ${params.employeeId}` : database_1.Prisma.empty}
          ${hasDayOfWeek ? database_1.Prisma.sql`AND ea.day_of_week = ${params.dayOfWeek}` : database_1.Prisma.empty}
          ${hasEffectiveDate ? database_1.Prisma.sql`AND ${new Date(params.effectiveDate)} >= COALESCE(ea.effective_from, ${new Date(params.effectiveDate)}) AND ${new Date(params.effectiveDate)} <= COALESCE(ea.effective_until, ${new Date(params.effectiveDate)})` : database_1.Prisma.empty}
          ${hasIsActive ? database_1.Prisma.sql`AND ${new Date(params.effectiveDate)} >= COALESCE(ea.effective_from, ${new Date(params.effectiveDate)}) AND ${new Date(params.effectiveDate)} <= COALESCE(ea.effective_until, ${new Date(params.effectiveDate)})` : database_1.Prisma.empty}
 ...
 */
async function createAvailability(formData) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) throw new Error("Not authenticated");
  const tenantId = await (0, tenant_1.getTenantIdForOrg)(orgId);
  if (!tenantId) throw new Error("No tenant found");
  ...
  const effectiveUntil = formData.get("effectiveUntil");
  ...
  const [existingActive] = await database_1.database.$queryRaw(database_1.Prisma
    .sql`
      SELECT COUNT(*)::bigint
  ...
  const availability = await database_1.database.employee_availability.create({
    data: {
      tenant_id: tenantId,
  ...
  (0, cache_1.revalidatePath)("/scheduling/availability");
  return { availability };
}
/**
 * Update an existing availability record
 */
async function updateAvailability(availabilityId, formData) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) throw new Error("Not authenticated");
  const tenantId = await (0, tenant_1.getTenantIdForOrg)(orgId);
  if (!tenantId) throw new Error("No tenant found");
  ...
  const availability =
    await database_1.database.employee_availability.update(updateData);
  (0, cache_1.revalidatePath)("/scheduling/availability");
  return { availability };
}
/**
...
async function deleteAvailability(availabilityId) {
  ...
  const tenantId = await (0, tenant_1.getTenantIdForOrg)(orgId);
  if (!tenantId) throw new Error("No tenant found");
  await database_1.database.employee_availability.update({
    where: {
      tenant_id_id: {
        tenant_id: tenantId,
        id: availabilityId,
 ...
 */
async function createBatchAvailability(formData) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) throw new Error("Not authenticated");
  const tenantId = await (0, tenant_1.getTenantIdForOrg)(orgId);
  if (!tenantId) throw new Error("No tenant found");
  ...
  const [existingActive] = await database_1.database.$queryRaw(database_1.Prisma
    .sql`
      ...
      WHERE tenant_id = ${tenantId}
        AND employee_id = ${employeeId}
        AND day_of_week IN (${database_1.Prisma.join(daysArray)})
        AND deleted_at IS NULL
        AND effective_until IS NULL
    `);
  if (Number(existingActive.count) > 0) {
  ...
  const createdAvailability = await Promise.all(
    patterns.map(async (pattern) => {
      return database_1.database.employee_availability.create({
        data: {
          tenant_id: tenantId,
          employee_id: employeeId,
 ...
 */
async function getEmployeeAvailability(params) {
  const { orgId } = await (0, server_1.auth)();
  if (!orgId) throw new Error("Not authenticated");
  const tenantId = await (0, tenant_1.getTenantIdForOrg)(orgId);
  if (!tenantId) throw new Error("No tenant found");
  ...
  const employeesQuery =
    params.employeeIds && params.employeeIds.length > 0
      ? database_1.Prisma
          .sql`AND e.id IN (${database_1.Prisma.join(params.employeeIds)})`
  ...
  const availabilityByEmployee = availability.reduce((acc, avail) => {
  ...
  if (params.includeTimeOff) {
    timeOffRequests = await database_1.database.$queryRaw(database_1.Prisma.sql`
        SELECT
          tor.id,
          tor.employee_id,
          tor.start_date,
