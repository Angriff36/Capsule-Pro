...
exports.TimeOffForm = TimeOffForm;
...
function TimeOffForm({ request, onSuccess, onCancel }) {
  const router = (0, navigation_1.useRouter)();
  const isEditing = Boolean(request?.id);
  const [employees, setEmployees] = (0, react_1.useState)([]);
  const [loading, setLoading] = (0, react_1.useState)(true);
  const [submitting, setSubmitting] = (0, react_1.useState)(false);
  const [checkingConflicts, setCheckingConflicts] = (0, react_1.useState)(
    false
  );
  // Form state
  ...
  (0, react_1.useEffect)(() => {
    async function loadData() {
      setLoading(true);
      try {
        const data = await (0, actions_1.getEmployees)();
        setEmployees(data.employees || []);
      } catch (error) {
        sonner_1.toast.error("Failed to load employees", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await (0, actions_1.createTimeOffRequest)(submitData);
      ...
      sonner_1.toast.error("Failed to create time-off request", {
        description: error instanceof Error ? error.message : "Unknown error",
      });
    } finally {
      setSubmitting(false);
  ...
  return (
    <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
      <div className="grid gap-6 md:grid-cols-2">
          ...
          >
            ...
            <select_1.SelectContent>
              {employees.map((employee) => (
                <select_1.SelectItem key={employee.id} value={employee.id}>
        ...
        {/* Request Type */}
        <div className="flex flex-col gap-2">
          <label_1.Label htmlFor="requestType">Request Type *</label_1.Label>
          <select_1.Select
            onValueChange={(value) =>
          ...
          >
            <select_1.SelectTrigger id="requestType">
              <select_1.SelectValue placeholder="Select request type" />
            </select_1.SelectTrigger>
            <select_1.SelectContent>
              {actions_1.timeOffTypes.map((type) => (
        ...
        {/* Duration Display */}
        <div className="flex flex-col gap-2">
          <label_1.Label>Duration</label_1.Label>
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2">
            ...
            />
            <label_1.Label htmlFor="partialDay">Partial Day</label_1.Label>
          </div>
