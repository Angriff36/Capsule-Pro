...
export function useUpdatePOItemQuantity() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      itemId: string;
      request: UpdateQuantityReceivedRequest;
    }) => updatePurchaseOrderItemQuantity(poId, itemId, request),
...
export function useUpdatePOItemQuality() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      poId,
      ...
      itemId: string;
      request: UpdateQualityStatusRequest;
    }) => updatePurchaseOrderItemQuality(poId, itemId, request),
    onSuccess: (_data, { poId }) => {
      queryClient.invalidateQueries({ queryKey: purchaseOrderKeys.detail(poId) });
      queryClient.invalidateQueries({ queryKey: purchaseOrderKeys.lists() });
    },
...
/** Complete receiving */
export function useCompletePOReceiving() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({
      poId,
      ...
      poId: string;
      request: CompleteReceivingRequest;
    }) => completePurchaseOrderReceiving(poId, request),
    onSuccess: (_data, { poId }) => {
      queryClient.invalidateQueries({ queryKey: purchaseOrderKeys.detail(poId) });
      queryClient.invalidateQueries({ queryKey: purchaseOrderKeys.lists() });
    },
