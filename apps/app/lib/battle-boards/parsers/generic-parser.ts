...
export function parseGenericPdf(lines: string[]): ParsedDocumentResult {
  ...
  for (const line of lines) {
    ...
    if (kvMatch) {
      ...
      if (key.includes('client') || key.includes('customer') || key.includes('company')) {
        meta.event_name = value;
      } else if (key.includes('invoice') || key.includes('event number') || key.includes('job')) {
        meta.event_number = value;
      } else if (key.includes('date') && !key.includes('update')) {
        try {
          const d = new Date(value);
          if (!isNaN(d.getTime())) meta.event_date = d.toISOString().split('T')[0];
        } catch { /* skip */ }
      } else if (key.includes('venue') || key.includes('location')) {
        meta.venue_name = value;
      } else if (key.includes('address')) {
        meta.venue_address = value;
      } else if (key.includes('guest') || key.includes('headcount') || key.includes('attendee')) {
        const num = parseInt(value, 10);
        if (!isNaN(num)) meta.headcount = num;
