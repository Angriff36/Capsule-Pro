...
async function main() {
  ...
  const passwords = ["testpassword123", "Test12345!", "password123", "Clerk123!"];
  ...
  if (!page.url().includes("sign-in")) {
    ...
    const clerkSession = cookies.filter(c => c.domain.includes('capsule') && c.name.toLowerCase().includes('session'));
    console.log("Session cookies:", clerkSession.map(c => c.name));
