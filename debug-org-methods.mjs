...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    if (s2.status !== "complete" || !s2.createdSessionId) return { error: s2.status };
    
    await c.setActive({ session: s2.createdSessionId });
    
    // Switch to org that has platform.accounts
    await c.setActiveOrganization({ organizationId: "org_38BryCr5yDMDLvASQfW7cHl824Q" });
    
    return { success: true, orgId: c.organization?.id };
  });
