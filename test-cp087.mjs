...
async function main() {
  ...
  try {
    ...
    if (stations[0]) {
      console.log("  Sample station keys:", Object.keys(stations[0]).join(", "));
      console.log("  Sample station:", JSON.stringify(stations[0])?.substring(0, 100));
    }

    // Get tasks by station
    if (stations.length > 0) {
      const stationId = stations[0].id;
      const tasksByStationR = await api(page, "GET", "/api/kitchen/stations/" + stationId + "/tasks");
      console.log("K1: Tasks by station:", tasksByStationR.status, "count:", tasksByStationR.data?.tasks?.length || 0);
    }
    ...
    if (taskId) {
      const claimR = await api(page, "POST", "/api/kitchen/tasks/" + taskId + "/claim", {});
      console.log("K2: Claim task:", claimR.status, claimR.status === 200 ? "✓ PASS" : "✗ " + claimR.status, JSON.stringify(claimR.data)?.substring(0, 80));

      // Unclaim/release task
      const releaseR = await api(page, "POST", "/api/kitchen/tasks/" + taskId + "/release", {});
      console.log("K2: Release task:", releaseR.status, releaseR.status === 200 ? "✓ PASS" : "✗ " + releaseR.status, JSON.stringify(releaseR.data)?.substring(0, 80));
    }
    ...
    console.log("K3: My tasks (my-tasks endpoint):", myTasksR.status, myTasksR.status === 200 ? "✓ PASS" : "✗ " + myTasksR.status, "count:", myTasksR.data?.tasks?.length || 0);

    // Available tasks (for my work queue)
    const availableR = await api(page, "GET", "/api/kitchen/tasks/available");
    console.log("K3: Available tasks:", availableR.status, availableR.status === 200 ? "✓ PASS" : "✗ " + availableR.status, "count:", availableR.data?.tasks?.length || 0);

    // Start a task
    if (taskId) {
      const startR = await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, { status: "in_progress" });
      console.log("K3: Start task:", startR.status, startR.status === 200 ? "✓ PASS" : "✗ " + startR.status);

      // Complete a task
      const completeR = await api(page, "PATCH", "/api/kitchen/tasks/" + taskId, { status: "done" });
      console.log("K3: Complete task:", completeR.status, completeR.status === 200 ? "✓ PASS" : "✗ " + completeR.status, JSON.stringify(completeR.data)?.substring(0, 80));
      ...
      console.log("K3: Done tasks count:", doneCount, doneCount > 0 ? "✓ PASS" : "⚠ 0");
    }

    console.log("\n" + "=".repeat(60));
    console.log("📊 cp-087 — K1-3: Kitchen view workflow — RESULT");
    console.log("=".repeat(60));
    console.log("K1 — Station list:         " + (stationsR.status === 200 ? "✓ PASS (" + stations.length + " stations)" : "✗ " + stationsR.status));
    console.log("K1 — Tasks by station:     " + (tasksByStationR?.status === 200 ? "✓ PASS" : "⚠ " + (tasksByStationR?.status || "N/A")));
    console.log("K2 — Claim task:           " + (claimR?.status === 200 ? "✓ PASS" : "✗ " + (claimR?.status || "N/A")));
    console.log("K2 — Release task:         " + (releaseR?.status === 200 ? "✓ PASS" : "✗ " + (releaseR?.status || "N/A")));
    console.log("K3 — My tasks:            " + (myTasksR.status === 200 ? "✓ PASS" : "✗ " + myTasksR.status));
    console.log("K3 — Start task:          " + (startR?.status === 200 ? "✓ PASS" : "✗ " + (startR?.status || "N/A")));
    console.log("K3 — Complete task:       " + (completeR?.status === 200 ? "✓ PASS" : "✗ " + (completeR?.status || "N/A")));
    console.log("=".repeat(60));
