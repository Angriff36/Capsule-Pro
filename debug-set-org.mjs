...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    if (s2.status === "complete" && s2.createdSessionId) {
      // Try setOrganization via the organizationSwitcher
      // In Clerk v5, use setActive with organizationId
      await c.setActive({
        session: s2.createdSessionId,
        organizationId: "org_38BryCr5yDMDLvASQfW7cHl824Q",
      });
      return { success: true };
    }
