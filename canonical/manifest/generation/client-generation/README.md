...
The biggest lever is #1: **upgrade `@angriff36/manifest`** to a version that ships react-query (with the upstream D23 route/envelope overrides, commit `6a7db4b`) and fixes `ts.types`/`ts.client`. That single upgrade would let us delete the custom types post-processing, the custom client, and the custom hooks — and domain routing then becomes a projection config, not a parallel stack.
...
| Q006 | **Upgrade `@angriff36/manifest`** to ship react-query + fix `ts.types`/`ts.client`, then delete the custom generators? | Biggest single lever — removes most custom glue (types post-process, client, hooks). | Verified: 2.18.0 ships none of it; react-query projection + D23 overrides are upstream/future. | A: upgrade + migrate to documented surfaces (delete custom stack); B: stay on 2.18.0 + keep custom; C: upgrade incrementally, retire one generator at a time | NEEDS-RYAN |
