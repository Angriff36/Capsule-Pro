...
features.forEach((feature) => {
  ...
  try {
    ...
    const matchingApiFiles = apiFiles.filter(
      (file) =>
        typeof file === "string" &&
    ...
    if (matchingApiFiles.length > 0) {
      result.api_files = matchingApiFiles.map((f) =>
        path.join(searchLocations.api, f)
      );
  ...
  try {
    ...
    const matchingUiFiles = uiFiles.filter(
      (file) =>
        typeof file === "string" &&
        (file.toLowerCase().includes(feature.replace(/-/g, "-")) ||
          file.toLowerCase().includes(feature.replace(/-/g, "_")))
    ...
    if (matchingUiFiles.length > 0) {
      result.ui_files = matchingUiFiles.map((f) =>
        path.join(searchLocations.ui, f)
      );
      result.status = result.status === "missing" ? "partial" : "survived";
    }
  } catch (e) {}
  ...
  try {
    const manifestFiles = fs.readdirSync(searchLocations.manifest, {
      recursive: true,
    });
    const matchingManifestFiles = manifestFiles.filter(
      (file) =>
        typeof file === "string" &&
        (file.toLowerCase().includes(feature.replace(/-/g, "-")) ||
          file.toLowerCase().includes(feature.replace(/-/g, "_")))
    ...
    if (matchingManifestFiles.length > 0) {
      result.manifest_files = matchingManifestFiles.map((f) =>
        path.join(searchLocations.manifest, f)
      );
      result.status = result.status === "missing" ? "partial" : "survived";
    }
  } catch (e) {}
  ...
  // Check test files
  try {
    const testFiles = fs.readdirSync(searchLocations.tests, {
      recursive: true,
    });
    ...
    if (matchingTestFiles.length > 0) {
      result.test_files = matchingTestFiles.map((f) =>
        path.join(searchLocations.tests, f)
      );
      result.status = result.status === "missing" ? "partial" : "survived";
    }
  } catch (e) {}
