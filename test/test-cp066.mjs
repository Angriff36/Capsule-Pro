  ...
  try {
    ...
    for (const status of STATUSES) {
      ...
      console.log(
        `  Create ${status} task:`,
        r.status,
        r.status === 200 ? "✓" : JSON.stringify(r.data)?.substring(0, 60)
      );
    }
    ...
    for (const status of STATUSES) {
      ...
      console.log(
        ...
        r.status,
        `total=${tasks.length}, ours=${ours.length}`
      );
    }
    ...
    console.log(
      "  Filter minPriority=3:",
      priR.status,
      `total=${priTasks.length}, ours=${priOurs.length}`
    );

    // === J2: Combined filters ===
    console.log("\n--- J2: Combined Filters ---");
    const combinedR = await api(
      page,
      "GET",
      "/api/kitchen/tasks?status=pending&minPriority=2"
    );
    console.log(
      "  Filter status=pending&minPriority=2:",
      combinedR.status,
      `count=${combinedR.data?.tasks?.length || 0}`
    );

    // === J2: URL persistence (browser) ===
    console.log("\n--- J2: URL Persistence ---");
    // Visit tasks page with filters in URL
    await page.goto(`${BASE}/tasks?status=pending&minPriority=2`, {
      waitUntil: "networkidle",
      timeout: 30_000,
    });
    ...
    console.log(
      ...
      urlAfterReload.includes("status=pending") ? "✓" : "✗"
    );
    ...
    const employees =
      employeesR.data?.staff || employeesR.data?.employees || [];
    console.log("  Employees:", employeesR.status, "count:", employees.length);
    const empId = employees[0]?.id;
    if (empId) {
      const assigneeR = await api(
        page,
        "GET",
        `/api/kitchen/tasks?assignee=${empId}`
      );
      console.log(
        "  Filter by assignee:",
        assigneeR.status,
        `count=${assigneeR.data?.tasks?.length || 0}`
      );
    } else {
      // Try claim endpoint
      ...
      console.log(
        "  my-tasks:",
        claimR.status,
        `count=${claimR.data?.tasks?.length || 0}`
      );
    }
    ...
    console.log(
      ...
      dateR.status,
      `count=${dateR.data?.tasks?.length || 0}`
    );

    // Summary
    const allStatusFiltered = Object.values(filterResults).every(
      (r) => r.ours >= 1
    );
    console.log("\n" + "=".repeat(60));
    console.log(
      "📊 cp-066 — J2: Filter tasks by status/assignee/date — RESULT"
    );
    ...
    STATUSES.forEach((s) => {
      ...
      console.log(
        `  Filter ${s}: ` + (r?.ours >= 1 ? `✓ PASS (${r.ours})` : "✗ FAIL (0)")
      );
    });
    console.log(
      "J2 — Priority Filter:  " + (priOurs >= 1 ? "✓ PASS" : "✗ FAIL (0)")
    );
    console.log(
      "J2 — Combined Filter:  " +
        (combinedR.data?.tasks?.length >= 0
          ? "✓ PASS"
          : "✗ " + combinedR.status)
    );
    console.log(
      "J2 — URL Persistence:   " +
        (urlHasStatus && urlHasPriority ? "✓ PASS" : "⚠ partial")
    );
    console.log(
      "J2 — Assignee Filter:  " +
        (empId ? "✓ API supports" : "⚠ no employees found")
    );
    console.log(
      "J2 — Date Filter:      " +
        (dateR.data?.tasks ? "✓ PASS" : "✗ " + dateR.status)
    );
    console.log("=".repeat(60));
  } finally {
    await browser.close();
