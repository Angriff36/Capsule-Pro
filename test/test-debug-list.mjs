...
async function main() {
  ...
  try {
    ...
    const eventCardTexts = await page
    ...
    // ── Search page ────────────────────────────────────────────────
    console.log("\n🔍 Search page:");
    await page.goto(`${BASE}/search?q=${encodeURIComponent(LATEST)}`, {
      waitUntil: "networkidle",
      timeout: 15_000,
    });
    await page.waitForTimeout(3000);
    ...
    if (latestEvent) {
      console.log(
        "   Latest event exists in API:",
        latestEvent.id,
        latestEvent.title
      );

      // Check event detail page
      await page.goto(`${BASE}/events/${latestEvent.id}`, {
        waitUntil: "networkidle",
        timeout: 15_000,
      });
