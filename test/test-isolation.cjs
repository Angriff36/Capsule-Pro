...
// Test cases
const testFile =
  "C:\\Projects\\capsule-pro\\apps\\api\\app\\api\\user-preferences\\route.ts";
...
const matching = exemptions.find((e) => {
  ...
  return (
    normalizedFile.endsWith(exemptionNormalized) && e.methods.includes("POST")
  );
});
...
const userPrefsExemption = exemptions.find((e) =>
  e.path.includes("user-preferences")
);
console.log("User preferences exemption path:", userPrefsExemption.path);
console.log(
  "Exemption normalized:",
  userPrefsExemption.path.replace(/\\/g, "/")
);
console.log("");
...
console.log(
  "endsWith check:",
  normalizedFile.endsWith(userPrefsExemption.path.replace(/\\/g, "/"))
);
console.log(
  "methods includes POST:",
  userPrefsExemption.methods.includes("POST")
);
