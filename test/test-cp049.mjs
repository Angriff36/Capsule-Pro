...
async function main() {
  ...
  try {
    ...
    console.log(
      `    Connections count: ${listR.data?.commandBoardConnections?.length ?? "?"}`
    );
    const existingConnections = listR.data?.commandBoardConnections || [];
    console.log(
      `    Status: ${listR.status === 200 && existingConnections.length === 0 ? "✓ PASS (empty list)" : "⚠ EXISTS (" + existingConnections.length + ")"}`
    );

    // Try create connection
    const createR = await apiFetch(
      page,
    ...
    console.log(
      `    ${createR.status === 404 ? "✗ NOT DEPLOYED (404)" : createR.status === 200 ? "✓ PASS" : "⚠ " + createR.status}`
    );
    const createdId = createR.data?.result?.id || null;

    // Try delete connection
    if (createdId) {
      const deleteR = await apiFetch(
        page,
        `/api/command-board/connections/${createdId}`,
        "DELETE"
      );
      ...
      const deleteR = await apiFetch(
        page,
        `/api/command-board/connections/${fakeId}`,
        "DELETE"
      );
    ...
    const replayGetR = await apiFetch(
      page,
      `/api/command-board/${BOARD_ID}/replay`
    );
    console.log("  GET /{boardId}/replay:");
    console.log(`    Status: ${replayGetR.status}`);
    console.log(`    Events: ${replayGetR.data?.data?.events?.length ?? "?"}`);
    console.log(
      `    ${replayGetR.status === 200 ? "✓ PASS (read-only replay)" : "✗ FAIL"}`
    );

    // POST replay (undo/redo)
    const replayPostR = await apiFetch(
      page,
      `/api/command-board/${BOARD_ID}/replay`,
      "POST",
      { action: "undo" }
    ...
    console.log(
      `    ${replayPostR.status === 405 ? "✗ READ-ONLY (405) — undo not implemented" : replayPostR.status === 200 ? "✓ PASS" : "⚠ " + replayPostR.status}`
    );
    ...
    await page.goto(`${BASE}/command-board/${BOARD_ID}`, {
    ...
    console.log(
      `    ${uiHtml.includes("Oops") || uiHtml.includes("500") ? "✗ BLOCKED — 500 (Clerk orgs=[])" : "✓ LOADED"}`
    );
