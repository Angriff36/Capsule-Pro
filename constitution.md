...
## 21. Intent-First UX and System-Derived Fields

Users and AI agents provide business intent only. The system derives implementation fields.

This section is binding for:

* UI forms
* AI chat actions and tool calls
* API `transformBody` code
* Manifest commands (parameters, guards, mutates)
* Prisma adapters and stores
* tests and test fixtures

### User-supplied fields

User-supplied fields are values a normal user actually knows and intentionally chooses.

Examples:

* event name
* date or time the user picks
* source location and destination location
* items and quantities
* notes and free-form text the user types
* guest count
* selected client
* selected venue
* the requested action (create, submit, approve, cancel, receive)

A user-supplied field must be something the user can answer without inspecting the database, the manifest IR, the Prisma schema, or the runtime context.

### System-derived fields

System-derived fields must not be required from users or from AI chat. The system must derive them in `transformBody`, runtime command context, manifest adapters, or stores.

System-derived fields include:

* `tenantId`
* `userId`, `requestedBy`, `createdBy`, `updatedBy`
* `createdAt`, `updatedAt`
* sequence-derived identifiers such as `transferNumber`, `orderNumber`, `invoiceNumber`
* `status` and lifecycle state
* `versionNumber` where it is sequence-derived
* `slug` where it is title-derived
* audit metadata
* `correlationId`, `causationId`
* default workflow state

The single exception is a system-derived field that has no reliable derivation source. In that case the derivation gap must be documented in the manifest source, the adapter, or the entity's schema-drift allowlist entry, with an owner and a removal plan. "Ask the user" is not a substitute for documenting a missing derivation.

### Manifest guards

Manifest guards must validate business intent — the values the user actually chose.

Manifest guards must not force users to provide internal database plumbing as a precondition for command execution. A guard that requires `tenantId != null` from the request body is wrong: the runtime context already carries tenant.

### Workflow initial state

Initial workflow state is chosen by the command or the store. Users do not pick raw internal mutation states.

Users trigger actions: `create`, `submit`, `approve`, `cancel`, `receive`, and similar. The command decides the resulting state.

The only exception is when the product deliberately exposes raw state selection as an admin control. That exposure must be intentional, scoped to admin surfaces, and documented.

### AI chat actions

The AI action layer must ask for the minimum missing business facts.

It must not ask the user for fields that can be derived from:

* the authenticated request context (tenant, actor, organization, session)
* chat or conversation state
* defaults declared in the manifest
* currently selected records (selected client, selected venue, selected event)
* sequence generators (numbers, slugs)
* runtime adapters and stores

If the AI is missing only system-derivable values, it must derive them. It must not paper over a missing derivation by asking the user.

### Prisma-required fields that are not user-meaningful

If a Prisma column is required by the database but is not user-meaningful, the fix is not to add it to the UI form or the chat prompt.

The fix is to derive the value in:

* `transformBody` for the API route
* the runtime command context
* the manifest adapter or store

The derivation must be documented truthfully in the adapter file header, the manifest source, or the entity's documentation. A derivation that silently fabricates a value is not honest.

### Schema-drift fixes

A schema-drift fix is invalid if it makes audits green by moving a system-derived field into manual user input.

Moving `transferNumber`, `requestedBy`, `tenantId`, `createdAt`, `status`, or any other system-derived field into the UI form, the chat prompt, or the API request body is not a fix. It is a regression dressed as a fix.

The correct close for a schema-drift violation on a system-derived field is one of:

* derive the value in a store or adapter and update the schema-drift allowlist with the derivation source
* document the field as non-derivable, name the gap, and flag the entity as nonconforming until a derivation source exists

### Examples

**InventoryTransfer create.** The user supplies source location, destination location, items, quantities, and optional notes. The system derives `transferNumber`, `requestedBy`, `tenantId`, `createdAt`, `updatedAt`, initial status, and item persistence. The create command must not require `transferNumber` or `requestedBy` from the request body.

**Event creation via AI chat.** The AI must not ask the user for thirty database fields. It must ask for the smallest set of business facts — typically event name, date or time, client, venue, guest count, and a short list of menu or task selections — and derive the rest from runtime context, defaults, sequence generators, and adapters.
