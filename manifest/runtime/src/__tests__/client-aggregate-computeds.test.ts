/**
 * Client aggregate rollup computeds — compiled-IR regression lock.
 *
 * WHY this matters (not just WHAT it does):
 *
 * `crm/client-rules.manifest` declares two runtime-only (request-cached) aggregate
 * computeds that roll up a Client's lifetime payment history (LTV basis):
 ...
 *     payment attempts, overstating cash actually collected from the client —
 ...
 * when any of those drift, so client LTV inputs can't degrade unnoticed. The
 * predicate is deliberately identical to Event.totalPaidAmount's COMPLETED filter.
 *
...
describe("Client aggregate computeds — compiled IR (regression lock)", () => {
