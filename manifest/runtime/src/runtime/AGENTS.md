...
divergences: D9, D12, D13
pages: language/modules
note: >-
...
# Manifest reference for `manifest/runtime/src/runtime`

**Why this file:** loadManifests merge path: load the single committed kitchen.ir.json (already merged with sagas+reactions) or use native mergeIR(irs); make merges schema-driven so no IR section is silently dropped; inherit the real compilerVersion instead of hardcoding it.

**Relevant divergences:** D9, D12, D13 — see `manifest/MANIFEST-DIVERGENCES.md` for the full remediation detail.
