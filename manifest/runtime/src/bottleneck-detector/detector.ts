...
const DEFAULT_DETECTION_RULES: DetectionRule[] = [
  // Kitchen throughput bottlenecks
  {
    ...
    category: "throughput" as BottleneckCategory,
    type: "station_overload" as BottleneckType,
    enabled: true,
    severity: "high" as BottleneckSeverity,
    threshold: {
  ...
  },
  {
    ...
    category: "capacity" as BottleneckCategory,
    type: "prep_task_backlog" as BottleneckType,
    enabled: true,
    severity: "medium" as BottleneckSeverity,
    threshold: {
  ...
  },
  {
    ...
    category: "efficiency" as BottleneckCategory,
    type: "process_delay" as BottleneckType,
    enabled: true,
    severity: "medium" as BottleneckSeverity,
    threshold: {
  ...
  // Inventory bottlenecks
  {
    ...
    category: "resource" as BottleneckCategory,
    type: "stockout_risk" as BottleneckType,
    enabled: true,
    severity: "high" as BottleneckSeverity,
    threshold: {
  ...
  // Scheduling bottlenecks
  {
    ...
    category: "capacity" as BottleneckCategory,
    type: "overtime_spike" as BottleneckType,
    enabled: true,
    severity: "medium" as BottleneckSeverity,
    threshold: {
  ...
  },
  {
    ...
    category: "capacity" as BottleneckCategory,
    type: "insufficient_staff" as BottleneckType,
    enabled: true,
    severity: "high" as BottleneckSeverity,
    threshold: {
  ...
  // Event bottlenecks
  {
    ...
    category: "process" as BottleneckCategory,
    type: "lead_time_overflow" as BottleneckType,
    enabled: true,
    severity: "high" as BottleneckSeverity,
    threshold: {
...
const DEFAULT_CONFIG: BottleneckDetectorConfig = {
  enabled: true,
  sampleRate: 1.0,
...
export class BottleneckDetector {
  ...
  async analyze(
    ...
    const metricsData = await this.fetchAllMetrics(
      tenantId,
      startDate,
      endDate,
      locationId
  ...
  async detectByCategory(
    ...
    const categoryRules = this.config.rules.filter(
      (r) => r.enabled && r.category === category
    );
   ...
   */
  private async fetchStationMetrics(
    tenantId: string,
    startDate: Date,
    endDate: Date,
    locationId?: string
    ...
    >(
      ...
      WHERE pli.tenant_id = $1
        ...
        ${locationId ? "AND EXISTS (SELECT 1 FROM tenant_kitchen.stations s WHERE s.id::text = pli.station_id AND s.tenant_id = pli.tenant_id AND s.location_id = $4)" : ""}
   ...
   */
  private async fetchPrepTaskMetrics(
    tenantId: string,
    startDate: Date,
    endDate: Date,
    locationId?: string
   ...
   */
  private async fetchInventoryMetrics(
    tenantId: string,
    startDate: Date,
    endDate: Date,
    locationId?: string
    ...
    const wastePercentage =
      totalInventory > 0 && wasteData._sum.quantity
    ...
    return [
      ...
      {
        metric: "waste_percentage" as const,
  ...
  private async fetchSchedulingMetrics(
    ...
    const timeEntries = await this.prisma.$queryRawUnsafe<
      Array<{ regular_hours: string; overtime_hours: string }>
    >(
      `
      SELECT
        SUM(CASE
    ...
    const overtimePercentage =
      regularHours > 0 ? (overtimeHours / regularHours) * 100 : 0;

    return [
      {
        metric: "unfilled_shifts" as const,
      ...
      {
        metric: "overtime_percentage" as const,
        entity: { type: "scheduling", id: tenantId, name: "All Schedules" },
   ...
   * Fetch event metrics
   */
  private async fetchEventMetrics(
    tenantId: string,
    startDate: Date,
    endDate: Date,
    locationId?: string
  ...
  private async runDetectionRule(
    rule: DetectionRule,
    metricsData: Map<string, TimeSeriesData[]>,
    tenantId: string
  ): Promise<Bottleneck[]> {
    const bottlenecks: Bottleneck[] = [];
    ...
    for (const series of relevantMetrics) {
      ...
      if (isBreached) {
        const percentOverThreshold =
          rule.threshold.operator === "lt"
        ...
        bottlenecks.push({
          ...
          description: this.generateBottleneckDescription(
            rule,
            series,
            latestValue,
            threshold
  ...
  private generateRuleBasedSuggestion(
    ...
    const suggestionTemplates: Partial<
      Record<
        BottleneckType,
        Omit<
          ImprovementSuggestion,
          "id" | "bottleneckId" | "createdAt" | "aiGenerated"
        >
      >
    > = {
      station_overload: {
        type: "resource_reallocation" as SuggestionType,
        priority: "high" as SuggestionPriority,
        title: "Redistribute station workload",
        description:
          "Consider redistributing tasks from overloaded stations to underutilized ones or adjusting prep schedules.",
        reasoning:
          "Balancing workload across stations can improve overall throughput and reduce bottlenecks.",
        estimatedImpact: {
          area: "Throughput",
      ...
      prep_task_backlog: {
        type: "capacity_expansion" as SuggestionType,
        priority: "high" as SuggestionPriority,
        title: "Address prep task backlog",
        description:
          "Consider adding temporary staff, extending shift hours, or prioritizing critical tasks.",
        reasoning:
          "A growing backlog can lead to missed deadlines and quality issues.",
        estimatedImpact: {
          area: "Capacity",
      ...
      process_delay: {
        type: "process_change" as SuggestionType,
        priority: "medium" as SuggestionPriority,
        title: "Streamline preparation process",
        description:
          "Review and optimize the preparation workflow to reduce completion time.",
        reasoning:
          "Extended preparation times indicate potential process inefficiencies.",
        estimatedImpact: {
          area: "Efficiency",
      ...
      stockout_risk: {
        ...
        description:
          "Place urgent orders for items with low stock to prevent stockouts.",
        reasoning:
          "Stockouts can disrupt operations and impact customer satisfaction.",
        estimatedImpact: {
          area: "Reliability",
      ...
      insufficient_staff: {
        type: "resource_reallocation" as SuggestionType,
        priority: "urgent" as SuggestionPriority,
        title: "Fill unfilled shift positions",
        description:
          "Address unfilled shifts through hiring, overtime, or schedule adjustments.",
        reasoning:
          "Unfilled shifts reduce capacity and impact service quality.",
        estimatedImpact: {
          area: "Capacity",
      ...
      overtime_spike: {
        type: "scheduling_adjustment" as SuggestionType,
        priority: "medium" as SuggestionPriority,
        title: "Reduce overtime dependency",
        description:
          "Review schedules and adjust staffing levels to reduce overtime usage.",
        reasoning:
          "High overtime increases costs and can lead to staff burnout.",
        estimatedImpact: {
          area: "Cost",
      ...
      lead_time_overflow: {
        ...
        description:
          "Streamline the event planning process to reduce preparation time.",
  ...
  ): BottleneckAnalysis["summary"] {
    const bySeverity: Record<string, number> = {};
    const byCategory: Record<string, number> = {};
    const affectedEntities: Record<
      string,
  ...
  private calculateHealthScore(
    bottlenecks: Bottleneck[]
  ): BottleneckAnalysis["healthScore"] {
    const byCategory: Record<string, number> = {};
    ...
    const overall =
      Object.values(byCategory).reduce((sum, val) => sum + val, 0) /
   ...
   */
  private createEmptyAnalysis(tenantId: string): BottleneckAnalysis {
    return {
      tenantId,
      analysisPeriod: {
        start: new Date(),
      ...
      summary: {
        total: 0,
        bySeverity: {} as Record<BottleneckSeverity, number>,
   ...
   */
  private calculateStartDate(window: string): Date {
    const date = new Date();
