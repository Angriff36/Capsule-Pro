...
**How it was produced:** an 8-dimension audit of `capsule-pro` against the `@angriff36/manifest` source, where every candidate finding was **adversarially re-verified** against the *actually-installed* Manifest version (**2.4.2**, confirmed in `pnpm-lock.yaml`) before being kept. Findings that turned out to be thin wrappers, legitimate infrastructure, or already-correct usage were dropped (see the *Verified NOT Divergences* appendix so they are not re-flagged).
...
- **native?** — `yes` the native capability was confirmed to exist in 2.4.2 · `upstream` the native path needs a Manifest change first · `n/a` not a native-capability question (audit/hygiene).
...
| D9 | IR merge | Runtime re-merge silently DROPS sagas & reactions | MEDIUM | confirmed | upstream |
...
| D25 | Projection fork | Deep-imports into `dist/` for unexported projections | MEDIUM | confirmed | upstream |
...
### D9 — Runtime IR re-merge silently DROPS sagas & reactions · `MEDIUM` · confirmed · native? upstream
...
2. **Official Manifest method.** In the **test-only** `compileManifestSet` helper, replace the hand loop + `validateNoDuplicates` + flat-map with `compileProjectToIR({ entries, host })` (trivial `ResolverHost` = `{ readFile: fs.promises.readFile, resolvePath: path.resolve, fileExists: … }`) — deletes the duplicated dedupe and the `'2.2.0'` hardcode (it stamps the real `COMPILER_VERSION` + `irHash`). **Same `tenant` blocker as D10/D11 applies:** `compileProjectToIR` rejects capsule's per-file `tenant` declarations as duplicate-tenant errors, so this swap only works once tenant declarations are consolidated or native reconciliation lands — until then `validateNoDuplicates` is doing tenant-tolerant work the native path won't. Leave the **production precompiled-JSON reader** (`loadPrecompiledIR`, `loadMergedPrecompiledIR`, `findRepoRoot`) as-is for now: it merges precompiled shards, which `compileProjectToIR` (source-only) cannot do and for which **no native merge is exported**. To remove `mergeIrDocuments` too, file an upstream request to export `mergeIRs` (currently private). **This overlaps D9** — D9 is the correctness bug (dropped sections), this is the reinvention.
...
### D25 — Deep-imports into `dist/` for unexported projections · `MEDIUM` · confirmed · native? upstream
...
2. **Official Manifest method.** Two tracks. **(A) Upstream:** add the four missing subpaths to Manifest `package.json` exports (`./projections/{kysely,analytics,llm-context,materialized-views}` → existing `dist` targets) and cut a patch release; then capsule switches to `await import('@angriff36/manifest/projections/<name>')` like `generate-mermaid.mjs`/`generate-openapi.mjs` already do, deleting the path construction. **(B) Independent of upstream:** fix the three exported projections that needlessly deep-import to use the stable subpath now. No emitter logic is reinvented — purely an import-stability fix.
...
**Upstream Manifest changes required first (else migration won't yield a working system):**
- Add `sagas` to `multi-compiler.ts` `mergeIRs` (D9, D11).
- Make `compileProjectToIR` reconcile identical duplicate `tenant` declarations instead of erroring (D10, D11, D12) — *or* solve it capsule-side by consolidating to one shared `tenant` module. This, not command ownership, is the true gate for native multi-file merge. (~~Emit entity-scoped `command.entity`~~ — **already done in 2.4.2**, per the *Compiler probe*; D10/D14 corrected accordingly.)
- Export `./projections/{kysely,analytics,llm-context,materialized-views}` (D25).
- Native client projection options: `entityRouteBase`, `readEnvelopeKey`/`fallbackKey`, `fetchAdapter`, command-envelope type (D23).
- (Optional) `ir.enums → Prisma enum` emission (D22); export a public `mergeIR(ir[])` (D12); `GenericPrismaStore` status-based soft-delete + batch/upsert (D27, D2, D3).
...
### Upstream-status reconciliation (updates D9 / D11 / D12 / D25)

The coverage pass surfaced — and an empirical re-probe of capsule's **installed dist** confirmed — a precise split between what's in capsule's pinned 2.4.2 and what's since landed in the Manifest working copy:

- **In installed 2.4.2 (what capsule runs):** `multi-compiler` `mergeIRs` merges reactions + roles but **not sagas**, and there is **no exported `mergeIR`**. So **D9/D11/D12 stand as written for capsule today** — adopting native multi-file merge on 2.4.2 would silently drop all 5 sagas. (Verified: `node_modules/@angriff36/manifest/dist/manifest/multi-compiler.js` — `mergeIRs` body has no `sagas`; only internal `mergeIRs`, no `mergeIR` export.)
- **In the Manifest working copy (post-2.4.2, pending release):** `mergeIR` is now exported (`multi-compiler.ts:219`) and sagas are merged (`:263`, `:357`); the four projection subpaths (`kysely`, `analytics`, `llm-context`, `materialized-views`) are now in `package.json` exports. So the **upstream prerequisites for D9/D11/D12/D25 are already addressed in source** — they reach capsule on the next `@angriff36/manifest` publish + dependency bump. **Action:** when capsule bumps past 2.4.2, re-verify and then D11/D12 native-merge adoption + D25 stable subpath imports unblock together.
...
### U5 — Native multi-file compile/merge unused (three hand-rolled merges) · `MEDIUM` · partial · native? yes (with upstream caveat) · compounds D10/D11/D12
...
2. **Official Manifest method.** `compileProjectToIR({entries, host, basePath})` (topo-sort, cycle detection, cross-file validation, single deterministic merge) + `mergeIR(irs)` for pre-compiled IRs. End-state: one native merge replaces all three. **Blocked on three things, in order:** (a) consolidate the per-file `tenant` blocks (U6) so the duplicate-tenant hard error stops firing; (b) upgrade past 2.4.2 so native merge includes sagas and exports `mergeIR` (both landed in the working copy post-release — see *Upstream-status reconciliation*); (c) confirm the corpus has no intentional duplicate entities/enums/commands (native errors where capsule's merge warns-and-drops). Until (a)+(b) land, do **not** adopt — see D9/D11/D12.
...
### U6 — KEYSTONE: no shared base module; `tenant` repeated in all 102 files · `MEDIUM` (low-effort, high-leverage) · confirmed · native? yes · unblocks U4/U5 + D10/D11/D12

1. **The glue.** The identical line `tenant tenantId : string from context.tenantId` is declared in **all 102** files (a single distinct line across the corpus); `use` is used in exactly one file (which *still* re-declares its own tenant). There is no shared/base `.manifest`. This per-file repetition is the single root blocker for native multi-file merge, mixins, and inheritance.
2. **Official Manifest method.** Define the `tenant` declaration (and common audit/timestamp scaffolding) **once** in a base module and `use` it everywhere, removing the 101 redundant `tenant` blocks. End-state: `compileProjectToIR` no longer hard-errors on duplicate tenants → U4 (mixins), U5 (native merge), and D10/D11/D12 all unblock. This is the cheapest highest-leverage change in either part of the register.
3. **Blast radius.** `manifest/source/**/*.manifest` (remove the repeated `tenant` line; add `use "<base>"`), one new base `.manifest`.

### U7 — Closed value sets modeled as `string`, not `enum` · `MEDIUM` · partial · native? yes · compounds D22
...
- **U6** — consolidate the 102 per-file `tenant` blocks into one shared base module. Unblocks U4 (mixins), U5 (native merge), and Part I's D10/D11/D12 in one move.

**Adoptable now (native exists in 2.4.2, no upstream needed):**
- U1 (role hierarchy), U2 (referential actions in IR + scrape fix), U3 (`manifest coverage`), U8 (`schedule` declarations), U10 (native approval gate), U12 (stop hand-storing computed), U14/U22 (LSP + VS Code), U16–U21 (CLI dev-tools), U25–U27 (native constraint builtins).
...
- U9 (sagas) — decide saga-vs-middleware; if adopting, harden the merge (D9) first.

**Blocked on upstream Manifest (won't yield a working system otherwise):**
- U7 enum DB emission (`ir.enums → Prisma enum`, D22); U5 native-merge completeness (now in working copy, pending publish — D9/D11/D12); U13 configurable direct-write detector identifier/roots; the D23 client projection options.
