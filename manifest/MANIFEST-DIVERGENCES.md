...
> **▲ Version status — updated 2026-06-23: capsule installs `@angriff36/manifest@2.16.1`** (root + apps/api + manifest/runtime + mcp-server pins; `kitchen.provenance.json` / `kitchen.merge-report.json` confirm compiler **2.16.1**). Prior banner text referencing **2.5.0/2.5.1** is historical.
>
> **✅ U6 keystone — DONE.** Single `tenant` in `manifest/source/_base.manifest`; all **103** domain files `use "../_base.manifest"`. Native `compileProjectToIR` in `manifest/scripts/compile.mjs` (see `verify-invariants.mjs`). Current merged IR (`kitchen.merge-report.json`): **213 entities / 1059 commands / 1034 events / 2 sagas / 10 reactions / 270 compile warnings** (mostly nullable `datetime?` fields on `create`).
>
> **Still open (source audit 2026-06-23):** U1 role hierarchy barely adopted (~95 files still use `user.role in [...]` vs 3 files with `roleAllows`); U7 enums only in **15** files (most status fields remain `string`); schema-drift baseline still holds **58** violations across **12** entities; many cross-entity flows use runtime middleware instead of reactions (documented in `platform/reactions.manifest`).
...
**How it was produced:** an 8-dimension audit of `capsule-pro` against `@angriff36/manifest`, adversarially re-verified against the installed compiler (currently **2.16.1**). Findings that turned out to be thin wrappers, legitimate infrastructure, or already-correct usage were dropped (see the *Verified NOT Divergences* appendix).
...
2. **Official Manifest method.** Mixed — be precise about what's native vs not. **(a) Enums — ✅ SHIPPED in 2.5.1 (capsule now on 2.5.1):** the Prisma projection now emits `enum X { … }` blocks, types enum-valued properties as the enum instead of `String`, emits bare `@default(member)`, and places `@@schema` on enums under multiSchema (`src/manifest/projections/prisma/generator.ts`). **Action now unblocked:** author the vocabularies as `enum` in `.manifest` source and **retire the scrape** (`generate-full-schema.mjs:696-862`). **(b) Infra tables:** do **not** replace `OutboxEvent`/audit/`ApprovalHistory` with native `./outbox/postgres`+`./audit/postgres`+`./approval/postgres` here — those ship SQL DDL (`manifest_outbox_entries` etc.) with incompatible shapes and no Prisma-model output; swapping is a data migration, not codegen. **Actionable now:** migrate pure reference/lookup tables (WasteReason, status_types, units, unit_conversions, skills) into `.manifest` source to shrink the partial; document which remaining models are truly external (auth/tenant backbone, supplier sync, idempotency/telemetry); replace the manual "delete from partial as you author" step with a build assertion that fails if a model name appears in both sets.
