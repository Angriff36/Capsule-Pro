...
> **✅ Enum emission (D22 / U7) — SHIPPED in 2.5.1; capsule now on 2.5.1.** The Manifest Prisma projection now emits `enum` blocks (`src/manifest/projections/prisma/generator.ts`: `emitEnum`/`resolveEnumSchemaName`, enum-valued columns typed as the enum, **bare** `@default(member)`, `@@schema` on enums under multiSchema). Released as `@angriff36/manifest@2.5.1` (cut-release green) and capsule's 4 pins + lockfile bumped 2.5.0 → 2.5.1. **Now adoptable:** author the status vocabularies as `enum` in `.manifest` source and **retire the enum-scrape** (`generate-full-schema.mjs:696-862`). Per-entry "2.5" stamps appear inline below.
