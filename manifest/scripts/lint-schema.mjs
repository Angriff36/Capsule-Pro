...
const PATHS = {
  ...
  allowlist: path.join(ROOT, "manifest/governance/schema-naming-allowlist.json"),
...
export function parseModels(schemaText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!current) {
      ...
      if (m) current = { name: m[1], hasMap: false, mapValue: null, line: i + 1 };
      continue;
    }
...
export function evaluate(models, allowlist) {
  ...
  const pascalTable = new Set(allowlist?.pascalCaseTableExceptions?.models ?? []);
  ...
  for (const name of legacy) {
    if (!modelNames.has(name)) stale.push({ list: "legacySnakeCaseModels", model: name });
  }
  for (const name of pascalTable) {
    if (!modelNames.has(name)) stale.push({ list: "pascalCaseTableExceptions", model: name });
  }
  ...
  return {
    ...
    summary: {
      ...
      pascalCaseTables: models.filter((m) => /[A-Z]/.test(m.mapValue ?? m.name)).length,
...
function runSelfTest() {
  ...
  expect("R1 flags bad_thing", violations.some((v) => v.rule === "R1" && v.model === "bad_thing"));
  expect("R2 flags BadTable (no @@map)", violations.some((v) => v.rule === "R2" && v.model === "BadTable"));
  expect("R2 flags AnotherBad (PascalCase @@map)", violations.some((v) => v.rule === "R2" && v.model === "AnotherBad"));
  expect("does NOT flag GoodThing", !violations.some((v) => v.model === "GoodThing"));
  expect("does NOT flag allowlisted legacy_thing", !violations.some((v) => v.model === "legacy_thing"));
  expect("does NOT flag allowlisted FrozenPascalTable", !violations.some((v) => v.model === "FrozenPascalTable"));
  ...
  expect("R3 flags ghost_model as stale", stale2.some((s) => s.model === "ghost_model"));
  // suggestion helper sanity
  expect("toSnake(AnotherBad) === another_bad", toSnake("AnotherBad") === "another_bad");
...
function writeReports(report) {
  if (!existsSync(PATHS.outDir)) mkdirSync(PATHS.outDir, { recursive: true });
  writeFileSync(path.join(PATHS.outDir, "schema-naming.json"), JSON.stringify(report, null, 2));

  const lines = [];
  lines.push("# Schema Naming Convention Lint");
  lines.push("");
  lines.push(`- Total models: **${report.summary.totalModels}**`);
  lines.push(`- PascalCase models: ${report.summary.pascalCaseModels} · snake_case (legacy): ${report.summary.snakeCaseModels}`);
  lines.push(`- PascalCase physical tables (frozen): ${report.summary.pascalCaseTables}`);
  lines.push(`- Violations: **${report.summary.violations}** · Stale allowlist entries: ${report.summary.staleAllowlistEntries}`);
  lines.push("");
  if (report.violations.length) {
    ...
    for (const v of report.violations) {
      lines.push(`| ${v.rule} | \`${v.model}\` | ${v.line} | ${v.detail.replace(/\|/g, "\\|")} |`);
    }
    lines.push("");
  } else {
  ...
  if (report.stale.length) {
    ...
    lines.push("");
    for (const s of report.stale) lines.push(`- \`${s.model}\` (${s.list})`);
    lines.push("");
  }
...
function main() {
  ...
  const allowlist = existsSync(PATHS.allowlist)
    ...
    : { legacySnakeCaseModels: { models: [] }, pascalCaseTableExceptions: { models: [] } };
  ...
  const report = { generatedAt: new Date().toISOString(), summary, violations, stale };
  ...
  for (const s of stale) {
    console.log(`  [R3 stale] ${s.model} no longer in schema (${s.list}) — remove from allowlist`);
  }
  ...
  if (failures === 0) {
    ...
    console.log(`  ⚠ ${violations.length} violations, ${stale.length} stale allowlist entries`);
  }
