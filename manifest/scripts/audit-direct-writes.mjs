 ...
 *      with `manifest/governance/bypasses.json`.
 *   5. Emits a JSON report and a Markdown report to `manifest/reports/direct-writes/`.
 *
 ...
 *                       and not covered by manifest/governance/bypasses.json.
...
// Canonical layout: reports under manifest/reports/<audit-name>/. Old root
// manifest-audit/ dir is no longer canonical (see docs/audits/manifest-artifact-layout-adr.md).
const OUT_DIR = path.join(ROOT, "manifest", "reports", "direct-writes");
...
async function main() {
  ...
  // Canonical governance paths (see docs/audits/manifest-artifact-layout-adr.md).
  const bypasses = await readJsonSafe(
    path.join(ROOT, "manifest", "governance", "bypasses.json")
  );
  const bypassPaths = loadBypassPaths(bypasses);

  const entitiesDoc = await readJsonSafe(
    path.join(ROOT, "manifest", "governance", "entities.json")
  );
