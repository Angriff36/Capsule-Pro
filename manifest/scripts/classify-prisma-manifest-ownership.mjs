...
const hasCreate = new Set(
  (ir.commands || []).filter((c) => c.name === "create").map((c) => c.entity)
);
const commandsByEntity = new Map();
for (const c of ir.commands || []) {
  if (!commandsByEntity.has(c.entity)) {
    commandsByEntity.set(c.entity, []);
  }
...
if (
  counts.irNoModel_durable +
    counts.irNoModel_memory +
    counts.irNoModel_noStore !==
  counts.irNoModel
) {
...
out.push("|---|---|---|");
out.push(
  `| A durable_generated | IR entity, store=durable (projection emits) | ${counts.A} |`
);
out.push(
  `| B manifest_memory | IR entity, store=non-durable (flip candidates) | ${counts.B} |`
);
...
out.push(
  `| — irNoModel | IR entity, no live Prisma model | ${counts.irNoModel} |`
);
out.push("");
out.push(
...
tbl("A — durable_generated", buckets.A.sort(byName), [
  "name",
  "table",
  "pgSchema",
  "commands",
...
tbl("D — prisma-only legacy (no IR entity)", buckets.D.sort(byName), [
  "name",
  "table",
  "pgSchema",
]);
...
out.push(
  irNoModelSub.durable_no_model.map((x) => x.name).join(", ") || "(none)"
);
out.push("\n## IR entities with NO live Prisma model — memory_no_model\n");
out.push(
  irNoModelSub.memory_no_model.map((x) => x.name).join(", ") || "(none)"
);
out.push("\n## IR entities with NO live Prisma model — no_store_no_model\n");
out.push(
  irNoModelSub.no_store_no_model.map((x) => x.name).join(", ") || "(none)"
);

const md = out.join("\n");
const json = JSON.stringify(
  {
    generatedBy: "classify-prisma-manifest-ownership.mjs",
    counts,
    buckets,
    irNoModel,
    irNoModelSub,
...
if (writeDocs) {
  const docsPath = resolve(
    root,
    "docs/manifest/model-ownership-classification.md"
  );
...
process.stdout.write(
  `A=${counts.A} B=${counts.B} (create:${counts.B_withCreate}) C=${counts.C} D=${counts.D} ` +
    ...
    (writeDocs
      ? "wrote docs/manifest/model-ownership-classification.md\n"
      : "") +
    (errors.length
      ? `INVARIANTS FAILED:\n- ${errors.join("\n- ")}\n`
      : "invariants OK\n")
