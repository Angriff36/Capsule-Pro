...
const CREATED_AT_PROP = /^\s*property\s+(required\s+)?createdAt:\s*datetime\s*=\s*now\(\)\s*$/;
const UPDATED_AT_PROP = /^\s*property\s+(required\s+)?updatedAt:\s*datetime\s*=\s*now\(\)\s*$/;
...
let stats = { filesProcessed: 0, entitiesModified: 0, propsRemoved: 0, mutationsRemoved: 0, timestampsAdded: 0 };
...
console.log(`Total lines removed: ${stats.propsRemoved + stats.mutationsRemoved}`);
