...
// D9 reactions: expected count is DERIVED from active `on <Event> run <Entity.cmd>` declarations
// in source (reactions are deliberately converted to middleware over time — see
// manifest/source/platform/reactions.manifest — so a hardcoded floor goes stale).
const ON_REACTION_RE = /^\s*on\s+\w+\s+run\s+/gm;
...
async function main() {
  ...
  const domainFiles = files.filter(
    (f) => !f.replace(/\\/g, "/").endsWith("_base.manifest")
  );
  ...
  check(
    "U6",
    "_base.manifest exists",
    existsSync(BASE_FILE),
    `expected ${BASE_FILE}`
  );
  check(
    "U6",
  ...
  const tenantDeclarers = files.filter((f) =>
    TENANT_RE.test(readFileSync(f, "utf8"))
  );
  check(
    "U6",
  ...
  const domainTenantOffenders = domainFiles.filter((f) =>
    TENANT_RE.test(readFileSync(f, "utf8"))
  );
  check(
    "U6",
  ...
  const missingUse = domainFiles.filter(
    (f) =>
      !/(^|\n)\s*use\s+["'][^"']*_base\.manifest["']/.test(
        readFileSync(f, "utf8")
      )
  );
  check(
    ...
    missingUse.length === 0,
    `${missingUse.length} file(s) missing the base 'use': ${missingUse
      .slice(0, 8)
      .map((f) => relative(SRC, f))
      .join(", ")}${missingUse.length > 8 ? " ..." : ""}`
  );
  ...
  const compileSrc = existsSync(join(SRC, "../scripts/compile.mjs"))
    ...
    : readFileSync(
        resolve(process.cwd(), "manifest/scripts/compile.mjs"),
        "utf8"
      );
  check(
    "D11",
    "compile.mjs uses native compileProjectToIR",
    /compileProjectToIR/.test(compileSrc) &&
      /@angriff36\/manifest\/multi-compiler/.test(compileSrc),
  ...
  check(
    ...
    !(
      /\bmergeIrs\s*\(/.test(compileSrc) ||
      /import[^;]*\bmergeIrs\b/.test(compileSrc)
    ),
  ...
  const { ir: fresh, diagnostics } = await compileProjectToIR({
    entries: files,
    host,
    basePath: SRC,
  });
  ...
  check(
    ...
    `${errors.length} error(s); first: ${errors[0]?.message ?? ""}`
  );

  if (fresh) {
    check(
      "U6",
      "merged IR has exactly one tenant",
      !!fresh.tenant,
      "merged IR has no tenant"
    );
    check(
      "counts",
      `>= ${MIN_ENTITIES} entities`,
      (fresh.entities?.length ?? 0) >= MIN_ENTITIES,
      `got ${fresh.entities?.length}`
    );
    check(
      "counts",
      `>= ${MIN_COMMANDS} commands`,
      (fresh.commands?.length ?? 0) >= MIN_COMMANDS,
      `got ${fresh.commands?.length}`
    );
    check(
      "D9",
      `sagas preserved (>= ${MIN_SAGAS})`,
      (fresh.sagas?.length ?? 0) >= MIN_SAGAS,
      `got ${fresh.sagas?.length ?? 0}`
    );
    const declaredReactions = files.reduce(
      (n, f) =>
        n + (readFileSync(f, "utf8").match(ON_REACTION_RE)?.length ?? 0),
      0
    );
    check(
      "D9",
      `reactions preserved (IR count == ${declaredReactions} active \`on\` declarations in source)`,
      (fresh.reactions?.length ?? 0) === declaredReactions,
      `got ${fresh.reactions?.length ?? 0}, source declares ${declaredReactions}`
    );
    const cmdNoEntity = (fresh.commands ?? []).filter((c) => !c.entity);
    check(
      "D14",
      "every command carries an entity",
      cmdNoEntity.length === 0,
      `${cmdNoEntity.length} command(s) without .entity: ${cmdNoEntity
        .slice(0, 5)
        .map((c) => c.name)
        .join(", ")}`
    );
  ...
  if (existsSync(IR_FILE) && fresh) {
    ...
    check(
      "ir",
      "committed IR has a single tenant",
      !!committed.tenant,
      "committed kitchen.ir.json has no tenant"
    );
    const sameCounts =
    ...
    if (clone.provenance) {
      clone.provenance.irHash = "";
    }
    const recomputed = createHash("sha256")
      .update(deterministicStringify(clone))
      .digest("hex");
    check(
      "ir",
      "committed provenance.irHash is valid",
      stored.length > 0 && stored === recomputed,
      "stored irHash != recompute — IR was hand-edited or compiled with a different algorithm"
    );
  } else {
    check(
      "ir",
      "committed kitchen.ir.json exists",
      existsSync(IR_FILE),
      `missing ${IR_FILE}`
    );
  }
  ...
  const enumIR = {
    ...
    modules: [],
    values: [],
    enums: [{ name: "Stage", values: [{ name: "lead" }, { name: "won" }] }],
    entities: [
      {
        name: "Deal",
        properties: [
          ...
          },
          {
            ...
            modifiers: ["required"],
            defaultValue: { kind: "string", value: "lead" },
          },
        ],
        computedProperties: [],
        relationships: [],
  ...
  let enumCode = "";
  try {
    const out = new PrismaProjection().generate(enumIR, {
  ...
  check(
    "D22",
    "Prisma projection emits `enum` blocks",
    /enum\s+Stage\s*\{/.test(enumCode),
    `projection output had no enum block (installed ${installedVersion}). Output head: ${enumCode.slice(0, 120)}`
  );
  check(
    "D22",
    "enum-valued column typed as the enum (not String)",
    /\bstage\s+Stage\b/.test(enumCode),
    "stage column not typed as the Stage enum"
  );
  check(
    "D22",
    "enum default emitted bare (not quoted)",
    /@default\(lead\)/.test(enumCode) && !/@default\("lead"\)/.test(enumCode),
    "enum default not bare"
  );
  ...
  check(
    "D12",
    "native public mergeIR is exported",
    mergeIROk,
    "@angriff36/manifest/multi-compiler does not export mergeIR"
  );
  ...
  console.log(`\n${"-".repeat(60)}`);
  console.log(
    `Manifest invariants: ${results.length - failed.length}/${results.length} passed`
  );
  if (failed.length > 0) {
