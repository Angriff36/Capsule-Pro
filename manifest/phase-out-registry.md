...
| Hand-written React Query hooks for manifest entities | `projections/react-query` | **GATE-READY, wiring deferred (eval 2026-06-15).** Output `apps/app/app/lib/manifest-hooks.generated.ts` is tracked + deterministic (no timestamps) → ready for a drift gate mirroring `check-openapi-drift.mjs`. But it is consumed only by a pilot re-export (`manifest-hooks-pilot.ts`), not real components, and wiring `manifest:react-query:check` into the root `package.json`/`manifest:ci` is blocked by the uncommitted half-applied `@angriff36/manifest 2.5.1→2.7.0` bump in that file (lockfile still 2.5.1). Wire once the bump is resolved. |
