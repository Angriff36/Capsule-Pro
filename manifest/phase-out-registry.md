...
**Generated root consolidation (2026-06-10, superseded 2026-07-04):** Runtime Prisma metadata now lives at **`manifest/runtime/src/generated/` only**. The former `manifest/generated/runtime/` twin (once a junction, later a diverging dual-write copy — the staleness incident class) was deleted 2026-07-04; all generators and readers (generate-prisma-model-metadata / entity-accessor / store-projection, check-accessor-config, audit-schema-drift, audit-command-param-types, build-prisma-store-options, accessor-resolution) point at the single home. Orphan Prisma client copy under `manifest/generated/models/` removed via `cleanup-generated-orphans.mjs` (wired into `manifest:build`).

**Dead-twin deletions (2026-07-04, mission WS1):** `manifest/runtime/src/api-response.ts` (572 LOC, zero importers), `manifest/runtime/src/prep-list-autogeneration.ts` + its drain route `apps/api/app/api/kitchen/prep-lists/autogenerate/` + test (dead outbox pipeline superseded by the EventConfirmed reaction + prep-list-seed-middleware chain), the alternate engine-construction layer `manifest/runtime/src/kitchen/{types,workflow,runtime-factories,instances,event-listeners,constraint-outcomes,manifest-ir-loader,commands/*}` (zero consumers outside package re-exports; `kitchen/outbox-adapter.ts` KEPT — real factory consumer), and `apps/app/app/lib/manifest-hooks-pilot.ts` (zero importers).
...
|---|---|---|
| Hand-written Zod input schemas for manifest entities | `projections/zod` | **NO-GO for now (reconfirmed 2026-07-07 cleanup).** The repo has no hand-written Zod surface to retire, and the old local wrapper `generate-zod-schemas.mjs` was deleted as dead weight because its output was gitignored and had zero consumers. If input validation is ever wired to native `zod.entity`, reintroduce it from upstream `manifest generate --surface zod.entity` as a real consumed artifact rather than restoring the dead wrapper. |
| Hand-written React Query hooks for manifest entities | `projections/react-query` | **DONE (2026-07-07 cleanup).** Retired the bespoke `generate-react-query-hooks.mjs` + `check-react-query-drift.mjs` chain and deleted the committed `apps/app/app/lib/manifest-hooks*` artifacts after confirming they had zero app consumers. The native projection remains available upstream, but this repo no longer carries an unused committed hooks surface or pretends it is a meaningful CI gate. |
| Hand-written/partial OpenAPI specs for manifest routes | `projections/openapi` | **DONE (2026-06-15; CI rewired 2026-07-07)** — `projections/openapi` is wired (`pnpm manifest:openapi` → committed `manifest/api-docs/openapi.json`, served at `/api-docs`, consumed by MCP). No hand-written OpenAPI spec existed to retire. Drift is now enforced by the consolidated artifact job (`pnpm manifest:sync-artifacts && pnpm prisma:check` + CI `git diff`), not by a standalone `manifest:openapi:check` step inside `manifest:ci`. |
...
3. CI drift gate: re-running generation produces no diff against committed artifacts. **(Schema/OpenAPI/route projection drift is now enforced by the consolidated artifact job: `pnpm manifest:sync-artifacts && pnpm prisma:check` followed by CI `git diff`. The older standalone `manifest:schema:check` / `manifest:openapi:check`-style narrative no longer describes this branch's wiring. Live `schema.prisma` replacement is still Phase 2b, not yet auto-promoted.)**
