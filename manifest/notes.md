...
## 2026-07-05 uuid config->source migration (Manifest 3.2.0)

- Manifest 3.2.0 derives `@db.Uuid` from the `uuid` type (Prisma projection). Moved the 623 `dbAttributes: {..: "Uuid"}` lines out of manifest.config.yaml into the `.manifest` source: shared mixin `TenantScoped` id/tenantId -> uuid, plus 308 live-uuid FK fields flipped string->uuid, plus 15 tableless-but-config-Uuid FKs (preserve intent for tables not yet migrated). 59 body overrides pin the 30 text-`id` + 29 text-`tenant` entities (Budget/Deal/Vendor/StaffMember/EventStaff/Sel* training/TaskBundleItem/...) so the uuid mixin does NOT wrongly derive @db.Uuid on their genuinely-text columns.
- **GROUND TRUTH = live information_schema, NOT the 0_init snapshot.** 0_init is incomplete (later migrations added columns); a static-file approach misses columns like paymentPlanId/supersededBy/approvedBy and mis-types them. Column names are inconsistent (some `tenant_id` snake, some `tenantId` camel on public.* tables) — resolve via the prisma @@map/@map, never assume snake_case. `approved_by` is uuid in some tables (inventory_transfers/purchase_requisitions/vendor_contracts) and text in others (document_versions/inventory_forecasts/inventory_suppliers) — must be per-column.
- **toTsTypes now also maps bare `uuid` -> `string`** (same class as the `json` -> `unknown` leak): the 3.2.0 types surface emits `id: uuid;` which is invalid TS. Without this, apps/app gets ~hundreds of TS2304.
- PROOF (required before commit): (1) every schema @db.Uuid <-> a live uuid column (0 dangerous, 0 incomplete via db-truth); (2) `git diff` removes ZERO @db.Uuid (no annotation lost); (3) net schema change = +61 @db.Uuid on drift columns the config had forgotten + 2 net-zero deletedAt reorders. api typecheck 0, app typecheck 295 (unchanged), manifest:ci exit 0. The uuid type does NOT leak into command-param zod schemas (no validation change).


### 2026-07-05 uuid migration — CORRECTION (complete introspection)

The initial commit d1db744db had TWO defects an independent 3-agent audit caught (fixed in b42ada64c):
1. **NEVER hard-code the schema list when introspecting.** My first `information_schema.columns` query enumerated schemas explicitly and MISSED tenant_logistics/tenant_facilities/tenant_facility/auth/testing (293 columns incl. 60 uuid). Query `information_schema.schemata WHERE schema_name NOT IN (pg_catalog, information_schema, pg_toast) AND NOT LIKE 'pg_%'` to get ALL. The miss meant DeliveryRoute (tenant_logistics.delivery_routes) was treated as tableless when it exists live; eventId/vehicleId (live uuid) never got @db.Uuid. Complete truth = 918 uuid cols across 4506 cols / 16 schemas (not 858/4213/11).
2. **The prisma schema file must be in the git add.** I committed source+config+artifacts but forgot packages/database/prisma/schema/manifest.prisma, so the committed schema lagged the source. Always stage the schema with the source.

Final state: schema DANGEROUS 0 / INCOMPLETE 0, source GAP 0 / WRONG 0 against COMPLETE db truth; text-id/tenant pins = 30/29; api typecheck 0; app typecheck 295; manifest:ci exit 0. Independent verification is what caught both — my own proofs passed against the incomplete truth (garbage-in).
