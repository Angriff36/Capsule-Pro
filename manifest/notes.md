...
**Migration-history checksum repairs (user-approved, same class as the 0_init defect):** (a) `20260610041450_repair_drift` had a stale checksum (file modified post-apply, original content unrecoverable from any clone/stash/unreachable-blob; live schema verified drift-free) — re-resolved to the committed file's sha256 via checksum-guarded single-row UPDATE. (b) `20260611144817_*` was edited post-apply during review (IF NOT EXISTS healing) — checksum re-resolved the same way. Pattern: transaction + WHERE guards on name AND old checksum + expect exactly 1 row. Also note: `.env.local` DATABASE_URL points at stale branch ep-divine-math; prisma migrate targets ep-square-dust from `.env` — standalone scripts must load `.env` only (or override with DIRECT_URL) to hit the migrate target.
