...
async function main() {
  ...
  const result = await page.evaluate(async () => {
    ...
    console.log("supportedFirstFactors:", JSON.stringify(s1.supportedFirstFactors));
    console.log("supportedSecondFactors:", JSON.stringify(s1.supportedSecondFactors));
    
    const emailCodeFactor = s1.supportedFirstFactors?.find(f => f.strategy === "email_code");
    console.log("emailCodeFactor:", emailCodeFactor);
    ...
    return { 
      ...
      emailCodeFound: !!emailCodeFactor
    };
  });
