...
async function searchApi(page, q, type = "all") {
  return await page.evaluate(async ({ q: query, type: filter }) => {
...
async function main() {
  ...
  try {
    ...
    console.log(`  Page status: ${s1Oops ? "✗ 500 ERROR (Clerk orgs=[])" : "✓ LOADED"}`);
    ...
    for (const { q, label } of queries) {
      ...
      console.log(`  "${q}" (${label}):`);
      console.log(`    Status: ${r.status}, Total: ${total}, Groups: ${groupNames.length > 0 ? groupNames.join(", ") : "none"}`);
    }

    // Search with type filters
    const typeFilters = ["events", "clients", "venues", "inventory", "knowledge"];
    console.log("\n  Type filters:");
    ...
    await page.goto(`${BASE}/search?q=dinner&type=events`, { waitUntil: "domcontentloaded", timeout: 15000 });
    await page.waitForTimeout(2000);
    ...
    const showsResults = pageHtml.includes("Search Results") || pageHtml.includes("Searching");
    const isError = pageHtml.includes("Oops") || pageHtml.includes("500");
    console.log(`  Page shows search state: ${showsResults ? "✓ YES" : "✗ NO (blank/error)"}`);
    console.log(`  Page is error-free: ${!isError ? "✓ YES" : "✗ 500 ERROR"}`);
    ...
    console.log(`  Types available for "a": ${availableTypes.join(", ") || "none"}`);
    ...
    for (const target of deepLinkTargets) {
      ...
      await linkPage.goto(`${BASE}${deepLink}`, { waitUntil: "domcontentloaded", timeout: 10000 });
      ...
      console.log(`  ${target.type} → ${deepLink}: ${is500 ? "✗ 500 ERROR" : "✓ LOADED"}`);
    ...
    console.log(`  Returns success + empty: ${apiSuccess && isEmpty ? "✓ CORRECT (no results UI)" : "✗ UNEXPECTED"}`);
    console.log(`  No server error: ${r5.status !== 500 ? "✓ CORRECT" : "✗ SERVER ERROR"}`);
    ...
    console.log(`    /search page:       ${s1Oops ? "✗ 500 BLOCKED" : "✓ LOADED"}`);
    console.log(`    /api/search:         ${s1ApiR.status === 200 ? "✓ WORKS" : "✗ FAIL"}`);
    console.log("");
    ...
    console.log(`    URL params preserved: ${hasQ && hasType ? "✓ YES" : "✗ PARTIAL"}`);
    console.log(`    Page state reflects URL: ${showsResults && !isError ? "✓ YES" : "⚠ UI 500"}`);
    console.log("");
    console.log("  S4 (Deep links):");
    console.log(`    Deep links working: ${deepLinksWorking}/5`);
    console.log("");
    console.log("  S5 (No-results UI):");
    console.log(`    Nonsense → empty results: ${isEmpty ? "✓ CORRECT" : "✗ WRONG"}`);
    console.log(`    No server error: ${r5.status !== 500 ? "✓ CORRECT" : "✗ ERROR"}`);
    console.log("=".repeat(60));
